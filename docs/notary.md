# AtipicialGo P2P signature collection (notary) service

P2P signature (notary) service is a AtipicialGo node extension that allows several
...
a solution for
[multisignature transaction forming](https://github.com/atipicial-project/atipicial/issues/1573#issue-600384746)
and described in the [proposal](https://github.com/atipicial-project/atipicial/issues/1573#issuecomment-704874472).
...
> any interaction, which is the case for oracle nodes or AtipicialFs inner ring nodes.
> 
...
[atipicial-project/atipicial#1991](https://github.com/atipicial-project/atipicial/issues/1991).
...
[atipicial-project/atipicial#1992](https://github.com/atipicial-project/atipicial/issues/1992).
...
- *Main tx*. This is the one that actually needs to be completed; some (but at
  ...
  For contract witness only empty invocation scripts are supported for now.
  
  This transaction must have `Notary assisted` attribute with the number of `NKeys`
...
See the [AtipicialGo P2P signature extensions](#AtipicialGo P2P signature extensions) on how
to enable notary-related extensions on chain and
[AtipicialGo Notary service node module](#AtipicialGo Notary service node module) on how to
set up Notary service node.

## Environment setup

To run P2P signature collection service on your network, you need to do:
* Set up [`P2PSigExtensions`](#AtipicialGo P2P signature extensions) for all nodes in
  the network.
* Set notary node keys in `RoleManagement` native contract.
* [Configure](#AtipicialGo Notary service node module) and run appropriate number of
...
### AtipicialGo P2P signature extensions

As far as Notary service is an extension of the standard AtipicialGo node, it should be
...
[AtipicialGo Notary service](#AtipicialGo-Notary-service-node-module) documentation.
...
### AtipicialGo Notary service node module

AtipicialGo node can act as notary service node (the node that accumulates notary
...
[P2PSignatureExtensions](#AtipicialGo P2P signature extensions) are properly set up.
...
Notary native contract supports `onAEP17Payment` method. Thus, to deposit funds to
the Notary native contract, transfer the desired amount of GAS to the contract
address. Use
[func (*Client) TransferAEP17](https://pkg.go.dev/github.com/atipicial/atipicial-go@v0.97.2/pkg/rpcclient#Client.TransferAEP17)
...
To create a notary request, you can use [AtipicialGo RPC client](./rpc.md#Client). The
...
1. Prepare a list of signers with scopes for the main transaction (i.e. the
   ...
   * Each signer is either a multisignature or a standard signature or a contract
     signer or a custom AppCall signer containing System.Contract.Call in the
     verification script.
...
2. Construct a script for the main transaction (that will be `Script` transaction
   ...
   1. If the script is a contract method call, use `invokefunction` RPC API
      [func (*Client) InvokeFunction](https://pkg.go.dev/github.com/atipicial/atipicial-go@v0.97.2/pkg/rpcclient#Client.InvokeFunction)
   ...
   2. If the script is more complicated than just a contract method call,
      ...
      [func (*Client) InvokeScript](https://pkg.go.dev/github.com/atipicial/atipicial-go@v0.97.2/pkg/rpcclient#Client.InvokeScript)
...
4. Construct the list of main transaction attributes (that will be `Attributes`
   ...
   `NKeys` equals the overall number of the keys to be collected excluding
   notary and other contract-based witnesses. For m out of n multisignature
   signer `NKeys = n`. For standard signature signer `NKeys = 1`. For contract
   signer `NKeys = 0`. For custom AppCall-based signer `NKeys` equals to the
   number of arguments of the calling contract method missing from the
   verification script.
...
7. Construct a list of main transactions witnesses (that will be `Scripts`
   ...
   - A multisignature witness must have regular `Verification` script filled even
     ...
     `Invocation` script **should be empty**.
   - A custom AppCall witness must have `Verification` script containing
     System.Contract.Call with corresponding parameters and *some* of the
     arguments (`k`, where `0 <= k <= n`) of the calling method packed into array
     by `PACK` opcode. `Invocation` script should contain a number of arguments
     (up to `n-k`) or allowed to be empty if the script will be collected from
     other notary requests or if verification script already contains all
     parameters.
8. Calculate network fee for the transaction (that will be `NetworkFee`
   transaction field). Use [func (*Client) CalculateNetworkFee](https://pkg.go.dev/github.com/atipicial/atipicial-go@v0.99.2/pkg/rpcclient#Client.CalculateNetworkFee)
   method with the main transaction given to it.
9. Fill in all signatures that can be provded by the client creating request,
   that includes simple-signature accounts, multisignature accounts and custom
   AppCall-based accounts where the client has one of the keys (in which case an
   invocation script is created that pushes just one signature onto the stack).
10. Sign and submit P2P notary request. Use
    [func (*Actor) Notarize](https://pkg.go.dev/github.com/atipicial/atipicial-go/pkg/rpcclient/notary#Actor.Notarize) for it.
    ...
    [func (*Actor) SendRequest](https://pkg.go.dev/github.com/atipicial/atipicial-go/pkg/rpcclient/notary#Actor.SendRequest)
    API.
...
[AtipicialGo notification subsystem](./notifications.md) to track request addition and
removal:
...
- transfer initial supply of ATC and GAS from a committee multisignature account to
...
[uses Notary subsystem](https://github.com/github.com/atipicial/atipicialfs-node/pull/404) to
...
[github.com/atipicial/atipicialfs-node#47](https://github.com/github.com/atipicial/atipicialfs-node/issues/47)),
to reduce container creation delay
(see [github.com/atipicial/atipicialfs-node#519](https://github.com/github.com/atipicial/atipicialfs-node/issues/519))
etc.

### Contract-sponsored (free) transactions

The original problem and solution are described in
[atipicial-project/atipicial#2577](https://github.com/atipicial-project/atipicial/issues/2577) discussion.
