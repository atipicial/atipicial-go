# AtipicialGo consensus node

AtipicialGo node can act as a consensus node. A consensus node differs from a regular
...
While regular nodes on Atipicial network don't need any special keys, CNs always have
...
While AtipicialGo can be very conservative with its resource consumption, public
...
AtipicialGo is a single binary that can be run on any modern GNU/Linux
...
No additional packages are needed for AtipicialGo CN.

### Installation

Download AtipicialGo binary [from
Github](https://github.com/atipicial/atipicial-go/releases) or use [Docker
image](https://hub.docker.com/r/atipicial/atipicial-go). It has everything included,
no additional plugins needed.

Take an appropriate (mainnet/testnet) configuration [from the
repository](https://github.com/atipicial/atipicial-go/tree/master/config) and save
...
```
where `wallet.json` is a path to your AEP-6 wallet and `welcometotherealworld`
is a password to your CN key. Run the node in a regular way after that:

```
$ atipicial-go node --mainnet --config-path ./
```
...
To register as a candidate, use atipicial-go as CLI command with an external RPC
...
Candidate registration is performed via GAS transfer to native AtipicialCoin contract
that costs 1000 GAS by default. You need to provide your wallet and address to
atipicial-go command:
```
$ atipicial-go wallet candidate register -a NiKEkwz6i9q6gqfCizztDoHQh9r9BtdCNa -w wallet.json -r http://localhost:10332
```
where `NiKEkwz6i9q6gqfCizztDoHQh9r9BtdCNa` is your address, `wallet.json` is a
path to AEP-6 wallet file and `http://localhost:10332` is an RPC node to
use.
...
Candidate registration via call to `registerCandidate` method of native AtipicialCoin
...
```
$ atipicial-go wallet candidate register -a NiKEkwz6i9q6gqfCizztDoHQh9r9BtdCNa -w wallet.json -r http://localhost:10332 --useRegisterCall
```

### Voting

After registration is completed, if you own some ATC, you can also vote for your
...
```
$ atipicial-go wallet dump-keys -w wallet.json
...
use:
```
$ atipicial-go wallet candidate vote -a NiKEkwz6i9q6gqfCizztDoHQh9r9BtdCNa -w wallet.json -r http://localhost:10332 -c 0363f6678ea4c59e292175c67e2b75c9ba7bb73e47cd97cdf5abaf45de157133f5

```
where `NiKEkwz6i9q6gqfCizztDoHQh9r9BtdCNa` is the voter's address, `wallet.json`
is the AEP-6 wallet file path, `http://localhost:10332` is the RPC node address and
...
## Private AtipicialGo network
### Using existing Dockerfile

atipicial-go comes with two preconfigured private network setups, the first one has
...
AtipicialGo default privnet configuration is made to work with four-node consensus,
...
Examples can be found at `config/protocol.privnet.docker.one.yml` (`two`, `three` etc.).
    1. Add `Consensus` section with `Enabled: true` field and an
       `UnlockWallet` subsection with `Path` and `Password` strings for AEP-6
...
3. Start all nodes with `atipicial-go node --config-path <dir-from-step-2>`.
