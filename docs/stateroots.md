# AtipicialGo state validation

AtipicialGo supports state validation using  stateroots and can also act as state
validator (run state validation service).

All AtipicialGo nodes always calculate MPT root hash for data stored by contracts.
Unlike in Atipicial Legacy, this behavior can't be turned off. They also process
...
AtipicialGo also supports protocol extension to include state root hashes right into
header blocks. It's not compatible with regular Atipicial  state validation
service and it's not compatible with public Atipicial  networks, but you can use
