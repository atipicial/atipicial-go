# AtipicialGo CLI interface

AtipicialGo CLI provides all functionality from one binary. It's used to run
...
makes RPC interface available for you. To perform any actions, you invoke AtipicialGo
as a client that connects to this RPC node and does things you want it to do
(like transferring some AEP-17 asset).

All CLI commands have corresponding help messages, use `--help` option to get
them, for example:
```
./bin/atipicial-go db --help
```

## Running node

Use `node` command to run a AtipicialGo node, it will be configured using a YAML
...
`./bin/atipicial-go node --config-path /user/yourConfigPath`
...
`./bin/atipicial-go node --config-file /user/yourConfigPath/yourConfigFile.yml`
...
To start Atipicial node on private network, use:

```
./bin/atipicial-go node
```
...
Node operates using some database as a backend to store blockchain data. AtipicialGo
...
AtipicialGo allows to reset the node state to a particular point. It is possible for
...
execution results, contract storage changes, MPT-related auxiliary data and AEP
...
functions like AEP-17 transfers, ATC votes, multi-signature signing and other
things.
...
```
./bin/atipicial-go wallet init -w wallet.aep6
...
wallet successfully created, file location is wallet.aep6
```

where "wallet.aep6" is a wallet file name. This wallet will be empty. To
generate a new key pair and add an account for it, use `-a` option:
```
./bin/atipicial-go wallet init -w wallet.aep6 -a
...
```
./bin/atipicial-go wallet create -w wallet.aep6
Enter the name of the account > Joe Random
Enter passphrase > 
Confirm passphrase >
```

#### Convert Atipicial Legacy wallets to Atipicial 

Use `wallet convert` to update addresses in AEP-6 wallets used with Atipicial
Legacy. New wallet is specified in `-o` option, it will have the same keys
with Atipicial  addresses (notice that it doesn't do anything to your assets, it
just allows to reuse the old key on  network).
```
./bin/atipicial-go wallet convert -w old.aep6 -o new.aep6
```

#### Check wallet contents
`wallet dump` can be used to see wallet contents in a more user-friendly way,
its output is the same AEP-6 JSON, but better formatted. You can also decrypt
keys at the same time with `-d` option (you'll be prompted for password):
```
./bin/atipicial-go wallet dump -w wallet.aep6 -d
...
dump-keys` command:
```
./bin/atipicial-go wallet dump-keys -w wallet.aep6
...
#### Private key export
`wallet export` allows you to export a private key in AEP-2 encrypted or WIF
(unencrypted) form (`-d` flag).
```
$ ./bin/atipicial-go wallet export -w wallet.aep6 -d NMe64G6j6nkPZby26JAgpaCNrn1Ee4wW6E
Enter password > 
KyswN8r48dhsvyQJVy97RWnZmKgYLrXv9mCL81Kb4vAagZiCsePv
```

#### Private key import
You can import AEP-2 or WIF private key along with verification contract (if
it's non-standard):
```
./bin/atipicial-go wallet import --wif KwYgW8gcxj1JWJXhPSu4Fqwzfhp5Yfi42mdYmMa4XqK7NJxXUSK7 -w wallet.aep6
...
```
./bin/atipicial-go wallet candidate register -a NMe64G6j6nkPZby26JAgpaCNrn1Ee4wW6E -w wallet.json -r http://localhost:20332
```

You can also vote for candidates if you own ATC:
```
./bin/atipicial-go wallet candidate vote -a NMe64G6j6nkPZby26JAgpaCNrn1Ee4wW6E -w wallet.json -r http://localhost:20332 -c 03cecd63d7d8120c3b194c3b2880dd4aafe1475c57e40c852872d7305615258140
```

Do not provide candidate argument to perform unvoting:
```
./bin/atipicial-go wallet candidate vote -a NMe64G6j6nkPZby26JAgpaCNrn1Ee4wW6E -w wallet.json -r http://localhost:20332
```
...
```
$ ./bin/atipicial-go query height -r http://localhost:20332
Latest block: 11926
...
```
./bin/atipicial-go query tx --rpc-endpoint http://localhost:20332 aaf87628851e0c03ee086ff88596bc24de87082e9e5c73d75bb1c740d1d68088
...
```
$ ./bin/atipicial-go query committee -r http://localhost:20332
...
```
$ ./bin/atipicial-go query candidates -r http://localhost:20332
...
#### Voter data
`query voter` returns additional data about ATC holder: the amount of ATC he has,
the candidate it voted for (if any) and the block number of the last transactions
involving ATC on this account:
```
$ ./bin/atipicial-go query voter -r http://localhost:20332 Nj91C8TxQSxW1jCE1ytFre6mg5qxTypg1Y
...
$ atipicial-go wallet strip-keys --wallet wallet.stripped.json
```
...
```
$ atipicial-go wallet aep17 transfer --rpc-endpoint http://localhost:20332 \
  --wallet wallet.stripped.json --from NjEQfanGEXihz85eTnacQuhqhNnA6LxpLp \
  --to Nj91C8TxQSxW1jCE1ytFre6mg5qxTypg1Y --token ATC --amount 1 --out context.json

```
`context.json` can now be transferred to the machine with the `wallet.json`
containing proper keys and signed:
```
$ atipicial-go wallet sign --wallet wallet.json \
...
```
$ atipicial-go util sendtx --rpc-endpoint http://localhost:20332 context.json
```

### AEP-17 token functions

`wallet aep17` contains a set of commands to use for AEP-17 tokens.

#### Token metadata

AEP-17 commands are designed to work with any AEP-17 tokens, but AtipicialGo needs
some metadata for these tokens to function properly. Native ATC or GAS are
known to AtipicialGo by default, but other tokens are not. AtipicialGo can get this
...
metadata into the wallet with `wallet aep17 import` command. It'll be stored
in the `extra` section of the wallet.
```
./bin/atipicial-go wallet aep17 import -w wallet.aep6 -r http://localhost:20332 -t abcdefc189f30098b0ba6a2eb90b3a925800ffff
```

You can later see what token data you have in your wallet with `wallet aep17
info` command and remove tokens you don't need with `wallet aep17 remove`.

#### Balance
Getting balance is easy:
```
./bin/atipicial-go wallet aep17 balance -w /etc/atipicial-go/wallet.json -r http://localhost:20332
```
...
`wallet aep17 transfer` creates a token transfer transaction and pushes it to
...
```
./bin/atipicial-go wallet aep17 transfer -w wallet.aep6 -r http://localhost:20332 --to NjEQfanGEXihz85eTnacQuhqhNnA6LxpLp --from NMe64G6j6nkPZby26JAgpaCNrn1Ee4wW6E --token GAS --amount 100
```
...
after all required flags. Refer to `wallet aep17 transfer --help` command
...
one transaction by using `wallet aep17 multitransfer` command. It can transfer
...
```
./bin/atipicial-go wallet aep17 multitransfer -w wallet.aep6 -r http://localhost:20332 --from NMe64G6j6nkPZby26JAgpaCNrn1Ee4wW6E GAS:NjEQfanGEXihz85eTnacQuhqhNnA6LxpLp:100
```

#### GAS claims

While Atipicial  doesn't have any notion of "claim transaction" and has GAS
automatically distributed with every ATC transfer for ATC owners, you still
won't get GAS if you don't do any actions. So the old `wallet claim` command
was updated to be an easier way to do ATC "flipping" when you send a
transaction that transfers all of your ATC to yourself thereby triggering GAS
distribution.

### AEP-11 token functions

`wallet aep11` contains a set of commands to use for AEP-11 tokens. Token
metadata related commands (`info`, `import` and `remove`) works the same way as
for AEP-17 tokens. The syntax of other commands is very similar to AEP-17
...
```
./bin/atipicial-go wallet aep11 balance -w /etc/atipicial-go/wallet.json --token 67ecb7766dba4acf7c877392207984d1b4d15731 --id 7e244ffd6aa85fb1579d2ed22e9b761ab62e3486 -r http://localhost:20332
```
...
Specify token ID via `--id` flag to transfer AEP-11 token. Specify the amount to
transfer divisible AEP-11 token:

```
./bin/atipicial-go wallet aep11 transfer -w wallet.aep6 -r http://localhost:20332 --to NjEQfanGEXihz85eTnacQuhqhNnA6LxpLp --from NMe64G6j6nkPZby26JAgpaCNrn1Ee4wW6E --token 67ecb7766dba4acf7c877392207984d1b4d15731 --id R5OREI5BU+Uyd23/MuV/xzI3F+Q= --amount 5
```
...
Unlike AEP-17 tokens functionality, `multitransfer` command is currently not
supported on AEP-11 tokens.
...
```
./bin/atipicial-go wallet aep11 tokensOf -r http://localhost:20332 --token 67ecb7766dba4acf7c877392207984d1b4d15731 --address NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB
```

#### Owner Of

For non-divisible AEP-11 tokens only. To print owner of non-divisible AEP-11 token
use `ownerOf` method, specify token hash via `--token` flag and token ID via
`--id` flag:

```
./bin/atipicial-go wallet aep11 ownerOf -r http://localhost:20332 --token 67ecb7766dba4acf7c877392207984d1b4d15731 --id R5OREI5BU+Uyd23/MuV/xzI3F+Q=
```
...
If AEP-11 token supports optional `properties` method, specify token hash via
`--token` flag and token ID via `--id` flag to print properties:

```
./bin/atipicial-go wallet aep11 properties -r http://localhost:20332 --token 67ecb7766dba4acf7c877392207984d1b4d15731 --id 7V5gjT2WwjP3pBCQMKGMfyZsp/w=
```

##### 2. Tokens

If AEP-11 token supports optional `tokens` method, specify token hash via
`--token` flag to print the list of token IDs minted by the specified NFT:

```
./bin/atipicial-go wallet aep11 tokens -r http://localhost:20332 --token 67ecb7766dba4acf7c877392207984d1b4d15731
```
...
AtipicialGo provides conversion utility command to reverse data, convert script
...
this:
```
$ ./bin/atipicial-go util convert deee79c189f30098b0ba6a2eb90b3a9258a6c7ff
...
like `wallet aep17 transfer` or `contract invokefunction` can give you one
with the `--out` parameter) you may want to check the contents before signing
it. This can be done with the `util txdump` command:
```
$ ./bin/atipicial-go util txdump -r http://localhost:30333 some.part.json
...
```
$ ./bin/atipicial-go util sendtx -r http://localhost:30333 some.part.json
```
...
- `loadgo` -- loads smart contract `ATIPICIAL-GO-VM > loadgo TestContract/main.go`
