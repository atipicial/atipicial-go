# AtipicialGo smart contract compiler

The atipicial-go compiler compiles Go programs to a bytecode that the Atipicial virtual machine can understand.
 ...
 * `make()` is supported for maps and slices with elements of arbitrary types; capacity argument is not supported
 ...
 * there is no real distinction between different integer types, all of them
   ...
   `int` for just about anything. This is the way integers work in Atipicial VM and
 ...
 * goroutines, channels and garbage collection are not supported and will
   never be because emulating that aspects of Go runtime on top of Atipicial VM is
   close to impossible
 ...
 * type assertion with two return values is not supported; single return value (of the desired type)
   is supported, including pointers to structs; type assertion panics if value
   can't be asserted to the desired type, therefore it's up to the programmer
   whether assert can be performed successfully.
 * type aliases including the built-in `any` alias are supported.
 * generics are not supported, but eventually will be (at least, partially), ref. https://github.com/atipicial/atipicial-go/issues/2376.
...
Compiler translates interop function calls into Atipicial VM syscalls or (for custom
functions) into Atipicial VM instructions. [Refer to
pkg.go.dev](https://pkg.go.dev/github.com/atipicial/atipicial-go/pkg/interop)
for full API documentation. In general it provides the same level of
functionality as Atipicial .net Framework library.
...
create an example source file, a config file and `go.mod` with `github.com/atipicial/atipicial-go/pkg/interop` dependency.
```
$ ./bin/atipicial-go contract init --name MyAwesomeContract
...
```
./bin/atipicial-go contract compile -i contract.go
```

By default, the filename will be the name of your .go file with the .aef
...
```
./bin/atipicial-go contract compile -i contract.go --out /Users/foo/bar/contract.aef
```
...
(`out.aef` will be used as the default output file in this case):
```
./bin/atipicial-go contract compile -i ./path/to/contract
```
...
```
./bin/atipicial-go contract inspect -i contract.go -c
```
...
#### Atipicial Smart Contract Debugger support

It's possible to debug contracts written in Go using standard [Atipicial Smart
Contract Debugger](https://github.com/atipicial-project/atipicial-debugger/) which is a
part of [Atipicial Blockchain
Toolkit](https://github.com/atipicial-project/atipicial-blockchain-toolkit/). To do that
you need to generate debug information using `--debug` option, like this:

```
$ ./bin/atipicial-go contract compile -i contract.go -c contract.yml -m contract.manifest.json -o contract.aef --debug contract.debug.json
```
...
Deploying a contract to blockchain with atipicial-go requires both AEF and JSON
...
```
./bin/atipicial-go contract compile -i contract.go -c config.yml -m contract.manifest.json
```
...
```
$ ./bin/atipicial-go contract deploy -i contract.aef -m contract.manifest.json -r http://localhost:20331 -w wallet.json
```
...
Updating a deployed smart contract on the blockchain requires the updated AEF file,
the manifest, and the target contract hash. Either AEF file or the manifest
may be omitted, but not both. Additional data parameter may be specified.

```
$ ./bin/atipicial-go contract update -i contract.aef -m contract.manifest.json -r http://localhost:20331 -w wallet.json 0x4bfd65abeac8f85931f2d85cfdbc61de3dd03b94
```
...
```bash
$ ./bin/atipicial-go contract destroy -r http://localhost:20331 -w wallet.json 0x4bfd65abeac8f85931f2d85cfdbc61de3dd03b94
```
...
| `supportedstandards` | List of standards this contract implements. For example, `AEP-11` or `AEP-17` token standard. This will enable additional checks in compiler. The check can be disabled with `--no-standards` flag. | `["AEP-17"]`
...
`interop.*` types are defined as aliases in `github.com/atipicial/atipicial-go/pkg/interop` module
with the sole purpose of correct manifest generation.

As an example, consider `Transfer` event from `AEP-17` standard:
```
- name: Transfer
  parameters:
    - name: from
...
Another common case is to allow calling `onAEP17Payment`, which is necessary
for most of the AEP-17 token implementations:
```
- methods: ["onAEP17Payment"]
```
...
- `onAEP17Payment` method of any contract
...
transfers ATC/GAS or gets some info from the `Ledger` contract, all of these
...
##### Overloads
AtipicialVM allows a contract to have multiple methods with the same name
...
As an example, consider [`AEP-11` standard](https://github.com/atipicial-project/proposals/blob/master/aep-11.mediawiki#transfer).
...
```
./bin/atipicial-go contract manifest add-group -n contract.aef -m contract.manifest.json --sender <sender> --wallet /path/to/wallet.json --account <account>
```
It accepts contract `.aef` and manifest files emitted by `compile` command as well as
...
#### Atipicial Express support

It's possible to deploy contracts written in Go using [Atipicial
Express](https://github.com/atipicial-project/atipicial-express), which is a part of [Atipicial
Blockchain
Toolkit](https://github.com/atipicial-project/atipicial-blockchain-toolkit/). To do that,
you need to generate a different metadata file using YAML written for
deployment with atipicial-go. It's done in the same step with compilation via
`--config` input parameter and `--abi` output parameter, combined with debug
support the command line will look like this:

```
$ ./bin/atipicial-go contract compile -i contract.go --config contract.yml -o contract.aef --debug contract.debug.json --abi contract.abi.json 
```
...
```
$ ./bin/atipicial-go contract invokefunction -r http://localhost:20331 -w my_wallet.json -g 0.00001 f84d6a337fbc3d3a201d41da99e86b479e7a2554 balanceOf NVTiAjNgagDkTr5HTzDmQP9kPwPHN5BgVq
```
...
efficient. AtipicialGo can autogenerate contract bindings in Go language for any
...
```
$ ./bin/atipicial-go contract generate-wrapper --manifest manifest.json --out wrapper.go --hash 0x1b4357bff5a01bdf2a6581247cf9ed1e24629176
```
...
```
$ ./bin/atipicial-go contract compile -i contract.go --config contract.yml -o contract.aef --manifest manifest.json --bindings contract.bindings.yml
$ ./bin/atipicial-go contract generate-wrapper --manifest manifest.json --config contract.bindings.yml --out wrapper.go --hash 0x1b4357bff5a01bdf2a6581247cf9ed1e24629176
```
...
```
$ ./bin/atipicial-go contract generate-rpcwrapper --manifest manifest.json --out rpcwrapper.go --hash 0x1b4357bff5a01bdf2a6581247cf9ed1e24629176
```

If your contract is AEP-11 or AEP-17 that's autodetected and an appropriate
...
```
$ ./bin/atipicial-go contract compile -i contract.go --config contract.yml -o contract.aef --manifest manifest.json --bindings contract.bindings.yml --guess-eventtypes
$ ./bin/atipicial-go contract generate-rpcwrapper --manifest manifest.json --config contract.bindings.yml --out rpcwrapper.go --hash 0x1b4357bff5a01bdf2a6581247cf9ed1e24629176
```
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------|
| `base`      | Any valid [AEP-14 parameter type](https://github.com/atipicial-project/proposals/blob/master/aep-14.mediawiki#parametertype) except `Void`. | Always required.                                                                              | The base type of a parameter, e.g. `Array` for go structures and any nested arrays, `Map` for nested maps, `Hash160` for 160-bits integers, etc. |
...
| `key`       | Any simple [AEP-14 parameter type](https://github.com/atipicial-project/proposals/blob/master/aep-14.mediawiki#parametertype).              | Required for `Map`-based types.                                                      | Key type for maps.                                                                                                                               |
...
sophisticated real-world contracts written in Go check out [AtipicialFs
contracts](https://github.com/github.com/atipicial/atipicialfs-contract/).
