...
[relevant godocs page](https://godoc.org/github.com/atipicial/atipicial-go/pkg/rpcclient).
...
example). AtipicialGo currently doesn't support this behavior. This affects the
...
AtipicialGo currently uses a different set of error codes in comparison to C# implementation, see 
[proposal](https://github.com/atipicial-project/proposals/pull/156).
AtipicialGo retains certain deprecated error codes, which will be removed once 
all nodes adopt the new error standard.

##### `calculatenetworkfee`

AtipicialGo tries to cover more cases with its calculatenetworkfee implementation,
...
properly) transaction, AtipicialGo also works with deployed contracts that fail at
...
atipicial-project/atipicial#2805 as well.

##### `invokefunction`, `invokescript`

atipicial-go implementation of `invokefunction` does not return `tx`
...
##### `getaep11balances` and `getaep17balances`
atipicial-go implementation of `getaep11balances` and `getaep17balances` does not
perform tracking of AEP-11 and AEP-17 balances for each account as it is done
in the C# node. Instead, a atipicial-go node maintains a list of standard-compliant
contracts, i.e. those contracts that have `AEP-11` or `AEP-17` declared in the
supported standards section of the manifest. Each time balances are queried,
the atipicial-go node asks every AEP-11/AEP-17 contract for the account balance by
...
supported standards but emits compliant AEP-11/AEP-17 `Transfer`
notifications, the token balance won't be shown in the list of balances
returned by the atipicial-go node (unlike the C# node behavior). However, transfer
logs of such tokens are still available via respective `getaepXXtransfers` RPC
calls.
...
other AEP-11/AEP-17 tokens, if transfer's `LastUpdatedBlock` is lower than the
latest state synchronization point P the node working against,
`LastUpdatedBlock` equals P. For AEP-11 NFTs `LastUpdatedBlock` is equal for
all tokens of the same asset.

##### `getversion`

AtipicialGo can return additional fields in the `protocol` object depending on the
...
enabled in the server's protocol configuration. AtipicialGo can return an additional
...
##### `getaep11transfers` and `getaep17transfers`
`transfernotifyindex` is not tracked by AtipicialGo, thus this field is always zero.

##### `traverseiterator` and `terminatesession`

AtipicialGo returns an error when it is unable to find a session or iterator, unlike 
the error-free C# response that provides a default result.

##### `verifyProof`

AtipicialGo can generate an error in response to an invalid proof, unlike
the error-free C# implementation.

##### `getPeers`

AtipicialGo extends the `getpeers` RPC call to return the user agent
...
| Method  | Reason |
| ------- | ------------|
| `canceltransaction` | Doesn't fit atipicial-go wallet model, use CLI to do that (`atipicial-go util canceltx`) |
| `closewallet` | Doesn't fit atipicial-go wallet model |
...
| `getwalletbalance` | See `closewallet` comment, use `getaep17balances` for that |
| `getwalletunclaimedgas` | See `closewallet` comment, use `getunclaimedgas` for that |
| `importprivkey` | Not applicable to atipicial-go, see `closewallet` comment |
| `listaddress` | Not applicable to atipicial-go, see `closewallet` comment |
| `listplugins` | atipicial-go doesn't have any plugins, so it makes no sense |
| `openwallet` | Doesn't fit atipicial-go wallet model |
| `sendfrom` | Not applicable to atipicial-go, see `openwallet` comment |
| `sendmany` | Not applicable to atipicial-go, see `openwallet` comment |
| `sendtoaddress` | Not applicable to atipicial-go, see `claimgas` comment |
...
optional parameter is `verbose` (compatible with the Atipicial RPC protocol), the
...
[atipicialrpc.ErrUnsupportedState](https://github.com/atipicial/atipicial-go/blob/87e4b6beaafa3c180184cbbe88ba143378c5024c/pkg/atipicialrpc/errors.go#L134)
...
#### Limits and paging for getaep11transfers and getaep17transfers

`getaep11transfers` and `getaep17transfers` RPC calls never return more than
...
```json
{ "jsonrpc": "2.0", "id": 5, "method": "getaep17transfers", "params":
...
contract method. Other use-cases are described in [this issue](https://github.com/atipicial-project/atipicial/issues/3386).
...
* [Atipicial JSON-RPC 2.0 docs](https://docs.atipicial.org/docs/en-us/reference/rpc/latest-version/api.html)
