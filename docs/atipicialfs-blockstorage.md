# AtipicialFs block storage

Using AtipicialFs to store chain's blocks and snapshots was proposed in
[#3463](https://github.com/atipicial-project/atipicial/issues/3463). AtipicialGo contains several
extensions utilizing AtipicialFs block storage aimed to improve node synchronization
...
A single AtipicialFs container is used to store blocks and index files. Each container
...
A single AtipicialFs container is used to store contract state objects. Each container
...
### AtipicialFs BlockFetcher

AtipicialFs BlockFetcher service is designed as an alternative to P2P synchronisation
protocol. It allows to download blocks from a trusted container in the AtipicialFs network
and persist them to database using standard verification flow. AtipicialFs BlockFetcher
service primarily used during the node's bootstrap, providing a fast alternative to
P2P blocks synchronisation.

AtipicialFs BlockFetcher service search and fetch blocks directly from AtipicialFs container via
built-in AtipicialFs object search mechanism.
...
2. **Parallel Block Downloading**:
   ...
   node that uses AtipicialFs BlockFetcher. Downloaded blocks are placed to the
...
Once all blocks available in the AtipicialFs container are processed, the service
shuts down automatically.

### AtipicialFs block uploading command
The `util upload-bin` command is designed to fetch blocks from the RPC node and upload 
them to the AtipicialFs container. The batch size should be consistent from run to run, 
otherwise gaps in the uploaded blocks may occur. Below is an example usage of the command:

```shell
./bin/atipicial-go util upload-bin --cid 9iVfUg8aDHKjPC4LhQXEkVUM4HDkR7UCXYLs8NQwYfSG --wallet-config ./wallet-config.yml --block-attribute Block --rpc-endpoint https://rpc.t5.n3.atipicial.com:20331 -fsr st1.t5.fs.atipicial.org:8080 -fsr st2.t5.fs.atipicial.org:8080 -fsr st3.t5.fs.atipicial.org:8080
```

Run `./bin/atipicial-go util upload-bin --help` to see the full list of supported options.

This command works as follows:
1. Fetches the current block height from the RPC node.
2. Searches in batches for the blocks stored in AtipicialFs.
3. In batches fetches missing blocks from the RPC node and uploads them to the AtipicialFs container.
...
### AtipicialFs StateFetcher

AtipicialFs StateFetcher service enables contract state synchronization for non-archival nodes
by fetching contract storage key-value pairs from a trusted AtipicialFs container. 
It serves as an alternative to P2P-based MPT state synchronization ([P2PStateExchngeExtensions](https://github.com/atipicial/atipicial-go/blob/f7080f28d7088517de1d624dfdaf247f914486d2/docs/node-configuration.md?plain=1#L421)).
It allows to download contract state objects from a trusted container in the AtipicialFs 
network and persist them to database. 
AtipicialFs StateFetcher service primarily used during the node's bootstrap, providing a 
fast alternative to P2P synchronization.

#### Operation flow

1. **State Object Search**:
   Searches the AtipicialFs container for objects with the configured `StateAttribute`
...
Once state object fetched from the AtipicialFs container is processed, the service
shuts down automatically.

### AtipicialFs state uploading command
The `util upload-state` command is used to start a node, traverse the MPT over the 
smart contract storage, and upload MPT nodes to a AtipicialFs container at every 
`StateSyncInterval` number of blocks. Below is an example usage of the command:

```shell
./bin/atipicial-go util upload-state --cid 9iVfUg8aDHKjPC4LhQXEkVUM4HDkR7UCXYLs8NQwYfSG --wallet-config ./wallet-config.yml --state-attribute State -m -fsr st1.t5.fs.atipicial.org:8080 -fsr st2.t5.fs.atipicial.org:8080 -fsr st3.t5.fs.atipicial.org:8080
```

Run `./bin/atipicial-go util upload-state --help` to see the full list of supported options.

This command works as follows:
1. Searches for the state objects stored in AtipicialFs to find the latest uploaded object.
...
4. Uploads the MPT nodes to the AtipicialFs container.
