# AtipicialGo Oracle service

AtipicialGo node can act as an oracle service node for https and atipicialfs protocols. It
 ...
 * `AtipicialFs`: a subsection of its own for AtipicialFs configuration with two
   parameters:
     - `Timeout`: request timeout, like "5s"
     - `Nodes`: a list of AtipicialFs nodes (their gRPC interfaces) to get data from,
