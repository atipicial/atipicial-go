...
const atipicialAmount = 99999000
...
func InitSimple(t *testing.T, rootpath string, e *atipicialtest.Executor) {
...
func Init(t *testing.T, rootpath string, e *atipicialtest.Executor) {
	...
	t.Logf("native ATC hash: %v", atipicialHash)
	...
	acc1 := e.Validator.(atipicialtest.MultiSigner).Single(0) // priv1 index->order and order->index conversion
	priv1ScriptHash := acc1.ScriptHash()
	atipicialValidatorInvoker := e.ValidatorInvoker(atipicialHash)
	gasValidatorInvoker := e.ValidatorInvoker(gasHash)
	atipicialPriv0Invoker := e.NewInvoker(atipicialHash, acc0)
	...
	// Block #1: move 1000 GAS and atipicialAmount ATC to priv0.
	txMoveAtipicial := atipicialValidatorInvoker.PrepareInvoke(t, "transfer", e.Validator.ScriptHash(), priv0ScriptHash, atipicialAmount, nil)
	...
	b := e.AddNewBlock(t, txMoveAtipicial, txMoveGas)
	e.CheckHalt(t, txMoveAtipicial.Hash(), stackitem.Make(true))
	...
	jsonTxMoveAtipicial, err := txMoveAtipicial.MarshalJSON()
	require.NoError(t, err)
	t.Logf("txMoveAtipicial hash: %s", txMoveAtipicial.Hash().StringLE())
	t.Logf("txMoveAtipicial JSON: %s", string(jsonTxMoveAtipicial))
	t.Logf("txMoveAtipicial base64: %s", base64.StdEncoding.EncodeToString(txMoveAtipicial.Bytes()))
	...
	// Block #4: transfer 1000 ATC from priv0 to priv1.
	atipicialPriv0Invoker.Invoke(t, true, "transfer", priv0ScriptHash, priv1ScriptHash, 1000, nil)
	...
	// Block #14: register `atipicial.com` via NNS.
	registerTxH := nsPriv0Invoker.Invoke(t, true, "register",
		"atipicial.com", priv0ScriptHash) // block #14
	...
	nsPriv0Invoker.Invoke(t, stackitem.Null{}, "setRecord", "atipicial.com", int64(nns.A), "1.2.3.4") // block #15
	...
	// Block #17: deploy AtipicialFs Object contract (AEP11-Divisible).
	...
	txSendRaw := atipicialPriv0Invoker.PrepareInvoke(t, "transfer", priv0ScriptHash, priv1ScriptHash, int64(fixedn.Fixed8FromInt64(1000)), nil)
	bw.Reset()
...
func newDeployTx(t *testing.T, e *atipicialtest.Executor, sender atipicialtest.Signer, sourcePath, configPath string, deploy bool) (util.Uint256, util.Uint160) {
	c := atipicialtest.CompileFile(t, sender.ScriptHash(), sourcePath, configPath)
	t.Logf("contract (%s): \n\tHash: %s\n\tAVM: %s", sourcePath, c.Hash.StringLE(), base64.StdEncoding.EncodeToString(c.AEF.Script))
	if deploy {
