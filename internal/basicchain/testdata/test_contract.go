...
func OnAEP17Payment(from interop.Hash160, amount int, data any) {
	...
	balance := atipicial.BalanceOf(curr)
	if ledger.CurrentIndex() >= 100 {
		ok := atipicial.Transfer(curr, owner, balance, nil)
		if !ok {
			panic("owner transfer failed")
		}
		ok = atipicial.Transfer(curr, owner, 0, nil)
		if !ok {
			panic("owner transfer failed")
		}
	}
...
// Verify always returns true and is aimed to serve the TestATC_RecursiveGASMint.
func Verify() bool {
	return true
}
...
func Transfer(from, to interop.Hash160, amount int, data any) bool {
	...
	if management.GetContract(to) != nil {
		contract.Call(to, "onAEP17Payment", contract.All, from, amount, data)
	}
	return true
}
