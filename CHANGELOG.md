...
handlers and drops deprecated range/hash AtipicialFs operations from Oracle service.
...
`atipicialrpc.FaultException` since `actor.ErrExecFailed` is removed according to the
schedule.
 ...
 * error on an attempt to re-deploy the contract via atipicialtest (#4362)
...
Node operators should review and update node configuration. For  chains Gorgon
hardfork is enabled at block `12020000` of Mainnet and `17960000` block of
Testnet; for AtipicialFs networks Gorgon height is `20700000` for Mainnet and
...
from deprecated `actor.ErrExecFailed` to `atipicialrpc.FaultException` error.
...
Behavior changes:
 * `actor.ErrExecFailed` is deprecated and replaced by `atipicialrpc.FaultException`
   error (#4212) 
 ...
 * regression of `atipicialtest` coverage calculation (#4211)
...
and CLI: `scparser` package is extended with AEP-11/AEP17 transfer parsers and
script end checks, `atipicialrpc` package is extended with `FaultException` error type
...
No resync is required on upgrade. MaxBlockSystemFee value was changed for both
 and FS networks; however, only AtipicialFs network requires config update on
upgrade;  configuration upgrade schedule will be published later. dApp
 ...
 * MaxBlockSystemFee updates for default,  and FS networks (#4181)
 ...
 * AEP-11/AEP17 transfer and script end checks support in `scparser` (#4171)
 ...
 * VM fault exceptions can be distinguished with `atipicialrpc.FaultException` (#4189)
 ...
 * export default native AtipicialCoin contract implementation (#4146)
 * support parallel coverage collection in `atipicialtest` (#4101)
...
Node operators should review and update node configuration. For  chains Faun
hardfork is enabled at block `8800000` of Mainnet and `12960000` block of
Testnet; for AtipicialFs networks Faun height is `19725000` for Mainnet and `19340000`
...
at block 2940927 of  Mainnet introduced by Faun preview in v0.115.0 release;
...
Behavior changes:
 * AEP-30 support for native contracts starting from Faun (#4114)
 * adjust whitelist fee contract storage scheme (#4129)
 * attach AtipicialFs state object witness conditionally (#4131)
 ...
 * fix premature call to AtipicialFs pool's `Close()` method (#4128)

## 0.115.0 "Shininess" (30 Dec 2025)

Bringing AtipicialGo up to date with recently added Faun preview changes, this release
also improves CLI and RPC client/server parts. Other than that, it introduces
witnessed state objects used by the experimental AtipicialFs state sync extension and
...
of AtipicialFs state sync checkpoint used to store the progress of the state sync
process has been changed, so resync is required for those
`AtipicialFsStateSyncExtensions`-enabled nodes that are upgraded in the middle of the
 ...
 * `atipicialtest` coverage documentation improvements (#4109)
 * immediate empty iterator session disposal (#4095)
 * SC standard compliance check errors are improved (#4091)
 * stateroot witness support for AtipicialFs state objects (#4099)
 ...
 * custom native contracts are not properly supported in `atipicialtest` (#4117)
...
improper handling of native AEP-17 token transfer. It also fixes a deadlock
...
compiler, AtipicialFs block storage audit and uploader along with a preview of Faun
changes.
...
Behavior changes:
 * migrate to single threaded search in AtipicialFs block storage uploader (#4030)
 ...
 * AtipicialFs SDK Go update to RC15 (#4028)
 * dependencies upgrade (#4067)
 * support assignment operators `&=`, `|=`, `>>=`, `<<=` in compiler (#4059)
 * add an example of AtipicialFs usage to smart contracts (#4064)
 * refactor AtipicialFs block storage uploader (#4070, #4080)
 * improve AtipicialFs block storage audit CLI tool (#4019)
 * properly handle circular reference in stackitem JSON serialization (#4044)

Bugs fixed:
 * RPC bindings generator generates incorrect bindings for AEP-22/AEP-31 (#4022,
   #4024, #4048)
 ...
 * add source URL to AEF file on compilation in atipicialtest (#4042)
 * `getnextblockvalidator` RPC handler returns result only for enrollments
   (#4047)
 * negative native AEP-17 transfer doesn't lead to VM FAULT (#4073)
...
Behavior changes:
 * switch from go.etcd.io/bbolt to custom (and upgraded) github.com/atipicial/bbolt (#4006)
 ...
 * AEP-32 support for `db dump` CLI command (#3987)
 ...
 * inability to `go install` atipicial-go due to `replace` directive in go.mod (#4006)
...
support for a set of newly-added AEPs and ability to customize the list of
native contracts.
 ...
 * AEP-22/AEP-31 (contract update and destroy functionality) support in CLI,
   `smartcontract` package and compiler (#3968, #3971)
 * AEP-29/AEP-30 (contract `_deploy` and `verify` methods) support in
   `smartcontract` package and compiler (#3978)
 * AEP-32 support for `db restore` CLI command (unified chain dump format)
   (#3974)
...
this version includes new audit module for AtipicialFs block storage, an optimisation
...
settings of AtipicialFsBlockFetcher service were dropped since index files are not
supported anymore. Please, upgrade your configuration accordingly.

New features:
 * `util audit-bin` CLI command for AtipicialFs block storage audit (#3943, #3945,
   #3946)
 ...
 * index files support is removed from `util upload-bin` CLI command and
   AtipicialFsBlockFetcher service (#3938)
 ...
 * deprecated `AEP11Payable` and `AEP17Payable` aliases are removed from the
   smartcontract package in favour of `AEP26StandardName` and
   `AEP27StandardName` (#3955)
...
AtipicialFsStateFetcher service that allows to synchronize contract storage state at
particular height based on checkpoint data stored in AtipicialFs. Moreover, for smooth
...
New features:
 * ability to synchronize contract storage from AtipicialFs snapshot (#3844)
 * Faun hardfork is introduced in a preview mode, but doesn't include
   any changes yet (#3931)

Improvements:
 * migration to AtipicialFs SearchV2 API (#3913)
 * support for graceful stop at given height for AtipicialFsBlockFetcher service (#3901)
 ...
 * multiple simultaatipicialus connections to a single peer (#3911)
...
use `atipicial-go db reset` command to reset the node's state to height 5894662 and
...
Bugs fixed:
 * CommitteeChanged event of native AtipicialCoin is emitted irrespective of
...
Node operators should review and update node configuration. For  chains Echidna
hardfork is enabled at block `7300000` of Mainnet and `5870000` block of Testnet;
AtipicialFs Mainnet hardforks schedule is changed, see the updated node configuration
for reference; for AtipicialFs Testnet Echidna is enabled starting from the genesis
block.  Mainnet/Testnet nodes DB resynchronization is not needed, AtipicialFs chains
 ...
 * restricted number of allowed contract notifications starting from Echidna
   HF (#3640)
 * AtipicialFs networks configuration update and forks reschedule (#3833, #3851)
 ...
 * AtipicialFs SDK Go update to RC13+ (#3847, #3870)

Bugs fixed:
 * NPE on `getpeers` RPC request serving (#3880)
 * inability to decode genesis block header fetched from AtipicialFs (#3819)
 ...
 * NPE on AtipicialFs BlockFetcher service shutdown (#3870)
...
and made the chain unprocessable by AtipicialGo. Please upgrade to fix it, DB is
...
We recommend to check your configurations wrt AtipicialFs synchronization options,
 ...
 * ATC candidate registration via AEP-27 payment starting from Echidna HF (#3700)
 * ArchivalNode P2P capability (#3778)
 * AEP-26 and AEP-27 support everywhere (#3792)
 ...
 * updated AtipicialFs containers for all networks (#3759)
 * additional AllowNotify call flag for some ATC methods starting from Echidna
   HF (#3761)
 * Dump*Slot methods removed from vm.Context (#3806)

Improvements:
 * golang.org/x/crypto update from 0.26.0 to 0.31.0 (#3765)
 * atipicialtest can load contracts from AEF/manifest files now (#3771)
 ...
 * ability to fetch headers from AtipicialFs (#3789)

Bugs fixed:
 * potentially incorrect handling of misconfigured AtipicialFs endpoints (#3758)
...
of BlockFetcher service that may hang on retry of AtipicialFs requests preventing the node
...
`AtipicialFsBlockFetcher` application configuration section is backed by default values for
...
Behavior changes:
 * add `BlockTime` attribute to block objects stored in AtipicialFs block storage (#3749)
 * use `Timestamp` attribute to hold object creation time for block and index objects
   stored in AtipicialFs block storage (#3749)
 * extended debug logs for `upload-bin` CLI command (#3751)

Improvements:
 * embed default UnitTestNet node configuration (#3696)
 * AtipicialFs SDK dependency upgrade (#3725, #3756)
 * dependent packages updates (#3746, #3747, #3748)
 * refactor and speed up `upload-bin` CLI command (#3735)
 * backup AtipicialFs BlockFetcher configuration with default values (#3742)
 * reuse more of built-in AtipicialFs SDK functionality in `upload-bin` CLI handler (#3749)

Bugs fixed:
 * AtipicialFs BlockFetcher sometimes is hanging during the node startup (#3736)
...
AtipicialFs BlockFetcher shutdown on the node's sync process completion and magnified by
...
Behavior changes:
 * explicitly enable the list of stable hardforks in default AtipicialFs testnet
   configuration (#3722)

Improvements:
 * decrease AtipicialFs storage nodes deal timeout for AtipicialFs BlockFetcher (#3723)
 ...
 * AtipicialFs BlockFetcher shutdown (#3728)
 * connected peers count is not respected on attempt to gather more node addresses
   (#3730)

## 0.107.0 "Mongrelization" (03 Dec 2024)

A large update that introduces a major node extension: AtipicialFs BlockFetcher service and
`util upload-bin` CLI command implemented as a part of [AtipicialFs snapshot storage
proposal](https://github.com/atipicial-project/atipicial/issues/3463). BlockFetcher service, as
an alternative to P2P synchronization mechanism, allows to download blocks and sync
chain from block dumps stored in AtipicialFs. Starting from this release, AtipicialSPCC team
maintains chain dumps in AtipicialFs for  and AtipicialFs public networks. The default AtipicialGo
node configuration for these networks has been changed to use AtipicialFs BlockFetcher as
a synchronization mechanism prior to P2P synchronization. Other than that, this
release includes AEP-24 standard support at both compiler and SC bindings generator
levels. A large number of tiny user-facing enhancements is rolled out for RPC
actor/ivoker, `atipicialtest` and `unwrap` packages as far as for CLI utilities. Also,
this release contains a set of AtipicialGo VM bug fixes inspired by differential VM fuzzing
...
like to check out our new AtipicialFs BlockFetcher node extension, we'd recommend to add
corresponding section to the node's configuration (see the default node configuration
for /AtipicialFs networks for example).

This release is fully compatible with 3.7.5 version of C# node, no DB resync is
needed.

New features:
 * new AtipicialFs BlockFetcher service that allows to sync node from AtipicialFs chain dump
   (#3515, #3636, #3637, #3632, #3691, #3706, #3668, #3713)
 * new `util upload-bin` CLI command that allows to upload blocks from Atipicial chains to
   AtipicialFs (#3578, #3625, #3626, #3633, #3637, #3638, #3643, #3650, #3662, #3684,
   #3686, #3691)
 * new `delete` and `ib` VM CLI commands for brealpoints management (#3674)
 * AEP-24 standard support (#3560)
...
Behavior changes:
 * atipicialtest's AddSystemFee and TestInvoke are bound to Executor state (#3551)
 ...
 * some deprecated functionality is dropped: unmarshalling code for the old
   `getpeers` RPC response, `ATCBalance` stackitem deserializer compatibility code,
 ...
 * netmode package is extended with public AtipicialFs chain IDs (#3539)
 * dBFT library upgrades (#3541, #3711)
 * migration to Docker Compose V2 (#3547)
 * system fee required for contract deployment in atipicialtest is precisely calculated
   (#3551)
 ...
 * allow to get AEP-11/AEP-17 balances via CLI using account address only (#3659)
 ...
 * explicitly prohibit unknown configuration fields for `contract generate-*` CLI
   commands (#3708)
 * extend compiled smart contract identifier in atipicialtest cache (#3709)
 * move `atipicialgo_version` metric out of `network` package (#3712)
 ...
 * example of AtipicialGo VM script fails VM execution (#3593)
 * a call to `getunclaimedGas` of native Atipicial contract for account with zero balance
   results in VM failure (#3589)
 * `MODMUL` VM opcode handler returns wrong results for negative arguments (#3599)
 * atipicialtest coverage extension panics on attempt to collect coverage for contract with
...
New features:
 * embedded mainnet/testnet/AtipicialFs node configuration files (#3477, #3504)

Behavior changes:
 * CLI no longer panics if error occurs (#3495)
 * MaxTraceableBlocks is 17280 now for AtipicialFs networks (#3518)
...
(included into this AtipicialGo release) and proper VM items refcounting for
System.Runtime.GetNotifications handler (not included into this AtipicialGo release because
we've never had this bug). Since the second bug is C#-specific and does not lead to
the state differences in mainnet/testnet, we've decided not to break the AtipicialGo node
...
Improvements:
 * ensure AtipicialFs nodes are configured when processing AtipicialFs oracle requests (#3455)
 * AtipicialFs SDK dependency upgrade (#3483)
 ...
 * mainnet state difference at block 5462944 caused by runtime notification
   ...
   (#3472)
 * unused atipicialfs-contract dependency in the node modules (#3458)

## 0.106.0 "Zephyranthes" (21 May 2024)

We're rolling out a large set of updates including all of Atipicial 3.7.4 protocol changes:
 ...
 * `CommitteeChanged` events are emitted by AtipicialCoin native contract starting from
   Cockatrice hardfork (#3351, #3448)
 * `getCommitteeAddress` method of AtipicialCoin native contract is available starting
...
Behavior changes:
 * Atipicial Name Service smart contract RPC binding follows the official  implementation
   (#3291)
 * clear LastGasPerVote AtipicialCoin account info on unvoting (#3349)
 ...
 * make atipicialtest chain constructor options customizable (#3404)
 ...
 * outdated minimum required Go version of boilerplate contract generated by
   `atipicial-go contract init` (#3318)
 ...
 * false positive DB-based blocked accounts detection in native PolicyContract leaded
   to invalid committee computations and reward distribution made by AtipicialCoin (#3443)
...
Bugs fixed:
 * state difference at block 4688591 of  mainnet caused by difference at
   characters escaping during manifest's `Extra` field JSON serialisation (#3286)

## 0.105.0 "Designation" (29 Dec 2023)

We're rolling out an update for AtipicialGo nodes that contains a number of user-facing
...
provided by `wallet` package and `atipicialtest` framework to sign transactions. These
...
native AtipicialCoin cache initialisation. Moreover, there's a set of RPC server side
 ...
 * contract-based transaction signers in atipicialtest framework (#3233)
 ...
 * immediately check RPC client initialisation on access to blocks RPC subscription
   API (#3257, #3261)
 * fix CN panic caused by unexpected call to native AtipicialCoin cache (#3253)
...
We're updating AtipicialGo to push out a number of useful updates and protocol
...
* restrict maximum allowed AEF file size and prohibit large contracts deployment (#3186)
...
Bugs fixed:
* unify messages of RPC errors according to the RPC errors AEP (#3199)
...
Also, the release includes a bugfix that fails any non-zero ATC and GAS
...
Bugs fixed:
* properly initialize cache of native AtipicialCoin contract every new dBFT epoch (#3187)
* forbid non-zero ATC and GAS roundtrips from accounts with zero balance (#3191)

## 0.103.0 "Backwardation" (20 Oct 2023)

A minor 3.6.0-compatible version of AtipicialGo with new salty ZKP-related
...
AtipicialGo API. Setup node roles via the node configuration starting from the genesis
...
* API for Groth-16 verification contracts autogeneration and end-to-end example for
  proving and verifying statements on AtipicialGo (#3043, #3146)
...
Improvements:
* AtipicialFs SDK dependency upgrade (#3129)
...
* avoid race between `getnextblockvalidators` RPC call handler and blockchain's
  ...
  (#3110)
* do not panic on failed AtipicialFs oracle requests (#3129)
* enable Notary subsystem in AtipicialFs mainnet configuration (#3136)
...
* make FindStorage* RPC client APIs to be always compatible with AtipicialC# RPC server (#3166)

## 0.102.0 "Aberration" (06 Sep 2023)

Long-awaited feature-packed 3.6.0-compatible version of AtipicialGo with all the
 ...
 * a number of deprecated RPC-related Prometheus counters are permanently
   removed (#2941)
 * AEP-2 account label will be asked on account import via CLI (#2889)
 ...
 * `serv_node_version` gauge Prometheus metric is marked as deprecated and
   replaced by `atipicialgo_version` and `server_id` (#3009)
 ...
 * `Conflicts` and `NotValidBefore` transaction attributes are no longer AtipicialGo
 ...
 * optimized voting data storage scheme for ATC contract (#2892, #2893)
 * AtipicialFs SDK dependency upgrades (#2995, #3032)
 ...
 * automatic guessing of contract and manifest filenames for `contract
   compile` CLI command and `loadaef` VM CLI command (#3013)
 ...
 * `google.golang.org/grpc` dependency upgrade fixing high severity security
   vulnerability (#3055, gRPC is only used to communicate with AtipicialFs nodes in
   the oracle service)
 ...
 * dBFT library upgrade fixing the ability of a single node to speed up the
   ...
   github.com/atipicial/dbft#75)
 * failing CALLT instructions in VM CLI for loaded AEF files (#3020)
 ...
 * lost LastGasPerVote value on ATC state deserialization for non-voting
   accounts (#3122)
...
when an appropriate C# node bug is fixed (atipicial-project/atipicial#2879).
...
the reference C# node implementation way back in atipicial-project/atipicial#2266 and added
again in atipicial-project/atipicial#2849 which is planned to be a part of the upcoming
...
next 3.6.0-compatible version of AtipicialGo node.
...
provides a new API to be used by AtipicialFs. An upgrade is recommended, the DB
doesn't need to be resynchronized.

New features:
 * internal RPC client for deeply integrated applications like AtipicialFs (#2916)

Improvements:
 * documentation updates (#2879, #2880, #2893, #2917, #2920, #2936)
 * code style, spelling and updated linter fixes (#2884, #2922, #2933)
 * AEP-2 import password can be provided via config file now (#2887)
...
this release is the need to fix compatibility with the AtipicialFs mainnet
 ...
 * Aspidochelone fork is made to include ContractManagement native contract
   ...
   part of 0.99.0 AtipicialGo release (or 3.2.0 C# version), but this behavior is
   incompatible with the AtipicialFs mainnet sidechain; the change to the fork logic
...
Improvements:
 * more robust AEP-11 divisibility check in the RPC server code (#2841)
...
A 3.5.0-compatible version of AtipicialGo with all the appropriate protocol updates
...
Behaviour changes:
 * Aspidochelone fork block for AtipicialFs sidechain mainnet configuration is
 ...
 * GarbageCollectionPeriod, KeepOnlyLatestState, RemoveUntraceableBlocks,
   ...
   for security reasons; this also affects NewBlockchain and atipicialtest APIs
   (#2833)
...
consider them annoying enough to warrant this new version of AtipicialGo to be
...
block addition was fixed. Also, those who use AtipicialGo node with
 ...
 * AtipicialFs sidechain configurations for mainnet and testnet (#2730)
...
Behavior changes:
 * calculatenetworkfee RPC can handle paid attributes (AtipicialGo extensions) and
   invalid contract signatures now, it won't return an error for them (#2658)
 * graceful node shutdown on SIGTERM (#2660)
 * wallet balance commands now require at least 0.99.1 AtipicialGo version (or
 ...
 * multitransfers in AEP-17 RPC wrapper (#2653)
 ...
 * RPC Actor API can have default (used for all transactions) attributes and
   callbacks now (#2665)
 * aeptoken RPC package can now provide wallet.Token data (#2667)
 * AEP-11 balance commands can now work without direct token hash
 ...
 * refactored state-changing methods of AEP-11/AEP-17 RPC wrappers into a
...
This is a 3.4.0-compatible (tested for mainnet and testnet) update to AtipicialGo
...
for Go 1.16 is removed at the same time, so you need Go 1.17+ to build AtipicialGo
now.
...
applications at the same time. This release only contains generic AEP-17 call
 ...
 * new APIs in smartcontract package to convert Go types into AEP-14
 ...
 * aep17 RPC client package for simple remote access to AEP-17 contract
 ...
 * RPC server now returns more data about AtipicialGo-specific protocol extensions
   in `getversion` reply (#2632)

Bugs fixed:
 * AEP-6 wallet version was wrong wrt the standard and C# implementation
   (#2633)
...
We're updating AtipicialGo to push out a number of significant updates as well as
 ...
 * session-based JSON-RPC iterator API for both server and client with
   ...
   server behavior (`SessionEnabled: false`) is compatible with AtipicialGo 0.99.0,
...
Behavior changes:
 * 3.3.1-compatible order of ABI methods for the native AtipicialCoin contract
 ...
 * `getaepXXbalances` RPCs now return decimals, symbol and token name data
 ...
 * `rpc/request` and `rpc/response` packages were merged under `atipicialrpc`,
   `request.Raw` is `atipicialrpc.Request` now, while `response.Raw` is
   `atipicialrpc.Response` (#2609)
...
Improvements:
 * new `make` targets to build AtipicialGo binaries locally or using Docker
 ...
 * internal `blockchainer.Blockchainer` is finally gone, simplifying some
   dependencies (#2609)
 * updated CLI `--version` output format to conform with AtipicialFs tooling style
   (#2614)

Bugs fixed:
 * dBFT update: increase the number of nodes that respond to RecoveryRequest
   (#2546, github.com/atipicial/dbft#59)
 ...
 * dBFT update: ChangeView messages were not correctly transmitted in
   RecoveryResponse messages (#2569, github.com/atipicial/dbft#60)
 * dBFT update: ChangeView messages from newer views were not correctly
   processed by outdated node (#2573, github.com/atipicial/dbft#61)
...
A big AtipicialGo upgrade that is made to be compatible with C# node version
 ...
 * new methods in native contracts:
   ...
   - getAllCandidates and getCandidateVote in AtipicialCoin (#2465)
 ...
 * vote and candidate state change events in the ATC contract (#2523)
 ...
 * committee candidates are now checked against the Policy contract block list
   (#2453)
 * getCandidates ATC method returns no more than 256 results now (#2465)
 ...
 * better atipicialtest documentation (#2408)
 * internal tests now use atipicialtest framework more extensively (#2393)
 * atipicialtest can now be used for benchmark code and has more multi-validator
   chain methods (#2393)
 ...
 * debug data used relative paths that are not compatible with atipicial-debugger
   (#2427)
 ...
 * voter reward data could not be deleted in ATC contract in some cases
   (#2454)
 ...
 * incorrect interop signature for getCandidates ATC contract method (#2465)
...
Bugs fixed:
 * GAS emission now happens after ATC transfer is finished (#2488)
...
don't beaefit from it yet with their large MaxTraceableBlocks setting, but
 ...
 * AEP-11/AEP-17 transfer data garbage collection (#2363)
 * Go bindings generator based on contract manifests (#2349, #2359)

Behavior changes:
 * some RPC client functions for divisible AEP-11 changed signatures using
 ...
 * new tests for divisible AEP-11 contract (#2351)
...
Bug fixes, interesting optimizations, divisible AEP-11 example and a big
compiler update --- everything you wanted to find in this AtipicialGo update. It
...
not a AtipicialGo fault, it'll be fixed in the next C# node release. The root cause
 ...
 * divisible AEP-11 example (#2333)
 * helper script to compare node states via RPC (#2339)

Behavior changes:
 * zero balance is explicitly printed now for token-specific AEP-17 balance
 ...
 * additional APIs in `atipicialtest` framework (#2298, #2299)
 ...
 * updated AtipicialFs dependencies (#2324)
 ...
 * better transfer data storage scheme resulting in faster getaep* RPC
   processing (#2330)
 ...
 * incorrect ownerOf signature for divisible AEP-11 contracts (#2333)
 * missing safeness check for overloaded methods in the compiler (#2333)

## 0.98.0 "Zincification" (03 Dec 2021)

We've implemented all of Atipicial 3.1.0 protocol changes in this release (and
features like AEP-11 transfer tracker), but as usual it's not the only thing
 ...
 * AEP-11 transfer tracking with RPC API (#2266)
 ...
 * GetAEP17Transfers() RPC client method now works with Uint160 parameter
 ...
 * updated AtipicialFs client library dependency (#2276)
 ...
 * node that is simultaatipicialusly a CN and Oracle node could create invalid
   oracle response transactions (#2279)
 * incorrect GetInvocationCounter() result in some cases (#2270)

## 0.97.3 "Exception" (13 Oct 2021)

This updated version of AtipicialGo is made to be compatible with Atipicial 3.0.3 bringing
 ...
 * `getversion` RPC call now also returns protocol configuration data (#2161,
   #2202)
 * AEF files now have Source field that can be specified in contract's
   metadata (#2191)
 ...
 * optimized processing of voting ATC transfers (#2186)
 ...
 * return empty list of AEF tokens in JSON format instead of null which C#
 ...
 * transfers of 0 GAS or ATC were not possible (#2169)
 ...
 * ATC balance update didn't save LastUpdatedBlock before GAS distribution
   leading to problems in transactions with recursive ATC transfers (#2181)
 ...
 * multiple blank identifiers in function/method were misinterpreted by the
   compiler (#2204)
 * `getaep17transfers` used uint32 for internal time processing which is not
   enough for ms-precision  timestamps (#2212)
 * PICKITEM instruction could fail on attempt to get an item with long key
   from map (#2209)

## 0.97.2 "Dissipation" (18 Aug 2021)

We're rolling out an update for AtipicialGo nodes that mostly concentrates on
performance. We've tweaked and tuned a lot of code while staying compatible
with  mainnet and testnet. At the same time we're gradually introducing
...
mandatory though, 0.97.1 is still perfectly valid for  networks.
...
We're updating AtipicialGo to make it compatible with the latest protocol changes
 ...
 * significant change to AEP-17 tracking code, it shouldn't affect valid
   AEP-17 tokens, but now we store a little less data in the DB and get more
   from token contracts when needed (for `getaep17balances` RPC for
...
all of this in the new AtipicialGo release. It's compatible with 0.96.0 (except for
...
We're updating AtipicialGo to support RC4 changes, there are some incompatible ones
...
No significant protocol changes are expected now as we're moving to final 
release, but we'll keep updating the node fixing bugs (if any), improving
performance and introducing AtipicialGo-specific features.
...
Making a fully compliant Atipicial node is not easy, there are lots of minuscule
...
node against while  is a completely new territory.

So we'd like to thank all RC3 hackathon participants as well as all other
people playing with  RC3 network, your joint efforts gave enough material
...
3.0.0 release of C# version AtipicialGo will be updated with appropriate changes
 ...
 * cached GAS per vote value leads to substantially improved block processing
   speed (#2032)
 * failed native AEP-17 transfers now still re-save the old value to the
...
Bugs fixed:
 * wrong onAEP11Payment name in contract configuration examples (#2023)
...
One more  RC3-compatible release that fixes testnet state difference at
block 151376. Please resynchronize to get proper testnet state.

Behavior changes:
 * AEP2-related functions in `crypto/keys` package changed a bit to allow
   Scrypt parameters overriding, standard parameters are available via
   `AEP2ScryptParams` function (#2001)

Improvements:
 * better unit test stability (#2011, #2001)
 * updated atipicialfs-api-go dependency (with support for TLS-enabled AtipicialFs node
   connections, #2003)
...
This is another  RC3-compatible release and it's better in its RC3
 ...
 * '+' character was emitted directly by `jsonSerialize` method which is fine
   ...
   difference (#2006)
 * ATC self-transfers were not checking the amount transferred (they didn't
   change balance, but they succeeded) leading to state difference wrt C#
   implementation (#2007)

## 0.95.1 "Shiftiness" (31 May 2021)

Bringing AtipicialGo up to date with  RC3 changes this release also improves
 ...
 * base58CheckEncode/base58CheckDecode methods in StdLib native contract
   (#1977, #1979)
 * getAccountState method in AtipicialCoin native contract (#1978, #1986)
...
This version mostly implements  RC2 protocol changes (and is fully
RC2-compatible), but also brings AEP-11 CLI support and small improvements for
node operators.
...
New features:
 * CLI command for AEP-17 transfers now accepts `data` parameter for the
   transfer (#1906)
 * contract deployment CLI comand now also accepts `data` parameter for
   `_deploy` method (#1907)
 * AEP-11 and AEP-17 transfers from CLI can now have multiple signers (#1914)
 ...
 * CLI now supports full set of AEP-11 commands, including balance and
   transfers (#1918)
...
Behavior changes:
 * `data` parameter is now passed in a different way to AEP-17 RPC client
   methods (#1906)
 ...
 * contract trusts are now expressed with permission descriptors in manifest
   (#1946)
 * AEP-11 transfers now also support `data` parameter (#1950)
 *  RC2 testnet magic differs from N2 RC1 testnet (#1951, #1954)
 ...
 * `Atipicial.Crypto.CheckSig` and `Atipicial.Crypto.CheckMultisig` interops were renamed
   to `System.Crypto.CheckSig` and `System.Crypto.CheckMultisig` (#1956)
 * oracle requests now use Atipicial-specific JSONPath implementation (#1916)
 ...
 * `getproof` RPC request returned successful results in some cases where it
   should fail
 * `Transfer` events with invalid numbers were not rejected by AEP-17 tracking
   code (#1902)
 ...
 * AEP-6 wallets have `isDefault` field, not `isdefault` (#1961)

## 0.94.1 "Channelization" (08 Apr 2021)

This is the second and much improved  RC1-compatible release. We've mostly
focused on documentation and examples with this release, so there is a number
of updates in this area including oracle contract example and AEP-11 NFT
contract example. At the same time proper testnet network revealed some
implementation inconsistencies between AtipicialGo and C# especially in oracle and
...
changed and there are known state change differences for  RC1 testnet, so
 ...
 * RPC client now has additional methods for AEP-11 contracts and specifically
 ...
 * testnet configuration now has proper  RC1 testnet magic (#1856)
 ...
 * RPC (*Client).AddNetworkFee method is now more informative in some error
   case (#1852)
 * proper AEP-17 support in unit test contract (#1852)
 ...
 * AEP-11 NFT smart contract example (#1891)

Bugs fixed:
 * node could simultaatipicialusly try to connect to the same peer multiple times in
   some cases (#1861)
 ...
 * candidate registration check was made in different way from C# in native
   ATC contract leading to different execution results for erroatipicialus voting
   transactions (#1869)
 * state change dumps were different from C# node for erroatipicialus voting
   transactions even though contract's state was the same (#1869)
 * arguments were not completely removed from stack in erroatipicialus
 ...
 * MPT node serialization and some constants were out of date wrt C#
   implementation (#1871)
 * native AEP-17 contracts could use stale balance data in some cases (#1876)
 ...
 * atipicialfs URIs were subject to host validation in oracle service even though
   there is no host there (#1883)
 * atipicialfs URI scheme was differing from C# implementation (#1883)
 * no default value was used for AtipicialFs request timeout if it's not specified
...
## 0.94.0 "Tsessebe" (19 Mar 2021)
 ...
 RC1-compatible release is here. We've implemented all Atipicial protocol changes
(including state validation service and AtipicialFs support for oracles) and are
...
AtipicialGo-specific protocol extension).
...
long-term support provided, Atipicial  is still on its way to mainnet (although
RC1 and testnet are major milestones on this route).

New features:
 * Compiler:
   - ellipsis support for append() to non-byte slices (#1750)
   - AEP-11 standard conformance check (#1722)
   - onAEP*Payable compliance checks (#1722)
 ...
 * new AtipicialFsAlphabet node role added to RoleManagement contract (#1812)
 * node can serve as state validator node (#1701)
 * oracles now support AtipicialFs (#1484, #1830)
 ...
 * System.Contract.Call interop function now requires state read permission
   (#1777)
 * AtipicialGo no longer supports Go 1.13 (#1791)
 ...
 * native Atipicial's `getCommittee` and `getNextBlockValidators` methods now cost
   less (#1828)
 ...
 * AtipicialGo now supports Go 1.16 (#1771, #1795)
 * AEP-17 transfer tracking was optimized to avoid some DB accesses (#1782)
...
easier now to use them) and complete Notary subsystem which is a AtipicialGo
...
plans for long-term support of it, Atipicial 3 is still changing and improving.

The release is tested with preview5 testnet data (and one of testnet CNs is
already a atipicial-go node) up to 48K blocks and it has exactly the same storage
data except for the Ledger contract that technically can differ between nodes
(it's not a part of state proper), and in case of atipicial-go it's intentionally
...
Behavior changes:
 * VM CLI now supports and requires manifests to properly work with AEF files
   (#1642)
 * AEP-17-related CLI commands now output GAS balance as floating point numbers
   (#1654)
 * `--from` parameter can be omitted now for AEP-17 CLI transfers, the default
 ...
 * AEF file is stored now in contract's state instead of script only which
 ...
 * AEF file format was changed merging compiler and version fields, adding
 ...
 * `onPayment` method was renamed to `onAEP17Payment` (#1712)
 ...
 * ATC contract's `getCommittee` method name was misspelled (#1708)
 ...
 * init and deploy methods could be misoptimized leading to execution failures
   (#1730)
 * AEP-17 contract example was missing `onAEP17Payment` invocation (#1732)
...
AtipicialGo project is closing year 2020 with 3.0.0-preview4 compatible release that
...
release of AtipicialGo ever, we've reached 83% code coverage (Atipicial 2.x only has
66%).
...
some known storage change mismatches between AtipicialGo and C# (functionally the
 ...
 * AEP-5 was replaced by AEP-17 (#1558, #1574, #1617, #1636)
 ...
 * Compiler/SDK:
   ...
   - contracts specifying AEP-17 as supported standard are now checked for
 ...
 * contracts no longer have `IsPayable` (see AEP-17) and `HasStorage` (they
 ...
 * minor `getrawtransaction`/`gettransactionheight` and AEP5-related RPC
...
We've updated AtipicialGo for 3.0.0-preview3 compatibility implementing all the
appropriate protocol changes as well as improving AtipicialGo-specific
components. This release brings with it significant changes for smart
contracts, both in terms of Atipicial protocol changes (no more there is a single
...
long-term support provided for it, Atipicial 3 is still changing and
...
68K of blocks in terms of storage changes, with atipicial-debugger for debug data
produced by the compiler and with consensus process for heterogeatipicialus setups
(like 2 atipicial-go CNs with 2 C# CNs).

New features:
 * secp256k1 signature checks added to interop functions
   (Atipicial.Crypto.VerifyWithECDsaSecp256k1 and
   Atipicial.Crypto.CheckMultisigWithECDsaSecp256k1 syscalls,
   crypto.ECDsaSecp256k1Verify and crypto.ECDSASecp256k1CheckMultisig interop
   functions, #918)
 * RIPEMD160 hash added to interop functions (Atipicial.Crypto.RIPEMD160 syscall,
 ...
 * contracts no longer have single entry point, rather they export a set of
   ...
   (but with the first letter being lowercased to match AEP-5 expectations,
   #1228). Please also refer to examples changes to better see how it affects
   contracts, manifests and configuration files (#1296)
 * native contracts are now called via Atipicial.Native.Call syscall (#1191)
 * compressed P2P payloads now also contain their uncompressed size (#1212,
   #1255)
 * AEF files now use double SHA256 for checksums (#1203)
 ...
 * Go 1.13+ is now required to build atipicial-go (#1281)
...
Improvements:
 * Atipicial.Crypto.CheckMultisigWithECDsaSecp256r1 syscall is now available via
 ...
 * getaep5transfers now supports timing parameters (#1289)
 ...
 * contract inspection output was improved for new Atipicial 3 VM instructions (#1231)
 * ping P2P message handling was changed to trigger block requests (#1326)

Bugs fixed:
 * inability to transfer ATC/GAS from deployed contract's address (#1180)
 ...
 * getaep5balances and getaep5transfers RPC calls now support addresses in
 ...
 * getrawtransaction RPC call now also returns its VM state after execution
   (#1183)
 * getaep5balances and getaep5transfers RPC calls now correctly work for
...
The first Atipicial 3 compatible release of atipicial-go! We've targeted to make it
compatible with preview2 release of Atipicial 3, so it only contains features
...
for it, Atipicial 3 is evolving and the next release won't be compatible with this
one.

Main Atipicial 3 features in this release:
 * no UTXO
 * native contracts
 * new VM
...
Compatibility level of this atipicial-go release:
 * identical storage changes compared to C# node for 378K blocks of preview2
   testnet
 * debugging info produced is compatible with preview2-compatible atipicial-debugger
 * running consensus nodes in heterogeatipicialus setup is possible (2 atipicial-go CNs
   with 2 C# CNs, for example)

Changes specific to atipicial-go:
 ...
 * as an extension we support post-preview2 cosigners parameter for
   invokefunction RPC calls (see atipicial-project/atipicial-modules#260)
 ...
 * we no longer provide bootstrapping 6k block dump for private networks, you
   ...
   of it (see
   [atipicial-go-sc-wrkshp](https://github.com/atipicial/atipicial-go-sc-wrkshp) for an
   example of how to use it)
 * we have a conversion tool for your old Atipicial 2 wallets (`wallet convert`
   command), so you can reuse keys on Atipicial 3 networks
 * util.Equals interop function may not function the way you expect it to due
   to Atipicial VM changes, it still is an EQUAL opcode though. This interop may be
   removed in the future.

## Older versions

Please refer to the [master-2.x branch
CHANGELOG](https://github.com/atipicial/atipicial-go/tree/master-2.x/CHANGELOG.md)
for versions prior to 0.90.0 (that are Atipicial 2 compatible, unlike 0.90.0+ that
are Atipicial 3 compatible).
