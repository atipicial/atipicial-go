...
Oracles allow you to get external (non-blockchain) data using HTTPS or AtipicialFs
protocols.
*/
package oracle
...
//
//   - url
//     URL to fetch, only https and atipicialfs URLs are supported like
//     https://example.com/some.json or
//     atipicialfs:6pJtLUnGqDxE2EitZYLsDzsfTDVegD6BrRUn8QAFZWyt/5Cyxb3wrHDw5pqY63hb5otCSsJ24ZfYmsA8NAjtho2gr
//
//   - filter
//     JSONPath filter to process the result; if specified, it will be
//     applied to the data returned from HTTP/AtipicialFs and you'll only get
...
func Request(url string, filter []byte, cb string, userData any, gasForResponse int) {
	atipicialgointernal.CallWithTokenNoRet(Hash, "request",
...
func GetPrice() int {
	return atipicialgointernal.CallWithToken(Hash, "getPrice", int(contract.ReadStates)).(int)
}
...
func SetPrice(amount int) {
	atipicialgointernal.CallWithTokenNoRet(Hash, "setPrice", int(contract.States), amount)
}
