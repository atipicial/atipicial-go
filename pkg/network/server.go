...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/network/bqueue"
	"github.com/atipicial/atipicial-go/pkg/network/capability"
	"github.com/atipicial/atipicial-go/pkg/network/extpool"
	"github.com/atipicial/atipicial-go/pkg/network/payload"
	"github.com/atipicial/atipicial-go/pkg/services/blockfetcher"
	"github.com/atipicial/atipicial-go/pkg/services/statefetcher"
...
func newServerFromConstructors(config ServerConfig, chain Ledger, stSync StateSync, log *zap.Logger,
	...
	if s.AtipicialFsBlockFetcherCfg.BQueueSize <= 0 {
		s.AtipicialFsBlockFetcherCfg.BQueueSize = blockfetcher.DefaultQueueCacheSize
	}
	s.bFetcherQueue = bqueue.New[*block.Block](chainBlockQueueAdapter{chain}, log, nil, s.AtipicialFsBlockFetcherCfg.BQueueSize, updateBlockQueueLenMetric, bqueue.Blocking)
	var err error
	s.blockFetcher, err = blockfetcher.New(chain, s.AtipicialFsBlockFetcherCfg, log, func(item bqueue.Indexable) error { return s.bFetcherQueue.Put(item.(*block.Block)) }, sync.OnceFunc(func() { close(s.blockFetcherFin) }), blockfetcher.Blocks)
	if err != nil {
		return nil, fmt.Errorf("failed to create AtipicialFs BlockFetcher: %w", err)
	}
	s.syncHFetcherQueue = bqueue.New[*block.Header](stateSyncHeaderQueueAdapter{s.stateSync}, log, nil, s.AtipicialFsBlockFetcherCfg.BQueueSize, updateBlockQueueLenMetric, bqueue.Blocking)
	s.syncHeaderFetcher, err = blockfetcher.New(s.stateSync, s.AtipicialFsBlockFetcherCfg, log, func(item bqueue.Indexable) error { return s.syncHFetcherQueue.Put(item.(*block.Header)) },
		nil, blockfetcher.Headers)
	if err != nil {
		return nil, fmt.Errorf("failed to create Sync AtipicialFs HeaderFetcher: %w", err)
	}
	s.syncStateFetcher, err = statefetcher.New(s.stateSync, s.AtipicialFsStateFetcherCfg, chain.GetConfig().StateSyncInterval, log, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to create AtipicialFs StateFetcher: %w", err)
	}
	s.syncBFetcherQueue = bqueue.New[*block.Block](stateSyncBlockQueueAdapter{s.stateSync}, log, nil, s.AtipicialFsBlockFetcherCfg.BQueueSize, updateBlockQueueLenMetric, bqueue.Blocking)
	s.syncBlockFetcher, err = blockfetcher.New(s.stateSync, s.AtipicialFsBlockFetcherCfg, log, func(item bqueue.Indexable) error { return s.syncBFetcherQueue.Put(item.(*block.Block)) }, nil, blockfetcher.Blocks)
	if err != nil {
		return nil, fmt.Errorf("failed to create Sync AtipicialFs BlockFetcher: %w", err)
	}
	if s.config.AtipicialFsStateSyncExtensions {
...
func (s *Server) Start() {
	...
	if s.AtipicialFsBlockFetcherCfg.Enabled && !s.config.AtipicialFsStateSyncExtensions && !s.config.P2PStateExchangeExtensions {
		if err := s.blockFetcher.Start(); err != nil {
			s.log.Error("skipping AtipicialFs BlockFetcher", zap.Error(err))
		}
	}
	if s.config.AtipicialFsStateSyncExtensions {
		s.tryInitStateSync()
...
func (s *Server) stateSyncCallBack() {
	...
	if needHeaders {
		...
		if !s.syncHeaderFetcher.IsShutdown() {
			...
			if err != nil {
				s.log.Error("skipping AtipicialFs Sync HeaderFetcher", zap.Error(err))
			}
		}
	}
	if needBlocks {
		...
		if !s.syncBlockFetcher.IsShutdown() {
			if err := s.syncBlockFetcher.Start(s.stateSync.GetStateSyncPoint()); err != nil {
				s.log.Error("skipping AtipicialFs Sync BlockFetcher", zap.Error(err))
			}
		}
	}
	if needStorage {
		...
		if !s.syncStateFetcher.IsShutdown() {
			if err := s.syncStateFetcher.Start(s.stateSync.GetStateSyncPoint()); err != nil {
				s.log.Error("skipping AtipicialFs Sync StateFetcher", zap.Error(err))
			}
		}
	}
	if !isActive {
		s.syncBlockFetcher.Shutdown()
		if s.AtipicialFsBlockFetcherCfg.Enabled {
			if err := s.blockFetcher.Start(); err != nil {
				s.log.Error("skipping AtipicialFs BlockFetcher", zap.Error(err))
			}
		}
...
func (s *Server) handleBlockCmd(p Peer, block *block.Block) error {
	if s.blockFetcher.IsActive() || s.config.AtipicialFsStateSyncExtensions && s.stateSync.IsActive() {
		return nil
	}
...
// Reference: the method OnRelay in C#: https://github.com/atipicial-project/atipicial/blob/master/atipicial/Network/P2P/LocalNode.cs#L159
