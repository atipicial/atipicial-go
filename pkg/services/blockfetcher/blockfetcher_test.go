...
func TestServiceConstructor(t *testing.T) {
	...
	t.Run("default values", func(t *testing.T) {
		...
		require.Equal(t, service.cfg.Timeout, atipicialfs.DefaultTimeout)
		require.Equal(t, service.cfg.OIDBatchSize, DefaultQueueCacheSize/2)
		require.Equal(t, service.cfg.DownloaderWorkersCount, atipicialfs.DefaultDownloaderWorkersCount)
		require.Equal(t, service.IsActive(), false)
	})

	t.Run("AtipicialFs client", func(t *testing.T) {
		cfg := config.AtipicialFsBlockFetcher{
			AtipicialFsService: config.AtipicialFsService{
				InternalService: config.InternalService{
					Enabled: true,
		...
		err = service.Start()
		require.Error(t, err)
		require.Contains(t, err.Error(), "failed to dial AtipicialFs pool:")
		require.Equal(t, service.IsActive(), false)
	})
