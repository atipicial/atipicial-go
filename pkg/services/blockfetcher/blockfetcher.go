...
import (
	...
	gio "github.com/atipicial/atipicial-go/pkg/io"
	"github.com/atipicial/atipicial-go/pkg/network/bqueue"
	"github.com/atipicial/atipicial-go/pkg/services/helpers/atipicialfs"
	"github.com/github.com/atipicial/atipicialfs-sdk-go/client"
	"github.com/github.com/atipicial/atipicialfs-sdk-go/container"
...
// Service is a service that fetches blocks from AtipicialFs.
type Service struct {
	atipicialfs.BasicService
	// isActive denotes whether the service is working or in the process of shutdown.
	isActive      atomic.Bool
	isShutdown    atomic.Bool
	log           *zap.Logger
	cfg           config.AtipicialFsBlockFetcher
...
func New(chain Ledger, cfg config.AtipicialFsBlockFetcher, logger *zap.Logger, put func(item bqueue.Indexable) error, shutdownCallback func(), opt OperationMode) (*Service, error) {
	if !cfg.Enabled {
		return &Service{}, nil
	}
	if cfg.Timeout <= 0 {
		cfg.Timeout = atipicialfs.DefaultTimeout
	}
	if cfg.OIDBatchSize <= 0 {
		cfg.OIDBatchSize = cfg.BQueueSize / 2
	}
	if cfg.DownloaderWorkersCount <= 0 {
		cfg.DownloaderWorkersCount = atipicialfs.DefaultDownloaderWorkersCount
	}
	if cfg.BlockAttribute == "" {
		cfg.BlockAttribute = atipicialfs.DefaultBlockAttribute
	}

	basic, err := atipicialfs.NewBasicService(cfg.AtipicialFsService)
	if err != nil {
		return nil, fmt.Errorf("failed to create AtipicialFs service: %w", err)
	}
	return &Service{
		BasicService:  basic,
		chain:         chain,
...
// Start runs the AtipicialFs BlockFetcher service.
func (bfs *Service) Start(stopAt ...uint32) error {
	...
	bfs.log.Info("starting AtipicialFs BlockFetcher service", zap.String("mode", bfs.operationMode.String()))
	var (
	...
	if err = bfs.Pool.Dial(context.Background()); err != nil {
		...
		return fmt.Errorf("failed to dial AtipicialFs pool: %w", err)
	}
...
func (bfs *Service) oidDownloader() {
	...
	if err != nil {
		if !atipicialfs.IsContextCanceledErr(err) {
			bfs.log.Error("AtipicialFs BlockFetcher service: OID downloading routine failed", zap.Error(err))
		}
		force = true
	}
...
// blockDownloader downloads the block from AtipicialFs and sends it to the blocks channel.
...
// fetchOIDsBySearch fetches block OIDs from AtipicialFs by searching through the Block objects.
func (bfs *Service) fetchOIDsBySearch() error {
	...
	results, errs := atipicialfs.ObjectSearch(ctx, bfs.Pool, bfs.Account.PrivateKey(), bfs.ContainerID, filters, []string{bfs.cfg.BlockAttribute})
	...
	for {
		...
		case <-bfs.exiterToOIDDownloader:
			return nil
		case err := <-errs:
			if err != nil && !atipicialfs.IsContextCanceledErr(err) {
...
// Shutdown stops the AtipicialFs BlockFetcher service. It prevents service from new
...
func (bfs *Service) exiter() {
	...
	force := <-bfs.quit
	bfs.log.Info("shutting down AtipicialFs BlockFetcher service",
...
// IsShutdown returns true if the AtipicialFs BlockFetcher service is completely shutdown.
// The service can not be started again.
func (bfs *Service) IsShutdown() bool {
	return bfs.isShutdown.Load()
}

// IsActive returns true if the AtipicialFs BlockFetcher service is running.
...
func (bfs *Service) objectGet(ctx context.Context, oid string, index int) (io.ReadCloser, error) {
	u, err := url.Parse(fmt.Sprintf("%s:%s/%s", atipicialfs.URIScheme, bfs.cfg.ContainerID, oid))
	if err != nil {
		return nil, err
	}
	var rc io.ReadCloser
	err = bfs.Retry(func() error {
		rc, err = atipicialfs.GetWithClient(ctx, bfs.Pool, bfs.Account.PrivateKey(), u, false)
		return err
	})
	return rc, err
}
...
func (bfs *Service) objectGetRange(ctx context.Context, oid string, height int) (io.ReadCloser, error) {
	...
	u, err := url.Parse(fmt.Sprintf("%s:%s/%s/%s/%d|%d", atipicialfs.URIScheme, bfs.cfg.ContainerID, oid, "range", 0, size))
	if err != nil {
		return nil, err
	}
	var rc io.ReadCloser
