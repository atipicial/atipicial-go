...
func (n *Notary) newTxCallbackLoop() {
	for {
		...
		case tx := <-n.newTxs:
			...
			// ref. https://github.com/atipicial/atipicial-go/pull/4093#issuecomment-3682809371.
			r.lock.RUnlock()
...
func verifyIncompleteWitnesses(tx *transaction.Transaction, nKeysExpected uint8) ([]witnessInfo, error) {
	...
	for i, w := range tx.Scripts {
		...
		if !tx.Signers[i].Account.Equals(hash.Hash160(w.VerificationScript)) { // https://github.com/atipicial/atipicial-go/pull/1658#discussion_r564265987
