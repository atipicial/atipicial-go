...
func (o *Oracle) processRequest(priv *keys.PrivateKey, req request) error {
	...
	if err != nil {
		...
		case "https":
			...
			httpReq.Header.Set("User-Agent", "AtipicialOracleService/3.0")
		...
		case atipicialfs.URIScheme:
			if len(o.MainCfg.AtipicialFs.Nodes) == 0 {
				o.Log.Warn("no AtipicialFs nodes configured", zap.String("url", req.Req.URL))
				resp.Code = transaction.Error
				break
			}
			ctx, cancel := context.WithTimeout(context.Background(), o.MainCfg.AtipicialFs.Timeout)
			defer cancel()
			index := (int(req.ID) + incTx.attempts) % len(o.MainCfg.AtipicialFs.Nodes)
			rc, err := atipicialfs.Get(ctx, priv, u, o.MainCfg.AtipicialFs.Nodes[index])
			if err != nil {
				resp.Code = transaction.Error
