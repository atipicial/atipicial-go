...
// Constants related to AtipicialFs block storage.
const (
	// DefaultTimeout is the default timeout for AtipicialFs requests.
...
// Constants related to AtipicialFs pool request timeouts.
const (
	// DefaultDialTimeout is a default timeout used to establish connection with
	// AtipicialFs storage nodes.
	DefaultDialTimeout = 30 * time.Second
	// DefaultStreamTimeout is a default timeout used for AtipicialFs streams processing.
	// It has significantly large value to reliably avoid timeout problems with heavy
	// SEARCH requests.
	DefaultStreamTimeout = 10 * time.Minute
	// DefaultHealthcheckTimeout is a timeout for request to AtipicialFs storage node to
...
// BasicService is a minimal service structure for AtipicialFs fetchers.
...
func NewBasicService(cfg config.AtipicialFsService) (BasicService, error) {
	var (
		account     *wallet.Account
		containerID cid.ID
		err         error
...
// Ref. https://github.com/github.com/atipicial/atipicialfs-sdk-go/issues/624.
