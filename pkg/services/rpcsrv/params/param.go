...
// GetSignerWithWitness returns a atipicialrpc.SignerWithWitness value of the parameter.
func (p *Param) GetSignerWithWitness() (atipicialrpc.SignerWithWitness, error) {
	// This one doesn't need to be cached, it's used only once.
	c := atipicialrpc.SignerWithWitness{}
	err := json.Unmarshal(p.RawMessage, &c)
	if err != nil {
		return atipicialrpc.SignerWithWitness{}, fmt.Errorf("not a signer: %w", err)
	}
	return c, nil
}
