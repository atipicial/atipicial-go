	...
	expected := []testCase{
		...
		},
		{
			check: func(t *testing.T, p *Param) {
				...
				require.NoError(t, err)
				require.Equal(t, atipicialrpc.SignerWithWitness{Signer: transaction.Signer{Account: expectedAcc}}, actual)
			},
		...
		},
		{
			check: func(t *testing.T, p *Param) {
				...
				require.NoError(t, err)
				require.Equal(t, atipicialrpc.SignerWithWitness{Signer: transaction.Signer{Account: expectedAcc, Scopes: transaction.Global}}, actual)
			},
...
func TestGetWitness(t *testing.T) {
	...
	testCases := []struct {
		raw        string
		expected   atipicialrpc.SignerWithWitness
		shouldFail bool
	}{
		{
...
func TestParamGetSigner(t *testing.T) {
	c := atipicialrpc.SignerWithWitness{
		Signer: transaction.Signer{
