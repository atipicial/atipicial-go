...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/rpcclient/management"
	...
	"github.com/atipicial/atipicial-go/pkg/rpcclient/aeptoken"
	"github.com/atipicial/atipicial-go/pkg/rpcclient/nns"
	"github.com/atipicial/atipicial-go/pkg/rpcclient/notary"
	"github.com/atipicial/atipicial-go/pkg/rpcclient/oracle"
	"github.com/atipicial/atipicial-go/pkg/rpcclient/policy"
	"github.com/atipicial/atipicial-go/pkg/rpcclient/rolemgmt"
...
func TestClient_AEP17(t *testing.T) {
	...
	require.NoError(t, err)
	rub := aep17.NewReader(invoker.New(c, nil), h)
...
func TestClientPolicyContract(t *testing.T) {
	...
	for _, cs := range chain.GetNatives() {
	...
	txwhitelist3, err := polis.RemoveWhitelistFeeContractUnsigned(whitelisted[0].Hash, "hexDecode", 1)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	txrecover, err := polis.RecoverFundUnsigned(blocked[0], nativehashes.AtipicialDollar)
	require.NoError(t, err)
...
func TestClientManagementContract(t *testing.T) {
	...
	require.NoError(t, err)
	txdepl, err := man.DeployUnsigned(&cs1.AEF, &cs1.Manifest, nil) // Redeploy from a different account.
	require.NoError(t, err)
...
func TestClientATCContract(t *testing.T) {
	chain, _, httpSrv := initServerWithInMemoryChain(t)
	...
	atipicialR := atipicial.NewReader(invoker.New(c, nil))

	sym, err := atipicialR.Symbol()
	require.NoError(t, err)
	require.Equal(t, "ATC", sym)

	dec, err := atipicialR.Decimals()
	require.NoError(t, err)
	require.Equal(t, 0, dec)

	ts, err := atipicialR.TotalSupply()
	require.NoError(t, err)
	require.Equal(t, big.NewInt(1_0000_0000), ts)

	comm, err := atipicialR.GetCommittee()
	require.NoError(t, err)
	...
	vals, err := atipicialR.GetNextBlockValidators()
	require.NoError(t, err)
	...
	atipicialC := atipicial.New(act)

	txgpb, err := atipicialC.SetGasPerBlockUnsigned(10 * 1_0000_0000)
	require.NoError(t, err)
	txregp, err := atipicialC.SetRegisterPriceUnsigned(1_0000)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	atipicial0 := atipicial.New(act0)

	txreg, err := atipicial0.RegisterCandidateTransaction(testchain.PrivateKey(0).PublicKey())
	require.NoError(t, err)
	bl = testchain.NewBlock(t, chain, 1, 0, txreg)
	_, err = c.SubmitBlock(*bl)
	require.NoError(t, err)

	txvote, err := atipicial0.VoteTransaction(acc0, testchain.PrivateKey(0).PublicKey())
	require.NoError(t, err)
	bl = testchain.NewBlock(t, chain, 1, 0, txvote)
	_, err = c.SubmitBlock(*bl)
	require.NoError(t, err)

	txunreg, err := atipicial0.UnregisterCandidateTransaction(testchain.PrivateKey(0).PublicKey())
	require.NoError(t, err)
...
func TestNotaryActor(t *testing.T) {
	...
	*multiAcc = *acc
	...
	require.NoError(t, err)
	atipicialW := atipicial.New(nact)
	_, _, _, err = nact.Notarize(atipicialW.SetRegisterPriceTransaction(1_0000_0000))
	require.NoError(t, err)
}
...
func TestGetRawNotaryPoolAndTransaction(t *testing.T) {
	...
	t.Run("getrawnotarypool", func(t *testing.T) {
		...
		*multiAcc = *acc
		...
		tx1, err = atipicialW.SetRegisterPriceTransaction(1_0000_0000)
		require.NoError(t, err)
		...
		tx2, err = atipicialW.SetRegisterPriceTransaction(2_0000_0000)
		require.NoError(t, err)
	...
	t.Run("getrawnotarytransaction", func(t *testing.T) {
		t.Run("client GetRawNotaryTransaction", func(t *testing.T) {
			t.Run("unknown transaction", func(t *testing.T) {
				...
				require.Error(t, err)
				require.ErrorIs(t, err, atipicialrpc.ErrUnknownTransaction)
			})
			_ = tx1.Size()
			_ = tx2.Size()
...
func TestCreateAEP17TransferTx(t *testing.T) {
	chain, _, httpSrv := initServerWithInMemoryChain(t)
	...
	t.Run("default scope, multitransfer", func(t *testing.T) {
		...
		require.NoError(t, err)
		gazprom := gas.New(act)
		tx, err := gazprom.MultiTransferTransaction([]aep17.TransferParameters{
...
func TestClient_AEP11_ND(t *testing.T) {
	chain, _, httpSrv := initServerWithInMemoryChain(t)
	...
	require.NoError(t, err)
	n11 := aep11.NewNonDivisible(act, h)
	...
	t.Run("TotalSupply", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.EqualValues(t, big.NewInt(1), s) // the only `atipicial.com` of acc0
	})
	t.Run("Symbol", func(t *testing.T) {
		sym, err := n11.Symbol()
		require.NoError(t, err)
	...
	t.Run("TokenInfo", func(t *testing.T) {
		...
		require.Equal(t, &wallet.Token{
			...
			Decimals: 0,
			Symbol:   "NNS",
			Standard: manifest.AEP11StandardName,
		}, tok)
	})
	t.Run("BalanceOf", func(t *testing.T) {
	...
	t.Run("OwnerOf", func(t *testing.T) {
		b, err := n11.OwnerOf([]byte("atipicial.com"))
		require.NoError(t, err)
	...
	t.Run("Tokens", func(t *testing.T) {
		...
		require.Equal(t, [][]byte{[]byte("atipicial.com")}, items)
		require.NoError(t, iter.Terminate())
	})
	...
	t.Run("Properties", func(t *testing.T) {
		p, err := n11.Properties([]byte("atipicial.com"))
		require.NoError(t, err)
		blockRegisterDomain, err := chain.GetBlock(chain.GetHeaderHash(14)) // `atipicial.com` domain was registered in 14th block
		require.NoError(t, err)
		require.Equal(t, 1, len(blockRegisterDomain.Transactions))
		expected := stackitem.NewMap()
		expected.Add(stackitem.Make([]byte("name")), stackitem.Make([]byte("atipicial.com")))
	...
	t.Run("Transfer", func(t *testing.T) {
		_, _, err := n11.Transfer(testchain.PrivateKeyByID(1).GetScriptHash(), []byte("atipicial.com"), nil)
		require.NoError(t, err)
	})
}

func TestClient_AEP11_D(t *testing.T) {
	_, _, httpSrv := initServerWithInMemoryChain(t)
	...
	require.NoError(t, err)
	n11 := aep11.NewDivisible(act, nfsoHash)
	...
	t.Run("TokenInfo", func(t *testing.T) {
		tok, err := aeptoken.Info(c, nfsoHash)
		require.NoError(t, err)
		require.Equal(t, &wallet.Token{
			Name:     "AtipicialFs Object NFT",
			Hash:     nfsoHash,
			Decimals: 2,
			Symbol:   "NFSO",
			Standard: manifest.AEP11StandardName,
	...
	t.Run("Properties", func(t *testing.T) {
		...
		require.NoError(t, err)
		expected := stackitem.NewMap()
		expected.Add(stackitem.Make([]byte("name")), stackitem.NewBuffer([]byte("AtipicialFs Object "+base64.StdEncoding.EncodeToString(token1ID))))
...
func TestClient_NNS(t *testing.T) {
	...
	t.Run("IsAvailable, false", func(t *testing.T) {
		b, err := nnc.IsAvailable("atipicial.com")
		require.NoError(t, err)
		require.Equal(t, false, b)
	})
	t.Run("IsAvailable, true", func(t *testing.T) {
		b, err := nnc.IsAvailable("atipicialgo.com")
		require.NoError(t, err)
		require.Equal(t, true, b)
	})
	t.Run("Resolve, good", func(t *testing.T) {
		b, err := nnc.Resolve("atipicial.com", nns.A)
		require.NoError(t, err)
		require.Equal(t, "1.2.3.4", b)
	})
	t.Run("Resolve, bad", func(t *testing.T) {
		_, err := nnc.Resolve("atipicialgo.com", nns.A)
		require.Error(t, err)
	})
	t.Run("Resolve, CNAME", func(t *testing.T) {
		_, err := nnc.Resolve("atipicialgo.com", nns.CNAME)
		require.Error(t, err)
	})
	t.Run("GetAllRecords, good", func(t *testing.T) {
		iter, err := nnc.GetAllRecords("atipicial.com")
		require.NoError(t, err)
	...
	t.Run("GetAllRecordsExpanded, good", func(t *testing.T) {
		rss, err := nnc.GetAllRecordsExpanded("atipicial.com", 42)
		require.NoError(t, err)
		require.Equal(t, []nns.RecordState{
			{
				Name: "atipicial.com",
	...
	t.Run("GetAllRecords, bad", func(t *testing.T) {
		_, err := nnc.GetAllRecords("atipicialpython.com")
		require.Error(t, err)
	})
	t.Run("GetAllRecordsExpanded, bad", func(t *testing.T) {
		_, err := nnc.GetAllRecordsExpanded("atipicialpython.com", 7)
		require.Error(t, err)
	})
}
...
func TestClient_IteratorSessions(t *testing.T) {
	...
	t.Run("terminate session", func(t *testing.T) {
		t.Run("manually", func(t *testing.T) {
			...
			ok, err = c.TerminateSession(sID)
			require.Error(t, err)
			require.ErrorIs(t, err, atipicialrpc.ErrUnknownSession)
	...
	run := func(t *testing.T, ws bool) {
		...
		check := func(t *testing.T, tx *transaction.Transaction, h util.Uint256, vub uint32, errExpected bool) {
			...
			go func() {
				var err error
				if tx != nil {
					// Send one more time to ensure [atipicialrpc.ErrAlreadyExists] is properly handled by Waiter.
				...
				if len(rpcSrv.subscribers) == 1 { // single client
					for s := range rpcSrv.subscribers {
						...
						for _, f := range s.feeds {
							if f.event != atipicialrpc.InvalidEventID {
								count++
							}
						}
...
func testSubClientWaitWithMissedEvent(t *testing.T, local bool) {
	...
	overNotification := atipicialrpc.Notification{
...
func TestWSClient_SubscriptionsCompat(t *testing.T) {
	...
	checkRelevant := func(t *testing.T, filtered bool) {
		...
		var (
			...
			bFlt                           *atipicialrpc.BlockFilter
			txFlt                          *atipicialrpc.TxFilter
			ntfFlt                         *atipicialrpc.NotificationFilter
			aerFlt                         *atipicialrpc.ExecutionFilter
			memFlt                         *atipicialrpc.MempoolEventFilter
			err                            error
		)
		if filtered {
			bFlt = &atipicialrpc.BlockFilter{Primary: &primary}
			txFlt = &atipicialrpc.TxFilter{Sender: &sender}
			ntfFlt = &atipicialrpc.NotificationFilter{Name: &ntfName}
			aerFlt = &atipicialrpc.ExecutionFilter{State: &st}
			memFlt = &atipicialrpc.MempoolEventFilter{Type: new(mempoolevent.TransactionAdded)}
		}
...
func TestClient_GetStorageHistoric(t *testing.T) {
	...
	require.ErrorIs(t, atipicialrpc.ErrUnknownStorageItem, err)
}
...
func TestClient_AEP24(t *testing.T) {
	_, _, httpSrv := initServerWithInMemoryChain(t)
	...
	require.NoError(t, err)
	reader := aep24.NewRoyaltyReader(invoker.New(c, nil), h)
...
func TestGetBlockNotifications(t *testing.T) {
	...
	t.Run("empty filter", func(t *testing.T) {
		bn, err := c.GetBlockNotifications(chain.GetHeaderHash(1), &atipicialrpc.NotificationFilter{})
		require.NoError(t, err)
		require.NotEmpty(t, bn)
	})
