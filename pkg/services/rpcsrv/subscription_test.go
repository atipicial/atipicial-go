...
func callWSGetRaw(t *testing.T, ws *websocket.Conn, msg string, respCh <-chan []byte) *atipicialrpc.Response {
	var resp = new(atipicialrpc.Response)
...
func getNotification(t *testing.T, respCh <-chan []byte) *atipicialrpc.Notification {
	var resp = new(atipicialrpc.Notification)
	body := <-respCh
	require.NoError(t, json.Unmarshal(body, resp))
	return resp
}
...
func TestSubscriptions(t *testing.T) {
	...
	for _, b := range getTestBlocks(t) {
		...
		require.Equal(t, atipicialrpc.ExecutionEventID, resp.Event)
		for {
			resp = getNotification(t, respMsgs)
			if resp.Event != atipicialrpc.NotificationEventID {
				break
			}
		}
		for i := range b.Transactions {
			...
			for {
				...
				require.Equal(t, atipicialrpc.TransactionEventID, resp.Event)
				break
			}
		}
		resp = getNotification(t, respMsgs)
		...
		require.Equal(t, atipicialrpc.HeaderOfAddedBlockEventID, resp.Event)
		resp = getNotification(t, respMsgs)
		require.Equal(t, atipicialrpc.BlockEventID, resp.Event)
	}
	...
	for {
		resp := getNotification(t, respMsgs)
		if resp.Event == atipicialrpc.NotaryRequestEventID {
			break
		}
	}
	...
	require.Equal(t, atipicialrpc.MempoolEventID, e.Event)

	chain.GetMemPool().Remove(tx.Hash())
	e2 := getNotification(t, respMsgs)
	require.Equal(t, atipicialrpc.MempoolEventID, e2.Event)
...
func TestFilteredSubscriptions(t *testing.T) {
	...
	var cases = map[string]struct {
		params      string
		check       func(*testing.T, *atipicialrpc.Notification)
		shouldCheck bool
	}{
		"tx matching sender": {
			params:      `["transaction_added", {"sender":"` + goodSender.StringLE() + `"}]`,
			shouldCheck: true,
			check: func(t *testing.T, resp *atipicialrpc.Notification) {
		...
		"notification matching contract hash": {
			...
			check: func(t *testing.T, resp *atipicialrpc.Notification) {
				rmap := resp.Payload[0].(map[string]any)
				require.Equal(t, atipicialrpc.NotificationEventID, resp.Event)
		...
		"tx non-matching": {
			...
			shouldCheck: false,
			check: func(t *testing.T, _ *atipicialrpc.Notification) {
		...
		"execution non-matching": {
			...
			shouldCheck: false,
			check: func(t *testing.T, n *atipicialrpc.Notification) {
	...
	for name, this := range cases {
		t.Run(name, func(t *testing.T) {
			...
			for {
				resp := getNotification(t, respMsgs)
				rmap := resp.Payload[0].(map[string]any)
				if resp.Event == atipicialrpc.BlockEventID {
					index := rmap["index"].(float64)
...
func TestFilteredMempoolSubscriptions(t *testing.T) {
	...
	var cases = map[string]struct {
		...
		"mempool_event matching type": {
			...
			check: func(t *testing.T, resp *atipicialrpc.Notification) {
				require.Equal(t, atipicialrpc.MempoolEventID, resp.Event)
				rmap := resp.Payload[0].(map[string]any)
...
func TestFilteredNotaryRequestSubscriptions(t *testing.T) {
	...
	var cases = map[string]struct {
		...
		"matching sender": {
			...
			check: func(t *testing.T, resp *atipicialrpc.Notification) {
				rmap := resp.Payload[0].(map[string]any)
				require.Equal(t, atipicialrpc.NotaryRequestEventID, resp.Event)
				require.Equal(t, "added", rmap["type"].(string))
...
func TestSubscriptionOverflow(t *testing.T) {
	...
	for range blockCnt {
		resp := getNotification(t, respMsgs)
		if resp.Event != atipicialrpc.BlockEventID {
			require.Equal(t, atipicialrpc.MissedEventID, resp.Event)
			receivedMiss = true
			break
		}
	}
...
func TestFilteredSubscriptions_InvalidFilter(t *testing.T) {
	...
	for name, this := range cases {
		t.Run(name, func(t *testing.T) {
			...
			require.NotNil(t, resp.Error)
			require.Nil(t, resp.Result)
			require.Contains(t, resp.Error.Error(), atipicialrpc.ErrInvalidSubscriptionFilter.Error())
		})
	}
}
