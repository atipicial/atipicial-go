...
func TestRPCRequestsMetricNaming(t *testing.T) {
	...
	for _, mf := range mfs {
		switch mf.GetName() {
		case "atipicialgo_rpc_requests_time":
			foundMetric = true
		...
		default:
			if strings.HasPrefix(mf.GetName(), "atipicialgo_rpc_") && strings.HasSuffix(mf.GetName(), "_time") {
				t.Fatalf("obsolete per-method metric %q is still registered", mf.GetName())
			}
			if mf.GetName() == "atipicialgo_rpc_requests_total" {
