...
type abstractResult interface {
	RunForErrors(f func(jsonErr *atipicialrpc.Error))
}
...
type abstract struct {
	atipicialrpc.Header
	Error  *atipicialrpc.Error `json:"error,omitzero"`
	Result any           `json:"result,omitzero"`
}

// RunForErrors implements abstractResult interface.
func (a abstract) RunForErrors(f func(jsonErr *atipicialrpc.Error)) {
	if a.Error != nil {
		f(a.Error)
	}
}
...
// RunForErrors implements abstractResult interface.
func (ab abstractBatch) RunForErrors(f func(jsonErr *atipicialrpc.Error)) {
	for _, a := range ab {
		a.RunForErrors(f)
	}
}
