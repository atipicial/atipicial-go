...
type (
	...
	feed struct {
		event  atipicialrpc.EventID
		filter atipicialrpc.SubscriptionFilter
	}
)

// EventID implements atipicialrpc.EventComparator interface and returns notification ID.
func (f feed) EventID() atipicialrpc.EventID {
	return f.event
}

// Filter implements atipicialrpc.EventComparator interface and returns notification filter.
func (f feed) Filter() atipicialrpc.SubscriptionFilter {
	return f.filter
}
