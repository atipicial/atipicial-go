...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/core/interop/iterator"
	"github.com/atipicial/atipicial-go/pkg/core/mempool"
	"github.com/atipicial/atipicial-go/pkg/core/mempoolevent"
	"github.com/atipicial/atipicial-go/pkg/core/mpt"
	...
	"github.com/atipicial/atipicial-go/pkg/network"
	"github.com/atipicial/atipicial-go/pkg/network/payload"
	"github.com/atipicial/atipicial-go/pkg/services/oracle/broadcaster"
	"github.com/atipicial/atipicial-go/pkg/services/rpcsrv/params"
	...
	"github.com/atipicial/atipicial-go/pkg/smartcontract/manifest/standard"
	"github.com/atipicial/atipicial-go/pkg/smartcontract/scparser"
...
type (
	...
	Ledger interface {
		...
		FeePerByte() int64
		ForEachAEP11Transfer(acc util.Uint160, newestTimestamp uint64, f func(*state.AEP11Transfer) (bool, error)) error
		ForEachAEP17Transfer(acc util.Uint160, newestTimestamp uint64, f func(*state.AEP17Transfer) (bool, error)) error
		...
		GetAEP11Contracts() []util.Uint160
		GetAEP17Contracts() []util.Uint160
...
var rpcHandlers = map[string]func(*Server, params.Params) (any, *atipicialrpc.Error){
	...
	"getaep11balances":             (*Server).getAEP11Balances,
	"getaep11properties":           (*Server).getAEP11Properties,
	"getaep11transfers":            (*Server).getAEP11Transfers,
	"getaep17balances":             (*Server).getAEP17Balances,
	"getaep17transfers":            (*Server).getAEP17Transfers,
...
var rpcWsHandlers = map[string]func(*Server, params.Params, *subscriber) (any, *atipicialrpc.Error){
...
func New(chain Ledger, conf config.RPC, coreServer *network.Server,
	...
	if conf.MaxAEP11Tokens <= 0 {
		conf.MaxAEP11Tokens = config.DefaultMaxAEP11Tokens
		log.Info("MaxAEP11Tokens is not set or wrong, setting default value", zap.Int("MaxAEP11Tokens", config.DefaultMaxAEP11Tokens))
	}
...
func (s *Server) handleHTTPRequest(w http.ResponseWriter, httpRequest *http.Request) {
	...
	if httpRequest.URL.Path == "/ws" && httpRequest.Method == "GET" {
		...
		if numOfSubs >= s.config.MaxWebSocketClients {
			s.writeHTTPErrorResponse(
				params.NewIn(),
				w,
				atipicialrpc.NewInternalServerError("websocket users limit reached"),
			)
			return
		}
	...
	if httpRequest.Method != "POST" {
		s.writeHTTPErrorResponse(
			params.NewIn(),
			w,
			atipicialrpc.NewInvalidParamsError(fmt.Sprintf("invalid method '%s', please retry with 'POST'", httpRequest.Method)),
		)
		return
	}
	...
	if err != nil {
		s.writeHTTPErrorResponse(params.NewIn(), w, atipicialrpc.NewParseError(err.Error()))
		return
	}
...
func (s *Server) RegisterLocal(ctx context.Context, events chan<- atipicialrpc.Notification) func(*atipicialrpc.Request) (*atipicialrpc.Response, error) {
	subChan := make(chan intEvent, notificationBufSize)
	...
	return func(req *atipicialrpc.Request) (*atipicialrpc.Response, error) {
...
func (s *Server) handleInternal(req *atipicialrpc.Request, sub *subscriber) (*atipicialrpc.Response, error) {
	var (
		res    any
		rpcRes = &atipicialrpc.Response{
			HeaderAndError: atipicialrpc.HeaderAndError{
				Header: atipicialrpc.Header{
					JSONRPC: req.JSONRPC,
	...
	rpcRes.Error = atipicialrpc.NewMethodNotFoundError(fmt.Sprintf("method %q not supported", req.Method))
...
func (s *Server) handleIn(req *params.In, sub *subscriber) abstract {
	var res any
	var resErr *atipicialrpc.Error
	if req.JSONRPC != atipicialrpc.JSONRPCVersion {
		return s.packResponse(req, nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("problem parsing JSON: invalid version, expected 2.0 got '%s'", req.JSONRPC)))
	}
	...
	resErr = atipicialrpc.NewMethodNotFoundError(fmt.Sprintf("method %q not supported", req.Method))
	handler, ok := rpcHandlers[req.Method]
	if ok {
...
func (s *Server) handleLocalNotifications(ctx context.Context, events chan<- atipicialrpc.Notification, subChan <-chan intEvent, subscr *subscriber) {
eventloop:
	for {
		select {
		case <-s.shutdown:
...
requestloop:
	for err == nil {
		...
		res.RunForErrors(func(jsonErr *atipicialrpc.Error) {
...
func (s *Server) dropSubscriber(subscr *subscriber, isLocal bool) int {
	...
	for _, e := range subscr.feeds {
		if e.event != atipicialrpc.InvalidEventID {
...
func (s *Server) getBestBlockHash(_ params.Params) (any, *atipicialrpc.Error) {
	return "0x" + s.chain.CurrentBlockHash().StringLE(), nil
}

func (s *Server) getBlockCount(_ params.Params) (any, *atipicialrpc.Error) {
	return s.chain.BlockHeight() + 1, nil
}

func (s *Server) getBlockHeaderCount(_ params.Params) (any, *atipicialrpc.Error) {
	return s.chain.HeaderHeight() + 1, nil
}

func (s *Server) getConnectionCount(_ params.Params) (any, *atipicialrpc.Error) {
	return s.coreServer.PeerCount(), nil
}

func (s *Server) blockHashFromParam(param *params.Param) (util.Uint256, *atipicialrpc.Error) {
	var (
		hash util.Uint256
		err  error
	)
	if param == nil {
		return hash, atipicialrpc.ErrInvalidParams
	}
...
func (s *Server) getBlock(reqParams params.Params) (any, *atipicialrpc.Error) {
	...
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnknownBlock, err.Error())
	}
...
func (s *Server) getBlockHash(reqParams params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) getVersion(_ params.Params) (any, *atipicialrpc.Error) {
	port, err := s.coreServer.Port(nil) // any port will suite
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("cannot fetch tcp port: %s", err))
	}
	...
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("cannot fetch standbyCommittee: %s", err))
	}
	return &result.Version{
		...
		Protocol: result.Protocol{
			AddressVersion:              address.ATIPICIALPrefix,
			Network:                     cfg.Magic,
...
func (s *Server) getPeers(_ params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) getRawMempool(reqParams params.Params) (any, *atipicialrpc.Error) {
	...
	if signerP != nil && err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid signer filter: %s", err))
	}
...
func (s *Server) validateAddress(reqParams params.Params) (any, *atipicialrpc.Error) {
	param, err := reqParams.Value(0).GetString()
	if err != nil {
		return nil, atipicialrpc.ErrInvalidParams
	}
...
func (s *Server) calculateNetworkFee(reqParams params.Params) (any, *atipicialrpc.Error) {
	if len(reqParams) < 1 {
		return 0, atipicialrpc.ErrInvalidParams
	}
	byteTx, err := reqParams[0].GetBytesBase64()
	if err != nil {
		return 0, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, err.Error())
	}
	...
	if err != nil {
		return 0, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("failed to compute tx size: %s", err))
	}
	...
	for i, signer := range tx.Signers {
		...
		if len(w.InvocationScript) == 0 { // No invocation provided, try to infer one.
			...
			if len(w.VerificationScript) == 0 { // Contract-based verification
				...
				if cs == nil {
					return 0, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidVerificationFunction, fmt.Sprintf("signer %d (%s) has no verification script and no deployed contract", i, address.Uint160ToString(signer.Account)))
				}
				md := cs.Manifest.ABI.GetMethod(manifest.MethodVerify, -1)
				if md == nil || md.ReturnType != smartcontract.BoolType {
					return 0, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidVerificationFunction, fmt.Sprintf("signer %d has no verify method in deployed contract", i))
				}
			...
			if inv.Err != nil {
				return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to create dummy invocation script (signer %d): %s", i, inv.Err.Error()))
			}
		...
		if err != nil && !errors.Is(err, core.ErrInvalidSignature) {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidSignature, fmt.Sprintf("witness %d: %s", i, err))
		}
...
func (s *Server) getApplicationLog(reqParams params.Params) (any, *atipicialrpc.Error) {
	...
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnknownScriptContainer, fmt.Sprintf("failed to locate application log: %s", err))
	}
	return result.NewApplicationLog(hash, appExecResults, trig), nil
}

func (s *Server) getAEP11Tokens(h util.Uint160, acc util.Uint160, bw *io.BufBinWriter) ([]stackitem.Item, string, int, error) {
	...
	vals := iterator.Values(items[0], s.config.MaxAEP11Tokens)
...
func (s *Server) getAEP11Balances(ps params.Params) (any, *atipicialrpc.Error) {
	u, err := ps.Value(0).GetUint160FromAddressOrHex()
	if err != nil {
		return nil, atipicialrpc.ErrInvalidParams
	}

	bs := &result.AEP11Balances{
		Address:  address.Uint160ToString(u),
		Balances: []result.AEP11AssetBalance{},
	}
	lastUpdated, err := s.chain.GetTokenLastUpdated(u)
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Failed to get AEP-11 last updated block: %s", err.Error()))
	}
	var count int
	stateSyncPoint := lastUpdated[math.MinInt32]
	bw := io.NewBufBinWriter()
contract_loop:
	for _, h := range s.chain.GetAEP11Contracts() {
		toks, sym, dec, err := s.getAEP11Tokens(h, u, bw)
		if err != nil {
			continue
		}
		if len(toks) == 0 {
		...
		isDivisible := (standard.ComplyABI(&cs.Manifest, standard.Aep11Divisible) == nil)
		lub, ok := lastUpdated[cs.ID]
		if !ok {
			cfg := s.chain.GetConfig()
			if !cfg.P2PStateExchangeExtensions && cfg.RemoveUntraceableBlocks {
				return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to get LastUpdatedBlock for balance of %s token: internal database inconsistency", cs.Hash.StringLE()))
			}
			lub = stateSyncPoint
		}
		bs.Balances = append(bs.Balances, result.AEP11AssetBalance{
			Asset:    h,
			Decimals: dec,
			Name:     cs.Manifest.Name,
			Symbol:   sym,
			Tokens:   make([]result.AEP11TokenBalance, 0, len(toks)),
		})
		...
		for i := range toks {
			...
			var amount = "1"
			if isDivisible {
				balance, err := s.getAEP11DTokenBalance(h, u, id, bw)
				if err != nil {
					continue
				}
				if balance.Sign() == 0 {
			...
			count++
			curAsset.Tokens = append(curAsset.Tokens, result.AEP11TokenBalance{
				ID:          hex.EncodeToString(id),
				Amount:      amount,
				LastUpdated: lub,
			})
			if count >= s.config.MaxAEP11Tokens {
				break contract_loop
			}
		}
	}
...
func (s *Server) invokeAEP11Properties(h util.Uint160, id []byte, bw *io.BufBinWriter) ([]stackitem.MapElement, error) {
...
func (s *Server) getAEP11Properties(ps params.Params) (any, *atipicialrpc.Error) {
	...
	props, err := s.invokeAEP11Properties(asset, token, nil)
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrExecutionFailed, fmt.Sprintf("Failed to get AEP-11 properties: %s", err.Error()))
	}
	...
	for _, kv := range props {
		...
		var val any
		if result.KnownAEP11Properties[string(key)] || kv.Value.Type() != stackitem.AnyT {
			v, err := kv.Value.TryBytes()
			if err != nil {
				continue
			}
			if result.KnownAEP11Properties[string(key)] {
				val = string(v)
			} else {
				val = v
			}
...
func (s *Server) getAEP17Balances(ps params.Params) (any, *atipicialrpc.Error) {
	u, err := ps.Value(0).GetUint160FromAddressOrHex()
	if err != nil {
		return nil, atipicialrpc.ErrInvalidParams
	}

	bs := &result.AEP17Balances{
		Address:  address.Uint160ToString(u),
		Balances: []result.AEP17Balance{},
	}
	lastUpdated, err := s.chain.GetTokenLastUpdated(u)
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Failed to get AEP-17 last updated block: %s", err.Error()))
	}
	stateSyncPoint := lastUpdated[math.MinInt32]
	bw := io.NewBufBinWriter()
	for _, h := range s.chain.GetAEP17Contracts() {
		balance, sym, dec, err := s.getAEP17TokenBalance(h, u, bw)
		if err != nil {
			continue
		}
		if balance.Sign() == 0 {
		...
		bs.Balances = append(bs.Balances, result.AEP17Balance{
			Asset:       h,
			Amount:      balance.String(),
			Decimals:    dec,
			LastUpdated: lub,
...
func (s *Server) getAEP17TokenBalance(h util.Uint160, acc util.Uint160, bw *io.BufBinWriter) (*big.Int, string, int, error) {
...
func (s *Server) getAEP11DTokenBalance(h util.Uint160, acc util.Uint160, id []byte, bw *io.BufBinWriter) (*big.Int, error) {
...
func (s *Server) getAEP11Transfers(ps params.Params) (any, *atipicialrpc.Error) {
	return s.getTokenTransfers(ps, true)
}

func (s *Server) getAEP17Transfers(ps params.Params) (any, *atipicialrpc.Error) {
	return s.getTokenTransfers(ps, false)
}

func (s *Server) getTokenTransfers(ps params.Params, isAEP11 bool) (any, *atipicialrpc.Error) {
	u, err := ps.Value(0).GetUint160FromAddressOrHex()
	if err != nil {
		return nil, atipicialrpc.ErrInvalidParams
	}
	...
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("malformed timestamps/limit: %s", err))
	}
	...
	var handleTransfer = func(tr *state.AEP17Transfer) (*result.AEP17Transfer, *result.AEP17Transfer, bool, error) {
		var received, sent *result.AEP17Transfer
	...
	if !isAEP11 {
		err = s.chain.ForEachAEP17Transfer(u, end, func(tr *state.AEP17Transfer) (bool, error) {
		...
		err = s.chain.ForEachAEP11Transfer(u, end, func(tr *state.AEP11Transfer) (bool, error) {
			r, s, res, err := handleTransfer(&tr.AEP17Transfer)
			if err == nil {
				id := hex.EncodeToString(tr.ID)
				if r != nil {
					bs.Received = append(bs.Received, aep17TransferToAEP11(r, id))
				}
				if s != nil {
					bs.Sent = append(bs.Sent, aep17TransferToAEP11(s, id))
				}
			}
			return res, err
		})
	...
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("invalid transfer log: %s", err))
	}
	return bs, nil
}
...
func (s *Server) contractIDFromParam(param *params.Param, root ...util.Uint256) (int32, *atipicialrpc.Error) {
	if param == nil {
		return 0, atipicialrpc.ErrInvalidParams
	}
	...
	if len(root) == 0 {
		...
		if cs == nil {
			return 0, atipicialrpc.ErrUnknownContract
		}
	} else {
		var respErr *atipicialrpc.Error
...
func (s *Server) contractScriptHashFromParam(param *params.Param) (util.Uint160, *atipicialrpc.Error) {
	var result util.Uint160
	if param == nil {
		return result, atipicialrpc.ErrInvalidParams
	}
	nameOrHashOrIndex, err := param.GetString()
	if err != nil {
		return result, atipicialrpc.ErrInvalidParams
	...
	id, err := strconv.ParseInt(nameOrHashOrIndex, 10, 32)
	if err != nil {
		return result, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("Invalid contract identifier (name/hash/index is expected) : %s", err.Error()))
	}
	result, err = s.chain.GetContractScriptHash(int32(id))
	if err != nil {
		return result, atipicialrpc.ErrUnknownContract
	}
	return result, nil
}
...
func (s *Server) getProof(ps params.Params) (any, *atipicialrpc.Error) {
	if s.chain.GetConfig().KeepOnlyLatestState {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnsupportedState, fmt.Sprintf("'getproof' is not supported: %s", errKeepOnlyLatestState))
	}
	...
	if err != nil {
		if errors.Is(err, mpt.ErrNotFound) {
			return nil, atipicialrpc.ErrUnknownStorageItem
		}
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to get proof: %s", err))
	}
...
func (s *Server) verifyProof(ps params.Params) (any, *atipicialrpc.Error) {
	if s.chain.GetConfig().KeepOnlyLatestState {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnsupportedState, fmt.Sprintf("'verifyproof' is not supported: %s", errKeepOnlyLatestState))
	}
	root, err := ps.Value(0).GetUint256()
	if err != nil {
		return nil, atipicialrpc.ErrInvalidParams
	...
	if !ok {
		return nil, atipicialrpc.ErrInvalidProof
	}
	vp.Value = val
	return vp, nil
}

func (s *Server) getState(ps params.Params) (any, *atipicialrpc.Error) {
	...
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, "invalid contract hash")
	}
	key, err := ps.Value(2).GetBytesBase64()
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, "invalid key")
	}
	...
	if err != nil {
		if errors.Is(err, mpt.ErrNotFound) {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid key: %s", err.Error()))
		}
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Failed to get historical item state: %s", err.Error()))
	}
	return res, nil
}

func (s *Server) findStates(ps params.Params) (any, *atipicialrpc.Error) {
	root, respErr := s.getStateRootFromParam(ps.Value(0))
	if respErr != nil {
		return nil, respErr
	}
	csHash, err := ps.Value(1).GetUint160FromHex()
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid contract hash: %s", err))
	}
	prefix, err := ps.Value(2).GetBytesBase64()
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid prefix: %s", err))
	}
	var (
		key   []byte
	...
	if len(ps) > 3 {
		...
		if err != nil {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid key: %s", err))
		}
		if len(key) > 0 {
			if !bytes.HasPrefix(key, prefix) {
				return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, "key doesn't match prefix")
			}
			key = key[len(prefix):]
		} else {
	...
	if len(ps) > 4 {
		...
		if err != nil {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid count: %s", err))
		}
	...
	if err != nil && !errors.Is(err, mpt.ErrNotFound) {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to find state items: %s", err))
	}
	...
	if len(kvs) > 0 {
		...
		if err != nil {
			return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to get first proof: %s", err))
		}
	...
	if len(kvs) > 1 {
		...
		if err != nil {
			return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to get last proof: %s", err))
		}
...
func (s *Server) getStateRootFromParam(p *params.Param) (util.Uint256, *atipicialrpc.Error) {
	root, err := p.GetUint256()
	if err != nil {
		return util.Uint256{}, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, "invalid stateroot")
	}
	if s.chain.GetConfig().KeepOnlyLatestState {
		curr, err := s.chain.GetStateModule().GetStateRoot(s.chain.BlockHeight())
		if err != nil {
			return util.Uint256{}, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to get current stateroot: %s", err))
		}
		if !curr.Root.Equals(root) {
			return util.Uint256{}, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnsupportedState, fmt.Sprintf("state-based methods are not supported for old states: %s", errKeepOnlyLatestState))
		}
	}
	return root, nil
}

func (s *Server) findStorage(reqParams params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) findStorageInternal(id int32, prefix []byte, start, take int, seeker ContractStorageSeeker) (any, *atipicialrpc.Error) {
	var (
		i   int
		end = start + take
...
func (s *Server) findStorageHistoric(reqParams params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) getFindStorageParams(reqParams params.Params, root ...util.Uint256) (int32, []byte, int, int, *atipicialrpc.Error) {
	if len(reqParams) < 2 {
		return 0, nil, 0, 0, atipicialrpc.ErrInvalidParams
	}
	...
	if err != nil {
		return 0, nil, 0, 0, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid prefix: %s", err))
	}
	...
	if len(reqParams) > 2 {
		...
		if err != nil {
			return 0, nil, 0, 0, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid start: %s", err))
		}
	}
	return id, prefix, skip, s.config.MaxFindStorageResultItems, nil
}

func (s *Server) getHistoricalContractState(root util.Uint256, csHash util.Uint160) (*state.Contract, *atipicialrpc.Error) {
	...
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnknownContract, fmt.Sprintf("Failed to get historical contract state: %s", err.Error()))
	}
	contract := new(state.Contract)
	err = stackitem.DeserializeConvertible(csBytes, contract)
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to deserialize historical contract state: %s", err))
	}
	return contract, nil
}

func (s *Server) getStateHeight(_ params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) getStateRoot(ps params.Params) (any, *atipicialrpc.Error) {
	p := ps.Value(0)
	if p == nil {
		return nil, atipicialrpc.NewInvalidParamsError("missing stateroot identifier")
	}
	...
	if err == nil {
		if err := checkUint32(height); err != nil {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, err.Error())
		}
	...
	if err != nil {
		return nil, atipicialrpc.ErrUnknownStateRoot
	}
	return rt, nil
}

func (s *Server) getStorage(ps params.Params) (any, *atipicialrpc.Error) {
	...
	if item == nil {
		return "", atipicialrpc.ErrUnknownStorageItem
	}

	return []byte(item), nil
}

func (s *Server) getStorageHistoric(ps params.Params) (any, *atipicialrpc.Error) {
	root, respErr := s.getStateRootFromParam(ps.Value(0))
	if respErr != nil {
		return nil, respErr
	}
	...
	if err != nil && !errors.Is(err, mpt.ErrNotFound) {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to get state item: %s", err))
	}
	if v == nil {
		return "", atipicialrpc.ErrUnknownStorageItem
	}
...
func (s *Server) getrawtransaction(reqParams params.Params) (any, *atipicialrpc.Error) {
	...
	if err != nil {
		return nil, atipicialrpc.ErrUnknownTransaction
	}
	if v, _ := reqParams.Value(1).GetBoolean(); v {
		...
		if err != nil {
			return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Failed to get header for the transaction: %s", err.Error()))
		}
		aers, err := s.chain.GetAppExecResults(txHash, trigger.Application)
		if err != nil {
			return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Failed to get application log for the transaction: %s", err.Error()))
		}
		if len(aers) == 0 {
			return nil, atipicialrpc.NewInternalServerError("Inconsistent application log: application log for the transaction is empty")
		}
...
func (s *Server) getTransactionHeight(ps params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) getContractState(reqParams params.Params) (any, *atipicialrpc.Error) {
	...
	cs := s.chain.GetContractState(scriptHash)
	if cs == nil {
		return nil, atipicialrpc.ErrUnknownContract
	}
	return cs, nil
}

func (s *Server) getNativeContracts(_ params.Params) (any, *atipicialrpc.Error) {
	return s.chain.GetNatives(), nil
}

// getBlockSysFee returns the system fees of the block, based on the specified index.
func (s *Server) getBlockSysFee(reqParams params.Params) (any, *atipicialrpc.Error) {
	num, err := s.blockHeightFromParam(reqParams.Value(0))
	if err != nil {
		return 0, atipicialrpc.WrapErrorWithData(err, fmt.Sprintf("invalid block height: %s", err.Data))
	}

	headerHash := s.chain.GetHeaderHash(num)
	block, errBlock := s.chain.GetBlock(headerHash)
	if errBlock != nil {
		return 0, atipicialrpc.ErrUnknownBlock
	}
...
func (s *Server) getBlockHeader(reqParams params.Params) (any, *atipicialrpc.Error) {
	param := reqParams.Value(0)
	hash, respErr := s.blockHashFromParam(param)
	if respErr != nil {
		return nil, respErr
	...
	if err != nil {
		return nil, atipicialrpc.ErrUnknownBlock
	}
	...
	if buf.Err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("encoding error: %s", buf.Err))
	}
	return buf.Bytes(), nil
}

// getUnclaimedGas returns unclaimed GAS amount of the specified address.
func (s *Server) getUnclaimedGas(ps params.Params) (any, *atipicialrpc.Error) {
	u, err := ps.Value(0).GetUint160FromAddressOrHex()
	if err != nil {
		return nil, atipicialrpc.ErrInvalidParams
	}

	atipicial, _ := s.chain.GetGoverningTokenBalance(u)
	if atipicial.Sign() == 0 {
	...
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Can't calculate claimable: %s", err.Error()))
	}
	return result.UnclaimedGas{
		Address:   u,
		Unclaimed: *gas,
...
func (s *Server) getCandidates(_ params.Params) (any, *atipicialrpc.Error) {
	var validators keys.PublicKeys

	validators, err := s.chain.GetNextBlockValidators()
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Can't get next block validators: %s", err.Error()))
	}
	enrollments, err := s.chain.GetEnrollments()
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Can't get enrollments: %s", err.Error()))
	}
...
func (s *Server) getNextBlockValidators(_ params.Params) (any, *atipicialrpc.Error) {
	validators, err := s.chain.GetNextBlockValidators()
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("Can't get next block validators: %s", err.Error()))
	}
...
// getCommittee returns the current list of ATC committee members.
func (s *Server) getCommittee(_ params.Params) (any, *atipicialrpc.Error) {
	keys, err := s.chain.GetCommittee()
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("can't get committee members: %s", err))
	}
	return keys, nil
}

// invokeFunction implements the `invokeFunction` RPC call.
func (s *Server) invokeFunction(reqParams params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) invokeFunctionHistoric(reqParams params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) getInvokeFunctionParams(reqParams params.Params, currIndex ...uint32) (*transaction.Transaction, bool, *atipicialrpc.Error) {
	if len(reqParams) < 2 {
		return nil, false, atipicialrpc.ErrInvalidParams
	}
	...
	if err != nil {
		return nil, false, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("can't create invocation script: %s", err))
	}

	return s.mockTx(script, reqParams.Value(3), reqParams.Value(4), currIndex...)
}

func (s *Server) mockTx(script []byte, signersParam *params.Param, verboseParam *params.Param, currIndex ...uint32) (*transaction.Transaction, bool, *atipicialrpc.Error) {
	...
	if signersParam != nil {
		...
		if err != nil {
			return nil, false, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("failed to get signers or witnesses: %s", err))
		}
		tx.Signers = signers
...
func (s *Server) invokescript(reqParams params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) invokescripthistoric(reqParams params.Params) (any, *atipicialrpc.Error) {
	nextH, respErr := s.getHistoricParams(reqParams)
	if respErr != nil {
		return nil, respErr
	}
...
func (s *Server) getInvokeScriptParams(reqParams params.Params, currIndex ...uint32) (*transaction.Transaction, bool, *atipicialrpc.Error) {
...
func (s *Server) fakeTxFromParam(p *params.Param) (*transaction.Transaction, *atipicialrpc.Error) {
	tx, err := p.GetFakeTx()
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("failed to unmarshal fake transaction: %s", err))
	}
	if len(tx.Script) == 0 {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, "transaction script is mandatory")
	}
	return tx, nil
}

func (s *Server) fakeBlockFromParam(p *params.Param) (*block.Block, *atipicialrpc.Error) {
	base, err := s.chain.GetFakeNextBlock(s.chain.BlockHeight() + 1)
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to mock block execution container: %s", err))
	}
	if p != nil && !p.IsNull() {
		h, err := p.GetFakeHeader()
		if err != nil {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("failed to unmarshal fake block: %s", err))
		}
...
func (s *Server) invokeContainedScript(reqParams params.Params) (any, *atipicialrpc.Error) {
	if len(reqParams) < 1 {
		return nil, atipicialrpc.ErrInvalidParams
	}
	...
	if len(reqParams) > 2 {
		...
		if err != nil {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid trigger parameter: %s", err))
		}
		trig, err = trigger.FromString(t)
		if err != nil {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("failed to parse trigger: %s", err))
		}
		if trig != trigger.Application && trig != trigger.Verification {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("unsupported trigger: expected %s or %s, got %s", trigger.Application, trigger.Verification, trig))
		}
	}
	if len(reqParams) > 3 {
		var err error
		...
		if err != nil {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid verbose parameter: %s", err))
		}
	}
...
func (s *Server) invokeContractVerify(reqParams params.Params) (any, *atipicialrpc.Error) {
...
func (s *Server) invokeContractVerifyHistoric(reqParams params.Params) (any, *atipicialrpc.Error) {
	nextH, respErr := s.getHistoricParams(reqParams)
	if respErr != nil {
		return nil, respErr
	}
...
func (s *Server) getInvokeContractVerifyParams(reqParams params.Params) (util.Uint160, *transaction.Transaction, []byte, *atipicialrpc.Error) {
	scriptHash, responseErr := s.contractScriptHashFromParam(reqParams.Value(0))
	if responseErr != nil {
	...
	if len(reqParams) > 1 {
		...
		if err != nil {
			return util.Uint160{}, nil, nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, err.Error())
		}
		if len(args) > 0 {
			err := params.ExpandArrayIntoScript(bw.BinWriter, args)
			if err != nil {
				return util.Uint160{}, nil, nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("can't create witness invocation script: %s", err))
			}
		}
	}
...
func (s *Server) getHistoricParams(reqParams params.Params) (uint32, *atipicialrpc.Error) {
	if s.chain.GetConfig().KeepOnlyLatestState {
		return 0, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnsupportedState, fmt.Sprintf("only latest state is supported: %s", errKeepOnlyLatestState))
	}
	if len(reqParams) < 1 {
		return 0, atipicialrpc.ErrInvalidParams
	}
	...
	if respErr != nil {
		hash, err := reqParams.Value(0).GetUint256()
		if err != nil {
			return 0, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("invalid block hash or index or stateroot hash: %s", err))
		}
		b, err := s.chain.GetBlock(hash)
		if err != nil {
			stateH, err := s.chain.GetStateModule().GetLatestStateHeight(hash)
			if err != nil {
				return 0, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("unknown block or stateroot: %s", err))
			}
			height = stateH
		} else {
			height = b.Index
...
func (s *Server) prepareInvocationContext(t trigger.Type, script []byte, contractScriptHash util.Uint160, tx *transaction.Transaction, b *block.Block, nextH *uint32, verbose bool) (*interop.Context, *atipicialrpc.Error) {
	var (
		err error
		ic  *interop.Context
	)
	if nextH == nil {
		...
		if err != nil {
			return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to create test VM: %s", err))
		}
	} else {
		ic, err = s.chain.GetTestHistoricVM(t, tx, *nextH)
		if err != nil {
			return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to create historic VM: %s", err))
		}
	}
	if verbose {
	...
	if t == trigger.Verification && b == nil { // don't initialize verification context for contained invocations.
		...
		if err != nil {
			...
			case errors.Is(err, core.ErrUnknownVerificationContract):
				return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnknownContract, err.Error())
			case errors.Is(err, core.ErrInvalidVerificationContract):
				return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidVerificationFunction, err.Error())
			default:
				return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("can't prepare verification VM: %s", err))
			}
		}
	} else {
		ic.VM.LoadScriptWithFlags(script, callflag.All)
...
func (s *Server) runScriptInVM(t trigger.Type, script []byte, contractScriptHash util.Uint160, tx *transaction.Transaction, b *block.Block, nextH *uint32, verbose bool) (*result.Invoke, *atipicialrpc.Error) {
	...
	if sess != nil {
		...
		if len(s.sessions) >= s.config.SessionPoolSize {
			ic.Finalize()
			s.sessionsLock.Unlock()
			return nil, atipicialrpc.NewInternalServerError("max session capacity reached")
		}
...
func (s *Server) traverseIterator(reqParams params.Params) (any, *atipicialrpc.Error) {
	if !s.config.SessionEnabled {
		return nil, atipicialrpc.ErrSessionsDisabled
	}
	sID, err := reqParams.Value(0).GetUUID()
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("invalid session ID: %s", err))
	}
	iID, err := reqParams.Value(1).GetUUID()
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("invalid iterator ID: %s", err))
	}
	count, err := reqParams.Value(2).GetInt()
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("invalid iterator items count: %s", err))
	}
	if err := checkInt32(count); err != nil {
		return nil, atipicialrpc.NewInvalidParamsError("invalid iterator items count: not an int32")
	}
	if count > s.config.MaxIteratorResultItems {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("iterator items count (%d) is out of range (%d at max)", count, s.config.MaxIteratorResultItems))
	}
	...
	if !ok {
		s.sessionsLock.Unlock()
		return nil, atipicialrpc.ErrUnknownSession
	}
	...
	if !found {
		return nil, atipicialrpc.ErrUnknownIterator
	}
	...
	for j := range iVals {
		...
		if err != nil {
			return nil, atipicialrpc.NewInternalServerError(fmt.Sprintf("failed to marshal iterator value: %s", err))
		}
	}
	return result, nil
}

func (s *Server) terminateSession(reqParams params.Params) (any, *atipicialrpc.Error) {
	if !s.config.SessionEnabled {
		return nil, atipicialrpc.ErrSessionsDisabled
	}
	sID, err := reqParams.Value(0).GetUUID()
...
// submitBlock broadcasts a raw block over the Atipicial network.
func (s *Server) submitBlock(reqParams params.Params) (any, *atipicialrpc.Error) {
	base64Bytes, err := reqParams.Value(0).GetString()
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("not a string: %s", err))
	}
	maxBytes := base64.StdEncoding.EncodedLen(int(s.chain.GetConfig().MaxBlockSize))
	if len(base64Bytes) > maxBytes {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidSize, fmt.Sprintf("base64-encoded block size exceeds maximum allowed: %d vs %d", len(base64Bytes), maxBytes))
	}
	blockBytes, err := base64.StdEncoding.DecodeString(base64Bytes)
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("not a base64: %s", err))
	}
	b := block.New(s.stateRootEnabled)
	r := io.NewBinReaderFromBuf(blockBytes)
	b.DecodeBinary(r)
	if r.Err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("can't decode block: %s", r.Err))
	}
	return getRelayResult(s.chain.AddBlock(b), b.Hash())
}

// submitNotaryRequest broadcasts P2PNotaryRequest over the Atipicial network.
func (s *Server) submitNotaryRequest(ps params.Params) (any, *atipicialrpc.Error) {
	if !s.chain.P2PSigExtensionsEnabled() {
		return nil, atipicialrpc.NewInternalServerError("P2PSignatureExtensions are disabled")
	}
	...
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("can't decode notary payload: %s", err))
	}
...
func getRelayResult(err error, hash util.Uint256) (any, *atipicialrpc.Error) {
	switch {
	case err == nil:
	...
	case errors.Is(err, core.ErrTxExpired) || errors.Is(err, core.ErrTxNotYetValid):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrExpiredTransaction, err.Error())
	case errors.Is(err, core.ErrAlreadyExists):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrAlreadyExists, err.Error())
	case errors.Is(err, core.ErrAlreadyInPool):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrAlreadyInPool, err.Error())
	case errors.Is(err, core.ErrOOM):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrMempoolCapReached, err.Error())
	case errors.Is(err, core.ErrPolicy):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrPolicyFailed, err.Error())
	case errors.Is(err, core.ErrInvalidScript):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidScript, err.Error())
	case errors.Is(err, core.ErrTxTooBig):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidSize, err.Error())
	case errors.Is(err, core.ErrTxSmallNetworkFee):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInsufficientNetworkFee, err.Error())
	case errors.Is(err, core.ErrInvalidAttribute):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidAttribute, err.Error())
	case errors.Is(err, core.ErrInsufficientFunds), errors.Is(err, core.ErrMemPoolConflict):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInsufficientFunds, err.Error())
	case errors.Is(err, core.ErrInvalidSignature):
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidSignature, err.Error())
	default:
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrVerificationFailed, err.Error())
	}
}

func (s *Server) submitOracleResponse(ps params.Params) (any, *atipicialrpc.Error) {
	oraclePtr := s.oracle.Load()
	if oraclePtr == nil {
		return nil, atipicialrpc.ErrOracleDisabled
	}
	...
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("public key is missing: %s", err))
	}
	reqID, err := ps.Value(1).GetInt()
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("request ID is missing: %s", err))
	}
	txSig, err := ps.Value(2).GetBytesBase64()
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("tx signature is missing: %s", err))
	}
	msgSig, err := ps.Value(3).GetBytesBase64()
	if err != nil {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("msg signature is missing: %s", err))
	}
	data := broadcaster.GetMessage(pubBytes, uint64(reqID), txSig)
	if !pub.Verify(msgSig, hash.Sha256(data).BytesBE()) {
		return nil, atipicialrpc.ErrInvalidSignature
	}
	oracle.AddResponse(pub, uint64(reqID), txSig)
	return json.RawMessage([]byte("{}")), nil
}

func (s *Server) sendrawtransaction(reqParams params.Params) (any, *atipicialrpc.Error) {
	if len(reqParams) < 1 {
		return nil, atipicialrpc.NewInvalidParamsError("not enough parameters")
	}
	...
	if len(base64Bytes) > maxBase64TransactionSize {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidSize, fmt.Sprintf("base64-encoded transaction size exceeds maximum allowed: %d vs %d", len(base64Bytes), maxBase64TransactionSize))
	}
	...
	tx, err := transaction.NewTransactionFromBytes(byteTx)
	if err != nil {
		return nil, atipicialrpc.NewInvalidParamsError(fmt.Sprintf("can't decode transaction: %s", err))
	}
...
func (s *Server) subscribe(reqParams params.Params, sub *subscriber) (any, *atipicialrpc.Error) {
	streamName, err := reqParams.Value(0).GetString()
	if err != nil {
		return nil, atipicialrpc.ErrInvalidParams
	}
	event, err := atipicialrpc.GetEventIDFromString(streamName)
	if err != nil || event == atipicialrpc.MissedEventID {
		return nil, atipicialrpc.ErrInvalidParams
	}
	if event == atipicialrpc.NotaryRequestEventID && !s.chain.P2PSigExtensionsEnabled() {
		return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, "P2PSigExtensions are disabled")
	}
	// Optional filter.
	var filter atipicialrpc.SubscriptionFilter
	if p := reqParams.Value(1); p != nil {
		...
		case atipicialrpc.BlockEventID, atipicialrpc.HeaderOfAddedBlockEventID:
			flt := new(atipicialrpc.BlockFilter)
			err = jd.Decode(flt)
			filter = *flt
		case atipicialrpc.TransactionEventID:
			flt := new(atipicialrpc.TxFilter)
			err = jd.Decode(flt)
			filter = *flt
		case atipicialrpc.NotaryRequestEventID:
			flt := new(atipicialrpc.NotaryRequestFilter)
			err = jd.Decode(flt)
			filter = *flt
		case atipicialrpc.NotificationEventID:
			flt := new(atipicialrpc.NotificationFilter)
			err = jd.Decode(flt)
			filter = *flt
		case atipicialrpc.ExecutionEventID:
			flt := new(atipicialrpc.ExecutionFilter)
			err = jd.Decode(flt)
			filter = *flt
		case atipicialrpc.MempoolEventID:
			if !s.config.MempoolSubscriptionsEnabled {
				return nil, atipicialrpc.NewInternalServerError("mempool subscriptions are disabled")
			}
			flt := new(atipicialrpc.MempoolEventFilter)
			err = jd.Decode(flt)
			filter = *flt
		default:
		}
	...
	for ; id < len(sub.feeds); id++ {
		if sub.feeds[id].event == atipicialrpc.InvalidEventID {
			break
		}
	}
	if id == len(sub.feeds) {
		s.subsLock.Unlock()
		return nil, atipicialrpc.NewInternalServerError("maximum number of subscriptions is reached")
	}
	...
	case <-s.shutdown:
		s.subsCounterLock.Unlock()
		return nil, atipicialrpc.NewInternalServerError("server is shutting down")
	default:
	}
...
// taken by the caller.
func (s *Server) subscribeToChannel(event atipicialrpc.EventID) {
	switch event {
	case atipicialrpc.BlockEventID:
		if s.blockSubs == 0 {
	...
	case atipicialrpc.NotaryRequestEventID:
		...
		s.notaryRequestSubs++
	case atipicialrpc.HeaderOfAddedBlockEventID:
...
func (s *Server) unsubscribe(reqParams params.Params, sub *subscriber) (any, *atipicialrpc.Error) {
	id, err := reqParams.Value(0).GetInt()
	if err != nil || id < 0 {
		return nil, atipicialrpc.ErrInvalidParams
	}
	s.subsLock.Lock()
	if len(sub.feeds) <= id || sub.feeds[id].event == atipicialrpc.InvalidEventID {
		s.subsLock.Unlock()
		return nil, atipicialrpc.ErrInvalidParams
	}
	event := sub.feeds[id].event
	sub.feeds[id].event = atipicialrpc.InvalidEventID
...
func (s *Server) unsubscribeFromChannel(event atipicialrpc.EventID) {
	switch event {
	case atipicialrpc.BlockEventID:
		s.blockSubs--
		if s.blockSubs == 0 {
...
func (s *Server) handleSubEvents() {
	var overflowEvent = atipicialrpc.Notification{
		JSONRPC: atipicialrpc.JSONRPCVersion,
		Event:   atipicialrpc.MissedEventID,
		Payload: make([]any, 0),
	}
...
chloop:
	for {
		var resp = atipicialrpc.Notification{
			JSONRPC: atipicialrpc.JSONRPCVersion,
			Payload: make([]any, 1),
		}
...
func (s *Server) blockHeightFromParam(param *params.Param) (uint32, *atipicialrpc.Error) {
	...
	if num < 0 || int64(num) > int64(s.chain.BlockHeight()) {
		return 0, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrUnknownHeight, fmt.Sprintf("param at index %d should be greater than or equal to 0 and less than or equal to current block height, got: %d", 0, num))
	}
	return uint32(num), nil
}

func (s *Server) packResponse(r *params.In, result any, respErr *atipicialrpc.Error) abstract {
	resp := abstract{
		Header: atipicialrpc.Header{
			JSONRPC: r.JSONRPC,
			ID:      r.RawID,
...
func (s *Server) logRequestError(r *params.Request, jsonErr *atipicialrpc.Error) {
	logFields := []zap.Field{
	...
	switch jsonErr.Code {
	case atipicialrpc.InternalServerErrorCode:
...
func (s *Server) writeHTTPErrorResponse(r *params.In, w http.ResponseWriter, jsonErr *atipicialrpc.Error) {
...
func (s *Server) writeHTTPServerResponse(r *params.Request, w http.ResponseWriter, resp abstractResult) {
	...
	resp.RunForErrors(func(jsonErr *atipicialrpc.Error) {
...
// validateAddress verifies that the address is a correct Atipicial address
// see https://docs.atipicial.org/en-us/node/cli/2.9.4/api/validateaddress.html
...
func (s *Server) getRawNotaryPool(reqParams params.Params) (any, *atipicialrpc.Error) {
	if !s.chain.P2PSigExtensionsEnabled() {
		return nil, atipicialrpc.NewInternalServerError("P2PSignatureExtensions are disabled")
	}
...
func (s *Server) getRawNotaryTransaction(reqParams params.Params) (any, *atipicialrpc.Error) {
	if !s.chain.P2PSigExtensionsEnabled() {
		return nil, atipicialrpc.NewInternalServerError("P2PSignatureExtensions are disabled")
	}
...
func (s *Server) getBlockNotifications(reqParams params.Params) (any, *atipicialrpc.Error) {
	param := reqParams.Value(0)
	hash, respErr := s.blockHashFromParam(param)
	if respErr != nil {
		return nil, respErr
	...
	var filter *atipicialrpc.NotificationFilter
	if len(reqParams) > 1 {
		var (
		...
		filter = new(atipicialrpc.NotificationFilter)

		err := decoder.Decode(filter)
		if err != nil {
			return nil, atipicialrpc.WrapErrorWithData(atipicialrpc.ErrInvalidParams, fmt.Sprintf("invalid filter: %s", err))
		}
	...
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError("failed to get app exec results for onpersist")
	}
	...
	for _, txHash := range block.Transactions {
		...
		if err != nil {
			return nil, atipicialrpc.NewInternalServerError("failed to get app exec results")
		}
	...
	if err != nil {
		return nil, atipicialrpc.NewInternalServerError("failed to get app exec results for postpersist")
	}
