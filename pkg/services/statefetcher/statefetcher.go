...
// Service fetches contract storage state from AtipicialFs.
type Service struct {
...
func New(chain Ledger, cfg config.AtipicialFsStateFetcher, stateSyncInterval int, logger *zap.Logger, shutdownCallback func()) (*Service, error) {
	if !cfg.Enabled {
		return &Service{}, nil
	}
	if cfg.Timeout <= 0 {
	...
	if cfg.StateAttribute == "" {
		cfg.StateAttribute = atipicialfs.DefaultStateAttribute
	}
	if cfg.KeyValueBatchSize <= 0 {
		cfg.KeyValueBatchSize = atipicialfs.DefaultKVBatchSize
	}
	...
	if err = s.Pool.Dial(context.Background()); err != nil {
		...
		return nil, fmt.Errorf("failed to dial AtipicialFs pool: %w", err)
	}
...
// Init initializes Service to fetch the most recent state from AtipicialFs within
...
func (s *Service) Init(minH uint32, maxH uint32) (uint32, error) {
	...
	s.log.Info("initializing AtipicialFs StateFetcher service",
...
func (s *Service) findStateObject(minH uint32, maxH uint32) (oid.ID, *state.MPTRoot, error) {
	...
	results, errs := atipicialfs.ObjectSearch(ctx, s.Pool, s.Account.PrivateKey(), s.ContainerID, filters, []string{s.cfg.StateAttribute, atipicialfs.DefaultStateRootAttribute, atipicialfs.DefaultWitnessAttribute})
loop:
	for {
		select {
		case res, ok := <-results:
...
func (s *Service) Start(syncPoint uint32) error {
	...
	s.log.Info("starting AtipicialFs StateFetcher service")
	go s.exiter()
	go s.run(syncPoint)
	return nil
}
...
func (s *Service) exiter() {
	...
	force := <-s.quit
	s.log.Info("shutting down AtipicialFs StateFetcher service", zap.Bool("force", force))
...
func (s *Service) objectGet(ctx context.Context, oid oid.ID) (io.ReadCloser, error) {
	u, err := url.Parse(fmt.Sprintf("%s:%s/%s", atipicialfs.URIScheme, s.cfg.ContainerID, oid))
	if err != nil {
		return nil, err
	}
	var rc io.ReadCloser
	err = s.Retry(func() error {
		rc, err = atipicialfs.GetWithClient(ctx, s.Pool, s.Account.PrivateKey(), u, false)
		return err
	})
	return rc, err
}
