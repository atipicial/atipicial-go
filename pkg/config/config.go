...
import (
	...
	"github.com/atipicial/atipicial-go/config"
...
const (
	...
	// DefaultMaxAEP11Tokens is the default maximum number of resulting AEP11 tokens
	// that can be traversed by `getaep11balances` JSON-RPC handler.
	DefaultMaxAEP11Tokens = 100
...
func (c Config) Blockchain() Blockchain {
	return Blockchain{
		...
		AtipicialFsBlockFetcher:           c.ApplicationConfiguration.AtipicialFsBlockFetcher,
		AtipicialFsStateFetcher:           c.ApplicationConfiguration.AtipicialFsStateFetcher,
...
func getEmbeddedConfig(configPath string) ([]byte, error) {
	...
	case fmt.Sprintf("%s/protocol.%s.yml", DefaultConfigPath, netmode.MainNetAtipicialFs):
		return config.MainNetAtipicialFs, nil
	case fmt.Sprintf("%s/protocol.%s.yml", DefaultConfigPath, netmode.TestNetAtipicialFs):
		return config.TestNetAtipicialFs, nil
