...
const (
	...
	// that tells it where to save coverage data. Atipicialtest coverage can be enabled
	...
	// disableAtipicialtestCover is name of the environmental variable used to explicitly disable atipicialtest coverage.
	disableAtipicialtestCover = "DISABLE_ATCTEST_COVER"
)
...
func isCoverageEnabled(t testing.TB) bool {
	...
	if v, ok := os.LookupEnv(disableAtipicialtestCover); ok {
		disabled, err := strconv.ParseBool(v)
		if err != nil {
			t.Fatalf("coverage: error when parsing environment variable '%s', expected bool, but got '%s'", disableAtipicialtestCover, v)
		}
