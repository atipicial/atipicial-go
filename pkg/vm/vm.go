...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/vm/invocations"
...
// LoadFileWithFlags loads a program in AEF format from the given path, ready to execute it.
...
// LoadAEFMethod allows to create a context to execute a method from the AEF
// file with the specified caller and executing hash, call flags, return value,
// method and _initialize offsets.
func (v *VM) LoadAEFMethod(exe *aef.File, manifest *manifest.Manifest, caller util.Uint160, hash util.Uint160, f callflag.CallFlag,
...
func (v *VM) loadScriptWithCallingHash(b []byte, exe *aef.File, manifest *manifest.Manifest, caller util.Uint160,
...
func (v *VM) execute(ctx *Context, op opcode.Opcode, parameter []byte) (err error) {
	...
	case opcode.MODPOW:
		...
		case 1:
			...
			// https://github.com/atipicial/atipicial-go/issues/3612
	...
	case opcode.RET:
		...
		if len(v.istack) != 0 && oldEstack != newEstack {
			...
			for i := rvcount; i > 0; i-- {
				...
				// Ref. https://github.com/atipicial-project/atipicial-vm/blob/ed0eb66359fd1d76ddb5be91cc5a703ddc714a18/src/Atipicial.VM/JumpTable/JumpTable.Control.cs#L540.
	...
	case opcode.ABORTMSG:
		...
		panic(fmt.Sprintf("%s is executed. Reason: %s", op, msg))
