...
func getAEP2Account(t *testing.T, wif, pass string) *Account {
	return getAccountWithScrypt(t, wif, pass, keys.AEP2ScryptParams())
}
...
func TestRegenerateSoloWallet(t *testing.T) {
	...
	wif := privnetWIFs[0]
	acc1 := getAEP2Account(t, wif, "one")
	acc2 := getAEP2Account(t, wif, "one")
	require.NoError(t, acc2.ConvertMultisig(3, getKeys(t)))

	acc3 := getAEP2Account(t, wif, "one")
	require.NoError(t, acc3.ConvertMultisig(1, keys.PublicKeys{getKeys(t)[0]}))

	createAEP2Wallet(t, walletPath, acc1, acc2, acc3)
}

func regenerateWallets(t *testing.T, dir string) {
	pubs := getKeys(t)
	for i := range privnetWIFs {
		acc1 := getAEP2Account(t, privnetWIFs[i], passwords[i])
		acc2 := getAEP2Account(t, privnetWIFs[i], passwords[i])
		require.NoError(t, acc2.ConvertMultisig(3, pubs))

		createAEP2Wallet(t, filepath.Join(dir, fmt.Sprintf("wallet%d.json", i+1)), acc1, acc2)
	}
}
...
func TestRegenerateExamplesWallet(t *testing.T) {
	...
	acc1 := getAEP2Account(t, acc1WIF, "qwerty")
	acc1.Label = "my_account"
	createAEP2Wallet(t, walletPath, acc1)
}
...
func TestRegenerateCLITestwallet_ATIPICIAL(t *testing.T) {
	if !regenerate {
		return
	}
	const walletPath = "../../cli/wallet/testdata/testwallet_ATIPICIAL.json"

	pubs := getKeys(t)
	acc1 := getAEP2Account(t, privnetWIFs[0], passwords[0])
	acc2 := getAEP2Account(t, privnetWIFs[0], passwords[0])
	require.NoError(t, acc2.ConvertMultisig(3, pubs))
	createAEP2Wallet(t, walletPath, acc1, acc2)
}

// createAEP2Wallet creates wallet with provided accounts and AEP2 scrypt parameters.
func createAEP2Wallet(t *testing.T, path string, accs ...*Account) {
	createWallet(t, path, keys.AEP2ScryptParams(), accs...)
}
...
func TestRegenerateCLIWallet1_solo(t *testing.T) {
	...
	const (
		...
		verifyAEFPath      = "../../cli/smartcontract/testdata/verify.aef"
	...
	f, err := os.ReadFile(verifyAEFPath)
	require.NoError(t, err)
	...
	hash := state.CreateContractHash(acc3.PrivateKey().GetScriptHash(), aefFile.Checksum, m.Name)
