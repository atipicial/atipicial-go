...
// Account represents a ATC account. It holds the private and the public key
...
// Account so it's AEP-6 compliant.
...
func (a *Account) Decrypt(passphrase string, scrypt keys.ScryptParams) error {
	...
	a.privateKey, err = keys.AEP2Decrypt(a.EncryptedWIF, passphrase, scrypt)
	if err != nil {
		return err
	}
...
func (a *Account) Encrypt(passphrase string, scrypt keys.ScryptParams) error {
	wif, err := keys.AEP2Encrypt(a.privateKey, passphrase, scrypt)
	if err != nil {
		return err
	}
	a.EncryptedWIF = wif
...
func NewAccountFromEncryptedWIF(wif string, pass string, scrypt keys.ScryptParams) (*Account, error) {
	priv, err := keys.AEP2Decrypt(wif, pass, scrypt)
	if err != nil {
		return nil, err
	}
