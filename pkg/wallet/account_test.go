...
func TestDecryptAccount(t *testing.T) {
	for _, testCase := range keytestcases.Arr {
		...
		err := acc.Decrypt(testCase.Passphrase, keys.AEP2ScryptParams())
		if testCase.Invalid {
			assert.Error(t, err)
			continue
		}
	...
	// No encrypted key.
	acc := &Account{}
	require.Error(t, acc.Decrypt("qwerty", keys.AEP2ScryptParams()))
}
...
func TestNewAccountFromEncryptedWIF(t *testing.T) {
	for _, tc := range keytestcases.Arr {
		acc, err := NewAccountFromEncryptedWIF(tc.EncryptedWif, tc.Passphrase, keys.AEP2ScryptParams())
		if tc.Invalid {
			assert.Error(t, err)
			continue
		}
...
func TestAccount_ConvertMultisig(t *testing.T) {
	// test is based on a wallet1_solo.json accounts from atipicial-local
