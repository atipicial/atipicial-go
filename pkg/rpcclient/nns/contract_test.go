...
func TestSimpleGetters(t *testing.T) {
	...
	require.Error(t, err)
	_, err = nns.IsAvailable("atipicial.atipicial")
	require.Error(t, err)
	_, err = nns.Resolve("atipicial.atipicial", A)
	require.Error(t, err)
	_, err = nns.GetRecord("atipicial.atipicial", A)
	require.Error(t, err)
	...
	ava, err := nns.IsAvailable("atipicial.atipicial")
	require.NoError(t, err)
	...
	txt, err := nns.Resolve("atipicial.atipicial", TXT)
	require.NoError(t, err)
	require.Equal(t, "some text", txt)

	rec, err := nns.GetRecord("atipicial.atipicial", TXT)
	require.NoError(t, err)
...
func TestGetAllRecords(t *testing.T) {
	...
	ta.err = errors.New("")
	_, err := nns.GetAllRecords("atipicial.atipicial")
	require.Error(t, err)
	...
	iter, err := nns.GetAllRecords("atipicial.atipicial")
	require.NoError(t, err)
...
func TestGetAllRecordsExpanded(t *testing.T) {
	...
	ta.err = errors.New("")
	_, err := nns.GetAllRecordsExpanded("atipicial.atipicial", 8)
	require.Error(t, err)
	...
	vals, err := nns.GetAllRecordsExpanded("atipicial.atipicial", 8)
	require.NoError(t, err)
	require.Equal(t, 1, len(vals))
	require.Equal(t, RecordState{
		Name: "n3",
...
func TestUpdate(t *testing.T) {
	...
	aef := []byte{0x01, 0x02, 0x03}
	manifest := "manifest data"

	ta.err = errors.New("test error")
	_, _, err := nns.Update(aef, manifest)
	require.Error(t, err)

	// Test successful update
	ta.err = nil
	ta.txh = util.Uint256{0x04, 0x05, 0x06}
	txh, vub, err := nns.Update(aef, manifest)
	require.NoError(t, err)
	require.Equal(t, ta.txh, txh)
	require.Equal(t, ta.vub, vub)

	for _, fun := range []func(aef []byte, manifest string) (*transaction.Transaction, error){
