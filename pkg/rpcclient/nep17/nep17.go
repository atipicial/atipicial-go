/*
Package aep17 contains RPC wrappers to work with AEP-17 contracts.

Safe methods are encapsulated into TokenReader structure while Token provides
various methods to perform the only AEP-17 state-changing call, Transfer.
*/
package aep17
...
// TokenReader represents safe (read-only) methods of AEP-17 token. It can be
// used to query various data.
type TokenReader struct {
	aeptoken.Base
}

// TokenWriter contains AEP-17 token methods that change state. It's not meant
...
// for other contracts that extend AEP-17 interface.
type TokenWriter struct {
	hash  util.Uint160
	actor Actor
}

// Token provides full AEP-17 interface, both safe and state-changing methods.
type Token struct {
	TokenReader
	TokenWriter
}

// TransferEvent represents a Transfer event as defined in the AEP-17 standard.
...
func NewReader(invoker Invoker, hash util.Uint160) *TokenReader {
	return &TokenReader{*aeptoken.New(invoker, hash)}
}
...
// MultiTransfer is not a real AEP-17 method, but rather a convenient way to
