...
// This method is only supported by AtipicialGo servers.
...
// GetCommittee returns the current public keys of ATC nodes in the committee.
...
// non-native ones can be requested only from AtipicialGo servers.
...
// GetAEP11Balances is a wrapper for getaep11balances RPC.
func (c *Client) GetAEP11Balances(address util.Uint160) (*result.AEP11Balances, error) {
	params := []any{address.StringLE()}
	resp := new(result.AEP11Balances)
	if err := c.performRequest("getaep11balances", params, resp); err != nil {
		return nil, err
	}
	return resp, nil
}

// GetAEP17Balances is a wrapper for getaep17balances RPC.
func (c *Client) GetAEP17Balances(address util.Uint160) (*result.AEP17Balances, error) {
	params := []any{address.StringLE()}
	resp := new(result.AEP17Balances)
	if err := c.performRequest("getaep17balances", params, resp); err != nil {
		return nil, err
	}
	return resp, nil
}

// GetAEP11Properties is a wrapper for getaep11properties RPC. We recommend using
// aep11 package and Properties method there to receive proper VM types and work with them.
...
func (c *Client) GetAEP11Properties(asset util.Uint160, token []byte) (map[string]any, error) {
	params := []any{asset.StringLE(), hex.EncodeToString(token)}
	resp := make(map[string]any)
	if err := c.performRequest("getaep11properties", params, &resp); err != nil {
		return nil, err
	}
	for k, v := range resp {
		if v == nil {
		...
		if result.KnownAEP11Properties[k] {
			continue
		}
...
// GetAEP11Transfers is a wrapper for getaep11transfers RPC. Address parameter
...
// parameters are only supported by AtipicialGo servers and can only be specified
// with start and stop (since they're positional in the protocol).
func (c *Client) GetAEP11Transfers(address util.Uint160, start, stop *uint64, limit, page *int) (*result.AEP11Transfers, error) {
	params, err := packTransfersParams(address, start, stop, limit, page)
	if err != nil {
		return nil, err
	}
	resp := new(result.AEP11Transfers)
	if err := c.performRequest("getaep11transfers", params, resp); err != nil {
		return nil, err
	}
	return resp, nil
}
...
// GetAEP17Transfers is a wrapper for getaep17transfers RPC. Address parameter
...
// parameters are only supported by AtipicialGo servers and can only be specified
// with start and stop (since they're positional in the protocol).
func (c *Client) GetAEP17Transfers(address util.Uint160, start, stop *uint64, limit, page *int) (*result.AEP17Transfers, error) {
	params, err := packTransfersParams(address, start, stop, limit, page)
	if err != nil {
		return nil, err
	}
	resp := new(result.AEP17Transfers)
	if err := c.performRequest("getaep17transfers", params, resp); err != nil {
		return nil, err
	}
	return resp, nil
}
...
// returned (supported by AtipicialGo nodes only).
...
// GetCandidates returns the current list of ATC candidate node with voting data and
// validator status.
...
// GetNextBlockValidators returns the current ATC consensus nodes information and voting data.
...
func (c *Client) invokeSomething(method string, p []any, signers []transaction.Signer, witnesses ...transaction.Witness) (*result.Invoke, error) {
	...
	if signers != nil {
		if witnesses == nil {
			...
			signersWithWitnesses := make([]atipicialrpc.SignerWithWitness, len(signers))
			for i := range signersWithWitnesses {
				signersWithWitnesses[i] = atipicialrpc.SignerWithWitness{
					Signer:  signers[i],
					Witness: witnesses[i],
				}
			}
...
// SendRawTransaction broadcasts the given transaction to the Atipicial network.
...
// SubmitBlock broadcasts a raw block over the ATC network.
...
// ValidateAddress verifies that the address is a correct ATC address.
...
// transaction) will be returned (supported by AtipicialGo nodes only).
...
func (c *Client) GetBlockNotifications(blockHash util.Uint256, filter *atipicialrpc.NotificationFilter) (*result.BlockNotifications, error) {
	var (
