...
// for Notarize, SendRequest or SendRequestExactly. Notice that [atipicialrpc.ErrAlreadyExists]
...
// need some additional event or stack checks. [atipicialrpc.FaultException] error is
...
func (a *Actor) WaitSuccess(mainHash, fbHash util.Uint256, vub uint32, err error) (*state.AppExecResult, error) {
	...
	if aer.VMState != vmstate.Halt {
		return nil, &atipicialrpc.FaultException{Message: aer.FaultException}
	}
	return aer, nil
}
