...
type (
	...
	RPCEventBased interface {
		...
		ReceiveHeadersOfAddedBlocks(flt *atipicialrpc.BlockFilter, rcvr chan<- *block.Header) (string, error)
		ReceiveExecutions(flt *atipicialrpc.ExecutionFilter, rcvr chan<- *state.AppExecResult) (string, error)
...
func (w *PollingBased) Wait(ctx context.Context, h util.Uint256, vub uint32, err error) (*state.AppExecResult, error) {
	if err != nil && !errors.Is(err, atipicialrpc.ErrAlreadyExists) {
		return nil, err
	}
...
func (w *EventBased) WaitAny(ctx context.Context, vub uint32, hashes ...util.Uint256) (res *state.AppExecResult, waitErr error) {
	...
	blocksID, err := w.ws.ReceiveHeadersOfAddedBlocks(&atipicialrpc.BlockFilter{Since: new(vub)}, hRcvr)
	if err != nil {
	...
	if wsWaitErr == nil {
		...
		for _, h := range hashes {
			txsID, err := w.ws.ReceiveExecutions(&atipicialrpc.ExecutionFilter{Container: &h}, aerRcvr)
			if err != nil {
