/*
Package atipicial provides an RPC-based wrapper for the ATCToken contract.
...
// ContractReader represents safe (read-only) methods of ATC. It can be
...
// Contract provides full ATC interface, both safe and state-changing methods.
...
// CandidateStateEvent represents a CandidateStateChanged ATC event.
...
// CommitteeChangedEvent represents a CommitteeChanged ATC event.
type CommitteeChangedEvent struct {
	Old []keys.PublicKey
	New []keys.PublicKey
}

// VoteEvent represents a Vote ATC event.
...
// Hash stores the hash of the native ATCToken contract.
var Hash = nativehashes.AtipicialCoin

// NewReader creates an instance of ContractReader to get data from the ATC
// contract.
func NewReader(invoker Invoker) *ContractReader {
	return &ContractReader{*aep17.NewReader(invoker, Hash), invoker}
}

// New creates an instance of Contract to perform state-changing actions in the
// ATC contract.
func New(actor Actor) *Contract {
	aep := aep17.New(actor, Hash)
	return &Contract{ContractReader{aep.TokenReader, actor}, aep.TokenWriter, actor}
}

// GetAccountState returns current ATC balance state for the account which
// includes balance and voting data. It can return nil balance with no error
// if the account given has no ATC.
func (c *ContractReader) GetAccountState(account util.Uint160) (*state.ATCBalance, error) {
...
// GetAllCandidatesExpanded is similar to GetAllCandidates (uses the same ATC
...
// any ATC state change ("claim") is to happen for the given account at the given
...
func regScript(unreg bool, k *keys.PublicKey) []byte {
	...
	// We know parameters exactly (unlike with aep17.Transfer), so this can't fail.
