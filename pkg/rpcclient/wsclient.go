...
// See https://github.com/atipicial/atipicial-go/blob/master/docs/notifications.md#ordering-and-persistence-guarantees
...
type WSClient struct {
	...
	requests    chan *atipicialrpc.Request
	...
	respChannels map[uint64]chan *atipicialrpc.Response
}
...
type blockReceiver struct {
	filter *atipicialrpc.BlockFilter
	ch     chan<- *block.Block
}

// EventID implements atipicialrpc.Comparator interface.
func (r *blockReceiver) EventID() atipicialrpc.EventID {
	return atipicialrpc.BlockEventID
}

// Filter implements atipicialrpc.Comparator interface.
func (r *blockReceiver) Filter() atipicialrpc.SubscriptionFilter {
	if r.filter == nil {
		return nil
	}
	return *r.filter
...
// EventID implements atipicialrpc.Comparator interface.
func (r *headerOfAddedBlockReceiver) EventID() atipicialrpc.EventID {
	return atipicialrpc.HeaderOfAddedBlockEventID
}

// Filter implements atipicialrpc.Comparator interface.
func (r *headerOfAddedBlockReceiver) Filter() atipicialrpc.SubscriptionFilter {
	if r.filter == nil {
		return nil
	}
	return *r.filter
...
type txReceiver struct {
	filter *atipicialrpc.TxFilter
	ch     chan<- *transaction.Transaction
}

// EventID implements atipicialrpc.Comparator interface.
func (r *txReceiver) EventID() atipicialrpc.EventID {
	return atipicialrpc.TransactionEventID
}

// Filter implements atipicialrpc.Comparator interface.
func (r *txReceiver) Filter() atipicialrpc.SubscriptionFilter {
	if r.filter == nil {
		return nil
	}
	return *r.filter
...
type executionNotificationReceiver struct {
	filter *atipicialrpc.NotificationFilter
	ch     chan<- *state.ContainedNotificationEvent
}

// EventID implements atipicialrpc.Comparator interface.
func (r *executionNotificationReceiver) EventID() atipicialrpc.EventID {
	return atipicialrpc.NotificationEventID
}

// Filter implements atipicialrpc.Comparator interface.
func (r *executionNotificationReceiver) Filter() atipicialrpc.SubscriptionFilter {
	if r.filter == nil {
		return nil
	}
	return *r.filter
...
type executionReceiver struct {
	filter *atipicialrpc.ExecutionFilter
	ch     chan<- *state.AppExecResult
}

// EventID implements atipicialrpc.Comparator interface.
func (r *executionReceiver) EventID() atipicialrpc.EventID {
	return atipicialrpc.ExecutionEventID
}

// Filter implements atipicialrpc.Comparator interface.
func (r *executionReceiver) Filter() atipicialrpc.SubscriptionFilter {
	if r.filter == nil {
		return nil
	}
	return *r.filter
...
type notaryRequestReceiver struct {
	filter *atipicialrpc.NotaryRequestFilter
	ch     chan<- *result.NotaryRequestEvent
}

// EventID implements atipicialrpc.Comparator interface.
func (r *notaryRequestReceiver) EventID() atipicialrpc.EventID {
	return atipicialrpc.NotaryRequestEventID
}

// Filter implements atipicialrpc.Comparator interface.
func (r *notaryRequestReceiver) Filter() atipicialrpc.SubscriptionFilter {
	if r.filter == nil {
		return nil
	}
	return *r.filter
...
type mempoolEventReceiver struct {
	filter *atipicialrpc.MempoolEventFilter
	ch     chan<- *result.MempoolEvent
}

// EventID implements atipicialrpc.Comparator interface.
func (r *mempoolEventReceiver) EventID() atipicialrpc.EventID {
	return atipicialrpc.MempoolEventID
}

// Filter implements atipicialrpc.Comparator interface.
func (r *mempoolEventReceiver) Filter() atipicialrpc.SubscriptionFilter {
	if r.filter == nil {
		return nil
	}
	return *r.filter
...
type Notification struct {
	Type  atipicialrpc.EventID
	Value any
}

// EventID implements Container interface and returns notification ID.
func (n Notification) EventID() atipicialrpc.EventID {
	return n.Type
}
...
func NewWS(ctx context.Context, endpoint string, opts WSOptions) (*WSClient, error) {
	...
	if err != nil {
		if resp != nil && resp.Body != nil {
			var srvErr atipicialrpc.HeaderAndError
	...
	wsc := &WSClient{
		...
		respChannels:  make(map[uint64]chan *atipicialrpc.Response),
		requests:      make(chan *atipicialrpc.Request),
...
readloop:
	for {
		...
		if rr.ID == nil && rr.Method != "" {
			event, err := atipicialrpc.GetEventIDFromString(rr.Method)
			if err != nil {
				// Bad event received.
				connCloseErr = fmt.Errorf("failed to perse event ID from string %s: %w", rr.Method, err)
				break readloop
			}
			if event != atipicialrpc.MissedEventID && len(rr.RawParams) != 1 {
				// Bad event received.
			...
			case atipicialrpc.MissedEventID:
				// No value.
			default:
				// Bad event received.
			...
			if event != atipicialrpc.MissedEventID {
...
func (c *WSClient) notifySubscribers(ntf Notification) {
	if ntf.Type == atipicialrpc.MissedEventID {
		c.subscriptionsLock.Lock()
...
func (c *WSClient) getResponseChannel(id uint64) chan *atipicialrpc.Response {
	c.respLock.RLock()
...
func (c *WSClient) makeWsRequest(r *atipicialrpc.Request) (*atipicialrpc.Response, error) {
	ch := make(chan *atipicialrpc.Response)
	c.respLock.Lock()
	select {
	case <-c.readerDone:
		c.respLock.Unlock()
...
// Events can be filtered by the given [atipicialrpc.BlockFilter], nil value doesn't add any filter.
// See [WSClient] comments for generic Receive* behaviour details.
func (c *WSClient) ReceiveBlocks(flt *atipicialrpc.BlockFilter, rcvr chan<- *block.Block) (string, error) {
	if rcvr == nil {
...
// block's header events. Events can be filtered by the given [atipicialrpc.BlockFilter],
// nil value doesn't add any filter. See [WSClient] comments for generic
// Receive* behaviour details.
func (c *WSClient) ReceiveHeadersOfAddedBlocks(flt *atipicialrpc.BlockFilter, rcvr chan<- *block.Header) (string, error) {
	if rcvr == nil {
		return "", ErrNilNotificationReceiver
	}
	if !c.cache.initDone {
...
// events. Events can be filtered by the given [atipicialrpc.TxFilter], nil value doesn't add any
// filter. See [WSClient] comments for generic Receive* behaviour details.
func (c *WSClient) ReceiveTransactions(flt *atipicialrpc.TxFilter, rcvr chan<- *transaction.Transaction) (string, error) {
	if rcvr == nil {
		return "", ErrNilNotificationReceiver
	}
...
// events. Events can be filtered by the given [atipicialrpc.NotificationFilter], nil value doesn't add
// any filter. See [WSClient] comments for generic Receive* behaviour details.
func (c *WSClient) ReceiveExecutionNotifications(flt *atipicialrpc.NotificationFilter, rcvr chan<- *state.ContainedNotificationEvent) (string, error) {
	if rcvr == nil {
		return "", ErrNilNotificationReceiver
	}
...
// Events can be filtered by the given [atipicialrpc.ExecutionFilter], nil value doesn't add any filter.
// See [WSClient] comments for generic Receive* behaviour details.
func (c *WSClient) ReceiveExecutions(flt *atipicialrpc.ExecutionFilter, rcvr chan<- *state.AppExecResult) (string, error) {
	if rcvr == nil {
		return "", ErrNilNotificationReceiver
	}
...
// payload addition or removal events. Events can be filtered by the given [atipicialrpc.NotaryRequestFilter]
...
func (c *WSClient) ReceiveNotaryRequests(flt *atipicialrpc.NotaryRequestFilter, rcvr chan<- *result.NotaryRequestEvent) (string, error) {
	if rcvr == nil {
		return "", ErrNilNotificationReceiver
	}
...
// [atipicialrpc.MempoolEventFilter] where sender corresponds to the transaction’s sender, signer
...
func (c *WSClient) ReceiveMempoolEvents(flt *atipicialrpc.MempoolEventFilter, rcvr chan<- *result.MempoolEvent) (string, error) {
	if rcvr == nil {
		return "", ErrNilNotificationReceiver
	}
