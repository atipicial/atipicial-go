...
type InternalHook func(context.Context, chan<- atipicialrpc.Notification) func(*atipicialrpc.Request) (*atipicialrpc.Response, error)

// Internal is an experimental "local" client that does not connect to RPC via
// network. It's made for deeply integrated applications like AtipicialFs that have
...
type Internal struct {
	...
	events chan atipicialrpc.Notification
}
...
func NewInternal(ctx context.Context, register InternalHook) (*Internal, error) {
	c := &Internal{
		...
		events: make(chan atipicialrpc.Notification),
	}
