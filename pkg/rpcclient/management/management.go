...
func (c *Contract) Deploy(exe *aef.File, manif *manifest.Manifest, data any) (util.Uint256, uint32, error) {
...
// the ContractManagement contract. If successful, this method returns deployed
// contract state that can be retrieved from the stack after execution.
func (c *Contract) DeployTransaction(exe *aef.File, manif *manifest.Manifest, data any) (*transaction.Transaction, error) {
	script, err := mkDeployScript(exe, manif, data)
	if err != nil {
		return nil, err
	}
...
// the ContractManagement contract. If successful, this method returns deployed
// contract state that can be retrieved from the stack after execution.
func (c *Contract) DeployUnsigned(exe *aef.File, manif *manifest.Manifest, data any) (*transaction.Transaction, error) {
	script, err := mkDeployScript(exe, manif, data)
	if err != nil {
		return nil, err
	}
...
func mkDeployScript(exe *aef.File, manif *manifest.Manifest, data any) ([]byte, error) {
	exeB, err := exe.Bytes()
	if err != nil {
		return nil, fmt.Errorf("bad AEF: %w", err)
	}
