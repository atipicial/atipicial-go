...
func TestReader(t *testing.T) {
	...
	aefFile, _ := aef.NewFile([]byte{1, 2, 3})
	aefBytes, _ := aefFile.Bytes()
...
func TestDeploy(t *testing.T) {
	...
	ta.err = errors.New("")
	_, _, err := man.Deploy(aefFile, manif, nil)
	require.Error(t, err)

	for _, m := range []func(exe *aef.File, manif *manifest.Manifest, data any) (*transaction.Transaction, error){
		man.DeployTransaction,
		man.DeployUnsigned,
	} {
		_, err = m(aefFile, manif, nil)
		require.Error(t, err)
	}
	...
	h, vub, err := man.Deploy(aefFile, manif, nil)
	require.NoError(t, err)
	require.Equal(t, ta.txh, h)
	require.Equal(t, ta.vub, vub)
	...
	for _, m := range []func(exe *aef.File, manif *manifest.Manifest, data any) (*transaction.Transaction, error){
		...
		tx, err := m(aefFile, manif, nil)
		require.NoError(t, err)
		require.Equal(t, ta.tx, tx)

		_, err = m(aefFile, manif, map[int]int{})
		require.Error(t, err)
	}

	_, _, err = man.Deploy(aefFile, manif, map[int]int{})
	require.Error(t, err)

	_, _, err = man.Deploy(aefFile, manif, 100500)
	require.NoError(t, err)

	aefFile.Compiler = "intentionally very long compiler string that will make AEF code explode on encoding"
	_, _, err = man.Deploy(aefFile, manif, nil)
	require.Error(t, err)
