...
func TestWSClientEvents(t *testing.T) {
	...
	wsc.subscriptions["5"] = &executionReceiver{filter: &atipicialrpc.ExecutionFilter{State: new("HALT")}, ch: aerCh2}
	wsc.receivers[chan<- *state.AppExecResult(aerCh2)] = []string{"5"}
	wsc.subscriptions["6"] = &executionReceiver{filter: &atipicialrpc.ExecutionFilter{State: new("FAULT")}, ch: aerCh3}
	...
	wsc.subscriptions["8"] = &mempoolEventReceiver{filter: &atipicialrpc.MempoolEventFilter{Type: new(mempoolevent.TransactionAdded)}, ch: memAddedCh}
	wsc.receivers[chan<- *result.MempoolEvent(memAddedCh)] = []string{"8"}
	wsc.subscriptions["9"] = &mempoolEventReceiver{filter: &atipicialrpc.MempoolEventFilter{Type: new(mempoolevent.TransactionRemoved)}, ch: memRemovedCh}
...
func TestWSExecutionVMStateCheck(t *testing.T) {
	...
	wsc.getNextRequestID = getTestRequestID
	require.NoError(t, wsc.Init())
	_, err = wsc.ReceiveExecutions(&atipicialrpc.ExecutionFilter{State: new("NONE")}, make(chan *state.AppExecResult))
	require.ErrorIs(t, err, atipicialrpc.ErrInvalidSubscriptionFilter)
	wsc.Close()
}
...
func TestWSExecutionNotificationNameCheck(t *testing.T) {
	...
	wsc.getNextRequestID = getTestRequestID
	require.NoError(t, wsc.Init())
	_, err = wsc.ReceiveExecutionNotifications(&atipicialrpc.NotificationFilter{Name: new("notification_from_execution_with_long_name")}, make(chan *state.ContainedNotificationEvent))
	require.ErrorIs(t, err, atipicialrpc.ErrInvalidSubscriptionFilter)
	wsc.Close()
}
	...
	var cases = []struct {
		...
		{"block header primary",
			func(t *testing.T, wsc *WSClient) {
				_, err := wsc.ReceiveHeadersOfAddedBlocks(&atipicialrpc.BlockFilter{Primary: new(byte(3))}, make(chan *block.Header))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
				filt := new(atipicialrpc.BlockFilter)
		...
		{"header since",
			func(t *testing.T, wsc *WSClient) {
				var since uint32 = 3
				_, err := wsc.ReceiveHeadersOfAddedBlocks(&atipicialrpc.BlockFilter{Since: &since}, make(chan *block.Header))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"header till",
			func(t *testing.T, wsc *WSClient) {
				var till uint32 = 3
				_, err := wsc.ReceiveHeadersOfAddedBlocks(&atipicialrpc.BlockFilter{Till: &till}, make(chan *block.Header))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"header primary, since and till",
			func(t *testing.T, wsc *WSClient) {
				...
				_, err := wsc.ReceiveHeadersOfAddedBlocks(&atipicialrpc.BlockFilter{
					Primary: new(byte(2)),
					Since:   &since,
					Till:    &till,
		...
		{"blocks primary",
			func(t *testing.T, wsc *WSClient) {
				_, err := wsc.ReceiveBlocks(&atipicialrpc.BlockFilter{Primary: new(byte(3))}, make(chan *block.Block))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"blocks since",
			func(t *testing.T, wsc *WSClient) {
				var since uint32 = 3
				_, err := wsc.ReceiveBlocks(&atipicialrpc.BlockFilter{Since: &since}, make(chan *block.Block))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"blocks till",
			func(t *testing.T, wsc *WSClient) {
				var till uint32 = 3
				_, err := wsc.ReceiveBlocks(&atipicialrpc.BlockFilter{Till: &till}, make(chan *block.Block))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"blocks primary, since and till",
			func(t *testing.T, wsc *WSClient) {
				...
				_, err := wsc.ReceiveBlocks(&atipicialrpc.BlockFilter{
					Primary: new(byte(2)),
					Since:   &since,
					Till:    &till,
		...
		{"transactions sender",
			func(t *testing.T, wsc *WSClient) {
				...
				_, err := wsc.ReceiveTransactions(&atipicialrpc.TxFilter{Sender: &sender}, make(chan *transaction.Transaction))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
				filt := new(atipicialrpc.TxFilter)
				require.NoError(t, json.Unmarshal(param.RawMessage, filt))
		...
		{"transactions signer",
			func(t *testing.T, wsc *WSClient) {
				...
				_, err := wsc.ReceiveTransactions(&atipicialrpc.TxFilter{Signer: &signer}, make(chan *transaction.Transaction))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"transactions sender and signer",
			func(t *testing.T, wsc *WSClient) {
				sender := util.Uint160{1, 2, 3, 4, 5}
				signer := util.Uint160{0, 42}
				_, err := wsc.ReceiveTransactions(&atipicialrpc.TxFilter{Sender: &sender, Signer: &signer}, make(chan *transaction.Transaction))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"notifications contract hash",
			func(t *testing.T, wsc *WSClient) {
				...
				_, err := wsc.ReceiveExecutionNotifications(&atipicialrpc.NotificationFilter{Contract: &contract}, make(chan *state.ContainedNotificationEvent))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
				filt := new(atipicialrpc.NotificationFilter)
				require.NoError(t, json.Unmarshal(param.RawMessage, filt))
		...
		{"notifications name",
			func(t *testing.T, wsc *WSClient) {
				_, err := wsc.ReceiveExecutionNotifications(&atipicialrpc.NotificationFilter{Name: new("my_pretty_notification")}, make(chan *state.ContainedNotificationEvent))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"notifications contract hash and name",
			func(t *testing.T, wsc *WSClient) {
				contract := util.Uint160{1, 2, 3, 4, 5}
				_, err := wsc.ReceiveExecutionNotifications(&atipicialrpc.NotificationFilter{Contract: &contract, Name: new("my_pretty_notification")}, make(chan *state.ContainedNotificationEvent))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"notifications parameters",
			func(t *testing.T, wsc *WSClient) {
				...
				require.NoError(t, err)
				_, err = wsc.ReceiveExecutionNotifications(&atipicialrpc.NotificationFilter{Contract: &contract, Name: new("my_pretty_notification"), Parameters: prms}, make(chan *state.ContainedNotificationEvent))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"executions state",
			func(t *testing.T, wsc *WSClient) {
				_, err := wsc.ReceiveExecutions(&atipicialrpc.ExecutionFilter{State: new("FAULT")}, make(chan *state.AppExecResult))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
				filt := new(atipicialrpc.ExecutionFilter)
				require.NoError(t, json.Unmarshal(param.RawMessage, filt))
		...
		{"executions container",
			func(t *testing.T, wsc *WSClient) {
				...
				_, err := wsc.ReceiveExecutions(&atipicialrpc.ExecutionFilter{Container: &container}, make(chan *state.AppExecResult))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"executions state and container",
			func(t *testing.T, wsc *WSClient) {
				container := util.Uint256{1, 2, 3}
				_, err := wsc.ReceiveExecutions(&atipicialrpc.ExecutionFilter{State: new("FAULT"), Container: &container}, make(chan *state.AppExecResult))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		},
		{
			...
			func(t *testing.T, wsc *WSClient) {
				sender := util.Uint160{1, 2, 3, 4, 5}
				_, err := wsc.ReceiveNotaryRequests(&atipicialrpc.NotaryRequestFilter{Sender: &sender}, make(chan *result.NotaryRequestEvent))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
				filt := new(atipicialrpc.NotaryRequestFilter)
				require.NoError(t, json.Unmarshal(param.RawMessage, filt))
				require.Equal(t, util.Uint160{1, 2, 3, 4, 5}, *filt.Sender)
				require.Nil(t, filt.Signer)
		...
		},
		{
			...
			func(t *testing.T, wsc *WSClient) {
				signer := util.Uint160{0, 42}
				_, err := wsc.ReceiveNotaryRequests(&atipicialrpc.NotaryRequestFilter{Signer: &signer}, make(chan *result.NotaryRequestEvent))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		},
		{
			"notary request type",
			func(t *testing.T, wsc *WSClient) {
				_, err := wsc.ReceiveNotaryRequests(&atipicialrpc.NotaryRequestFilter{Type: new(mempoolevent.TransactionAdded)}, make(chan *result.NotaryRequestEvent))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		{"notary request sender, signer and type",
			func(t *testing.T, wsc *WSClient) {
				sender := util.Uint160{1, 2, 3, 4, 5}
				signer := util.Uint160{0, 42}
				_, err := wsc.ReceiveNotaryRequests(&atipicialrpc.NotaryRequestFilter{Type: new(mempoolevent.TransactionAdded), Signer: &signer, Sender: &sender}, make(chan *result.NotaryRequestEvent))
				require.NoError(t, err)
			},
			func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		},
		{
			...
			clientCode: func(t *testing.T, wsc *WSClient) {
				_, err := wsc.ReceiveMempoolEvents(&atipicialrpc.MempoolEventFilter{Type: new(mempoolevent.TransactionAdded)}, make(chan *result.MempoolEvent))
				require.NoError(t, err)
			},
			serverCode: func(t *testing.T, p *params.Params) {
				param := p.Value(1)
				filt := new(atipicialrpc.MempoolEventFilter)
				require.NoError(t, json.Unmarshal(param.RawMessage, filt))
		...
		},
		{
			...
			clientCode: func(t *testing.T, wsc *WSClient) {
				sender := util.Uint160{1, 2, 3, 4, 5}
				_, err := wsc.ReceiveMempoolEvents(&atipicialrpc.MempoolEventFilter{Sender: &sender}, make(chan *result.MempoolEvent))
				require.NoError(t, err)
			},
			serverCode: func(t *testing.T, p *params.Params) {
				param := p.Value(1)
		...
		},
		{
			...
			clientCode: func(t *testing.T, wsc *WSClient) {
				signer := util.Uint160{0, 42}
				_, err := wsc.ReceiveMempoolEvents(&atipicialrpc.MempoolEventFilter{Signer: &signer}, make(chan *result.MempoolEvent))
				require.NoError(t, err)
			},
			serverCode: func(t *testing.T, p *params.Params) {
				param := p.Value(1)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		if req.URL.Path == "/ws" && req.Method == "GET" {
			...
			for {
				...
				case "getversion":
					response = fmt.Sprintf(`{"id":%s,"jsonrpc":"2.0","result":{"network":42,"tcpport":20332,"wsport":20342,"nonce":2153672787,"useragent":"/ATIPICIAL-GO:0.73.1-pre-273-ge381358/"}}`, r.RawID)
				case "getblockhash":
