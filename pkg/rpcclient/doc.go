/*
Package rpcclient implements ATC-specific JSON-RPC 2.0 client.

This package itself is designed to be a thin layer on top of the regular JSON-RPC
interface provided by Atipicial implementations. Therefore the set of methods provided
...
These layers are:
  ...
  - Standard-specific wrappers that are implemented in aep11 and aep17 packages
    (with common methods in aeptoken). They implement the respective AEP-11 and
    AEP-17 APIs both for safe (read-only) and state-changing methods. Safe methods
  ...
  - Contract-specific wrappers for native contracts that include management, gas,
    atipicial, oracle, policy and rolemgmt packages for the respective native contracts.
    Complete contract functionality is exposed (reusing aep17 package for gas and
    atipicial).

  - Notary actor and contract, a bit special since it's a AtipicialGo protocol
...
you can interact with the ATC blockchain by its exposed methods.
