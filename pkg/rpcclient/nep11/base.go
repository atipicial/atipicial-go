/*
Package aep11 contains RPC wrappers for AEP-11 contracts.

The set of types provided is split between common AEP-11 methods (BaseReader and
Base types) and divisible (DivisibleReader and Divisible) and non-divisible
(NonDivisibleReader and NonDivisible). If you don't know the type of AEP-11
...
// Actor is used by complete AEP-11 types to create and send transactions.
...
// BaseReader is a reader interface for common divisible and non-divisible AEP-11
...
// non-divisible AEP-11 methods. It simplifies reusing this set of methods,
...
// Base is a state-changing interface for common divisible and non-divisible AEP-11
// methods.
type Base struct {
	BaseReader
	BaseWriter
...
// TransferEvent represents a Transfer event as defined in the AEP-11 standard.
...
func NewBaseReader(invoker Invoker, hash util.Uint160) *BaseReader {
	return &BaseReader{*aeptoken.New(invoker, hash), invoker, hash}
}
...
// they return well-defined properties outlined in AEP-11 and
// UnwrapKnownProperties can be used to get them in more convenient way. It's an
// optional method per AEP-11 specification, so it can fail.
...
// per AEP-11 specification, so it can fail.
...
// TokensExpanded uses the same AEP-11 method as Tokens, but can be useful if
...
// TokensOfExpanded uses the same AEP-11 method as TokensOf, but can be useful if
// the server used doesn't support sessions and doesn't expand iterators. It
// creates a script that will get num of result items from the iterator right in
// the VM and return them to you. It's only limited by VM stack and GAS available
// for RPC invocations.
...
// AEP-11 properties (name/description/image/tokenURI) defined in the standard.
...
func UnwrapKnownProperties(m *stackitem.Map, err error) (map[string]string, error) {
	...
	for _, e := range elems {
		...
		ks := string(k)
		if !result.KnownAEP11Properties[ks] { // Some additional elements are OK.
			continue
		}
