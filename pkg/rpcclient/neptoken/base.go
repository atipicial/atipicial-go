/*
Package aeptoken contains RPC wrapper for common AEP-11 and AEP-17 methods.
...
const (
	...
	// is not imposed by AEP-17 or AEP-11, but we do it as a sanity check
...
// Base is a reader interface for common AEP-11 and AEP-17 methods built
// on top of Invoker.
type Base struct {
	invoker Invoker
	hash    util.Uint160
...
// Decimals implements `decimals` AEP-17 or AEP-11 method and returns the number
// of decimals used by token. For non-divisible AEP-11 tokens this method always
...
// Symbol implements `symbol` AEP-17 or AEP-11 method and returns a short token
// identifier (like "ATC" or "GAS").
...
// BalanceOf returns the token balance of the given account. For AEP-17 that's
// the token balance with decimals (1 TOK with 2 decimals will lead to 100
// returned from this method). For non-divisible AEP-11 that's the number of
// NFTs owned by the account, for divisible AEP-11 that's the sum of the parts
