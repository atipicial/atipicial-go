...
// emitStoreSelectorExpr stores RHS into a selector expression (x.f).
// Assumes the RHS is already on the stack before this call.
...
// Assumes the RHS is already on the stack before calling.
	...
	case *ast.CallExpr:
		...
		case *ast.ParenExpr:
			if tv := c.typeAndValueOf(fun); tv.IsType() {
				...
				// Conversions between func types are no-ops in AtipicialVM, so
		...
		case name != "":
			...
			// We care only about string and fixed-size array conversions because all others are effectively no-op in AtipicialVM.
...
func (c *codegen) convertBuiltin(expr *ast.CallExpr) {
	...
	case "clear":
		...
		case *types.Slice:
			emit.Opcodes(c.prog.BinWriter, opcode.DUP, opcode.SIZE, opcode.OVER,
				opcode.CLEARITEMS,
			)
			clearItemsBegin := c.newLabel()
			c.setLabel(clearItemsBegin)
			emit.Opcodes(c.prog.BinWriter, opcode.DUP, opcode.PUSH0)
			clearItemsEnd := c.newLabel()
			emit.Jmp(c.prog.BinWriter, opcode.JMPLEL, clearItemsEnd)
			emit.Opcodes(c.prog.BinWriter, opcode.DEC, opcode.OVER)
...
func (c *codegen) emitGetMapValueWithOKFlag(expr ast.Expr) {
	var (
		...
		typ     = c.typeOf(idxExpr.X).Underlying().(*types.Map)
	)
	...
	lHasKey := c.newLabel()
	emit.Jmp(c.prog.BinWriter, opcode.JMPIFL, lHasKey)
	emit.Opcodes(c.prog.BinWriter, opcode.DROP, opcode.DROP,
		opcode.PUSHF,
	)
	c.emitDefault(typ.Elem())
	emit.Instruction(c.prog.BinWriter, opcode.JMP, []byte{2 + 1 + 1 + 1})
	c.setLabel(lHasKey)
...
func codeGen(info *buildInfo) (*aef.File, *DebugInfo, error) {
	...
	f, err := aef.NewFile(buf)
	if err != nil {
		return nil, nil, fmt.Errorf("error while trying to create .aef file: %w", err)
	}
	if c.callTokens != nil {
		f.Tokens = c.callTokens
	}
	if info.options != nil && info.options.SourceURL != "" {
		if len(info.options.SourceURL) > aef.MaxSourceURLLength {
