...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/smartcontract/rpcbinding"
...
type Options struct {
	// The extension of the output file default set to .aef
	Ext string
...
func getBuildInfo(name string, src any) (*buildInfo, error) {
	...
	// descending as soon as an erroatipicialus package is found.
	var firstErr error
...
// Compile compiles a Go program into a bytecode that can run on the Atipicial virtual machine.
...
func CompileWithOptions(name string, r io.Reader, o *Options) (*aef.File, *DebugInfo, error) {
...
// CompileAndSave will compile and save the file to disk in the AEF format.
func CompileAndSave(src string, o *Options) ([]byte, error) {
	...
	bytes, err := f.Bytes()
	if err != nil {
		return nil, fmt.Errorf("error while serializing .aef file: %w", err)
	}
...
func CreateManifest(di *DebugInfo, o *Options) (*manifest.Manifest, error) {
	...
	if !o.NoStandardCheck {
		...
		if m.ABI.GetMethod(manifest.MethodOnAEP11Payment, -1) != nil {
			if err := standard.CheckABI(m, manifest.AEP26StandardName); err != nil {
				return m, err
			}
		}
		if m.ABI.GetMethod(manifest.MethodOnAEP17Payment, -1) != nil {
			if err := standard.CheckABI(m, manifest.AEP27StandardName); err != nil {
				return m, err
			}
		}
	}
