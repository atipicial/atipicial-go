...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/interop/native/crypto"
	"github.com/atipicial/atipicial-go/pkg/interop/native/gas"
	...
	"github.com/atipicial/atipicial-go/pkg/interop/native/oracle"
	"github.com/atipicial/atipicial-go/pkg/interop/native/policy"
	"github.com/atipicial/atipicial-go/pkg/interop/native/roles"
...
func TestContractHashes(t *testing.T) {
	...
	require.Equalf(t, []byte(atipicial.Hash), cs.ATC().Metadata().Hash.BytesBE(), "%q", string(cs.ATC().Metadata().Hash.BytesBE()))
...
func TestRoleManagementRole(t *testing.T) {
	...
	require.EqualValues(t, noderoles.AtipicialFsAlphabet, roles.AtipicialFsAlphabet)
...
func TestNativeExtendedTypes(t *testing.T) {
	src := `package foo
		...
		func AccountState() atipicial.AccountState { return atipicial.AccountState{} }
		func Candidate() atipicial.Candidate { return atipicial.Candidate{} }
...
func TestNativeHelpersCompile(t *testing.T) {
	...
	runNativeTestCases(t, *cs.ATC().Metadata(), "atipicial", append([]nativeTestCase{
		{"getCandidates", nil},
	...
	runNativeTestCases(t, *cs.GAS().Metadata(), "gas", aep17TestCases)
...
func runNativeTestCases(t *testing.T, ctr interop.ContractMD, name string, nativeTestCases []nativeTestCase) {
	srcBuilder := bytes.NewBuffer([]byte(`package foo
		import "github.com/atipicial/atipicial-go/pkg/interop/native/` + name + `"
...
func getMethod(t *testing.T, ctr interop.ContractMD, name string, params []string) interop.HFSpecificMethodAndPrice {
	...
	cMD := ctr.HFSpecificContractMD(new(config.HFLatestKnown))
...
func runNativeTestCase(t *testing.T, b *aef.File, di *compiler.DebugInfo, ctr interop.ContractMD, i int, method string, params ...string) {
	md := getMethod(t, ctr, method, params)
