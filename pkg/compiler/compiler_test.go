...
`
				...
				out := filepath.Join(tmp, "test.aef")
				_, err := compiler.CompileAndSave(in, &compiler.Options{Outfile: out})
				require.Error(t, err)
				require.Contains(t, err.Error(), "serialized AEF size exceeds VM stackitem limits")
			},
		},
	}
...
func TestOnPayableChecks(t *testing.T) {
	...
	t.Run("AEP-11, good", func(t *testing.T) {
		src := `package payable
	...
	t.Run("AEP-11, bad", func(t *testing.T) {
		src := `package payable
		import "github.com/atipicial/atipicial-go/pkg/interop"
		func OnAEP11Payment(from interop.Hash160, amount int, oldParam string, tokenID []byte, data any) {}`
		require.Error(t, compileAndCheck(t, src))
	})
	t.Run("AEP-17, good", func(t *testing.T) {
		src := `package payable
		import "github.com/atipicial/atipicial-go/pkg/interop"
		func OnAEP17Payment(from interop.Hash160, amount int, data any) {}`
		require.NoError(t, compileAndCheck(t, src))
	})
	t.Run("AEP-17, bad", func(t *testing.T) {
		src := `package payable
		import "github.com/atipicial/atipicial-go/pkg/interop"
		func OnAEP17Payment(from interop.Hash160, amount int, data any, extra int) {}`
		require.Error(t, compileAndCheck(t, src))
	})
}
...
func TestEventWarnings(t *testing.T) {
	...
	t.Run("event in imported package", func(t *testing.T) {
		t.Run("unused", func(t *testing.T) {
			src := `package foo
			import "github.com/atipicial/atipicial-go/pkg/compiler/testdata/notify"
			func Main() int {
				return notify.Value
			}`
...
func TestInvokedContractsPermissons(t *testing.T) {
	...
	t.Run("native", func(t *testing.T) {
		src := `package test
			import "github.com/atipicial/atipicial-go/pkg/interop/native/atipicial"
			import "github.com/atipicial/atipicial-go/pkg/interop/native/management"
			func Main() int {
				atipicial.Transfer(nil, nil, 10, nil)
	...
	t.Run("custom", func(t *testing.T) {
		...
		src := fmt.Sprintf(`package test
			...
			import "github.com/atipicial/atipicial-go/pkg/interop"
			import "github.com/atipicial/atipicial-go/pkg/compiler/testdata/runh"
...
func TestUnnamedParameterCheck(t *testing.T) {
	...
	t.Run("interface", func(t *testing.T) {
		src := `
		package testcase
		func OnAEP17Payment(h string, i int, _ any){}
	`
		_, _, err := compiler.CompileWithOptions("test.go", strings.NewReader(src), nil)
		require.Error(t, err)
		require.ErrorIs(t, err, compiler.ErrMissingExportedParamName)
	...
	t.Run("a set of unnamed params", func(t *testing.T) {
		src := `
		package testcase
		func OnAEP17Payment(_ string, _ int, _ any){}
	`
		_, _, err := compiler.CompileWithOptions("test.go", strings.NewReader(src), nil)
		require.Error(t, err)
		require.ErrorIs(t, err, compiler.ErrMissingExportedParamName)
	...
	t.Run("mixed named and unnamed params", func(t *testing.T) {
		src := `
		package testcase
		func OnAEP17Payment(s0, _, s2 string){}
	`
		_, _, err := compiler.CompileWithOptions("test.go", strings.NewReader(src), nil)
		require.Error(t, err)
		require.ErrorIs(t, err, compiler.ErrMissingExportedParamName)
	...
	t.Run("good", func(t *testing.T) {
		src := `
		package testcase
		func OnAEP17Payment(s string, i int, iface interface{}){}
	`
		_, _, err := compiler.CompileWithOptions("test.go", strings.NewReader(src), nil)
		require.NoError(t, err)
	})
	t.Run("good, use any keyword", func(t *testing.T) {
		src := `
		package testcase
		func OnAEP17Payment(s string, i int, iface any){}
	`
		_, _, err := compiler.CompileWithOptions("test.go", strings.NewReader(src), nil)
		require.NoError(t, err)
	})
	t.Run("method with unnamed params", func(t *testing.T) {
		...
		package testcase
		type A int
		func (rsv A) OnAEP17Payment(_ string, _ int, iface any){}
	`
		_, _, err := compiler.CompileWithOptions("test.go", strings.NewReader(src), nil)
	...
	for name, src := range tests {
		t.Run(name, func(t *testing.T) {
			...
			opts := &compiler.Options{
				ContractSupportedStandards: []string{manifest.AEP30StandardName},
