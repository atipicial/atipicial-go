...
var (
	...
	ErrGenericsUnsupported = errors.New("generics are currently unsupported, please, see the https://github.com/atipicial/atipicial-go/issues/2376")
)

var (
	// Go language builtin functions.
	goBuiltins = []string{"len", "append", "panic", "make", "copy", "recover", "delete", "min", "max", "clear"}
...
// pathToField returns a slice of indices (in reverse order: innermost to outermost)
// locating fldName inside strct, or nil if not found.
func pathToField(strct *types.Struct, fldName string) []int {
	...
	for i := range strct.NumFields() {
		...
		if inner, ok := getStruct(fld.Type()); ok {
			if p := pathToField(inner, fldName); p != nil {
				return append(p, i)
			}
		}
	}
...
func isSyscall(fun *funcScope) bool {
	...
	return fun.pkg.Name() == "atipicialgointernal" && (strings.HasPrefix(fun.name, "Syscall") ||
		strings.HasPrefix(fun.name, "Opcode") || strings.HasPrefix(fun.name, "CallWithToken"))
}

const interopPrefix = "github.com/atipicial/atipicial-go/pkg/interop"
...
func canConvert(s string) bool {
	...
	if isInteropPath(s) {
		...
		return s != "/iterator.Iterator" && s != "/storage.Context" &&
			...
			s != "/native/management.Contract" && s != "/native/atipicial.AccountState" &&
...
func canInline(s string, name string, isBuiltin bool) bool {
	if strings.HasPrefix(s, "github.com/atipicial/atipicial-go/pkg/compiler/testdata/inline") {
		return true
	}
	if !isInteropPath(s) {
		return false
	}
	return !strings.HasPrefix(s[len(interopPrefix):], "/atipicialgointernal") &&
