...
// verificationScriptSize is the normal  single-key verification script size.
...
// NewPublicKeyFromASN1 returns a ATC PublicKey from the ASN.1 serialized key.
...
// GetVerificationScript returns ATC VM bytecode with CHECKSIG command for the
// public key.
...
func (p *PublicKey) writeVerificationScript(buf []byte) []byte {
	...
	if address.Prefix == address.ATC2Prefix {
...
// Address returns a base58-encoded ATC-specific address based on the key hash.
