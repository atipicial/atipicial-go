...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/crypto/keys/internal/scrypt"
	"github.com/atipicial/atipicial-go/pkg/encoding/base58"
	"golang.org/x/text/unicode/norm"
)

// AEP-2 standard implementation for encrypting and decrypting private keys.

// AEP-2 specified parameters used for cryptography.
const (
	n       = 16384
	r       = 8
	p       = 8
...
// AEP2ScryptParams returns scrypt parameters specified in the AEP-2.
func AEP2ScryptParams() ScryptParams {
	return ScryptParams{
		N: n,
		R: r,
		P: p,
...
// AEP2Encrypt encrypts a the PrivateKey using the given passphrase
// under the AEP-2 standard.
func AEP2Encrypt(priv *PrivateKey, passphrase string, params ScryptParams) (s string, err error) {
	...
	var buf = make([]byte, 0, len(aepHeader)+1+len(addrHash)+len(encrypted))
	buf = append(buf, aepHeader...)
	buf = append(buf, aepFlag)
...
// AEP2Decrypt decrypts an encrypted key using the given passphrase
// under the AEP-2 standard.
func AEP2Decrypt(key, passphrase string, params ScryptParams) (*PrivateKey, error) {
	b, err := base58.CheckDecode(key)
	if err != nil {
		return nil, err
	}
	if err := validateAEP2Format(b); err != nil {
		return nil, err
	}
...
func validateAEP2Format(b []byte) error {
	if len(b) != 39 {
