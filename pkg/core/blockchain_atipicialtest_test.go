...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/core/native/nativeprices"
...
func TestBlockchain_InitHeaderHashes(t *testing.T) {
	...
	e := atipicialtest.NewExecutor(t, bcSpout, validators, validators)
	...
	check := func(t *testing.T, headerHeight int, trusted *block.Header) {
		...
		bc, _ := chain.NewSingleWithCustomConfigAndStore(t, func(cfg *config.Blockchain) {
			cfg.RemoveUntraceableBlocks = true // required for AtipicialFsStateSyncExtensions.
		}, ps, false)
		go bc.Run()
		...
		bc, _ = chain.NewSingleWithCustomConfigAndStore(t, func(cfg *config.Blockchain) {
			...
			cfg.AtipicialFsStateSyncExtensions = true
			cfg.AtipicialFsBlockFetcher.Enabled = true
			cfg.AtipicialFsStateFetcher.Enabled = true
...
// TestBlockchain_InitializeAtipicialCache_Bug3181 is aimed to reproduce and check situation
// when panic occurs on native Atipicial cache initialization due to access to native Policy
// cache when it's not yet initialized to recalculate candidates.
func TestBlockchain_InitializeAtipicialCache_Bug3181(t *testing.T) {
	ps, path := newLevelDBForTestingWithPath(t, "")
	...
	atipicial := e.NewInvoker(e.NativeHash(t, nativenames.Atipicial), acc)
	atipicial.Invoke(t, true, "registerCandidate", acc.(atipicialtest.SingleSigner).Account().PublicKey().Bytes()) // block #2

	// Put some empty blocks to reach N-1 block height, so that newEpoch cached
	// values of native Atipicial contract require an update on the subsequent cache
	// initialization.
	...
	// Atipicial newEpoch* cached values during initializaition. This update requires candidates
...
// TestBlockchain_InitializeAtipicialCache_Bug3424 ensures that Atipicial cache (new epoch
// committee and stand by validators) is properly initialized after node restart
// at the dBFT epoch boundary.
func TestBlockchain_InitializeAtipicialCache_Bug3424(t *testing.T) {
	ps, path := newLevelDBForTestingWithPath(t, "")
	bc, validators, committee, err := chain.NewMultiWithCustomConfigAndStoreNoCheck(t, nil, ps)
	require.NoError(t, err)
	go bc.Run()
	...
	atipicialCommitteeInvoker := e.CommitteeInvoker(e.NativeHash(t, nativenames.Atipicial))
...
func TestBlockchain_AddBadBlock(t *testing.T) {
	...
	tx := e.NewUnsignedTx(t, atipicialHash, "transfer", acc.ScriptHash(), util.Uint160{1, 2, 3}, 1, nil)
	...
	tx = e.NewUnsignedTx(t, atipicialHash, "transfer", acc.ScriptHash(), util.Uint160{1, 2, 3}, 1, nil) // Check the good tx.
	e.SignTx(t, tx, -1, acc)
	b = e.NewUnsignedBlock(t, tx)
	e.SignBlock(b)
	check(t, b, func(c *config.Blockchain) {
...
func TestBlockchain_GetBlock(t *testing.T) {
	...
	atipicialValidatorInvoker := e.ValidatorInvoker(e.NativeHash(t, nativenames.Atipicial))
	...
	t.Run("store only header", func(t *testing.T) {
		t.Run("non-empty block", func(t *testing.T) {
			tx := atipicialValidatorInvoker.PrepareInvoke(t, "transfer", acc.ScriptHash(), util.Uint160{1, 2, 3}, 1, nil)
			b := e.NewUnsignedBlock(t, tx)
			e.SignBlock(b)
			require.NoError(t, bc.AddHeaders(&b.Header))
...
func TestBlockchain_VerifyHashAgainstScript(t *testing.T) {
	...
	c1 := &atipicialtest.Contract{
		Hash:     cs.Hash,
		AEF:      &cs.AEF,
		Manifest: &cs.Manifest,
	}
	c2 := &atipicialtest.Contract{
		Hash:     csInvalid.Hash,
		AEF:      &csInvalid.AEF,
...
func TestBlockchain_IsTxStillRelevant(t *testing.T) {
	...
	t.Run("small ValidUntilBlock", func(t *testing.T) {
		tx := e.PrepareInvocation(t, []byte{byte(opcode.PUSH1)}, []atipicialtest.Signer{acc}, bc.BlockHeight()+1)
	...
	t.Run("tx is already persisted", func(t *testing.T) {
		tx := e.PrepareInvocation(t, []byte{byte(opcode.PUSH1)}, []atipicialtest.Signer{acc}, bc.BlockHeight()+2)
	...
	t.Run("tx with Conflicts attribute", func(t *testing.T) {
		tx1 := e.PrepareInvocation(t, []byte{byte(opcode.PUSH1)}, []atipicialtest.Signer{acc}, bc.BlockHeight()+5)

		tx2 := transaction.New([]byte{byte(opcode.PUSH1)}, 0)
		tx2.Nonce = atipicialtest.Nonce()
	...
	t.Run("contract witness check fails", func(t *testing.T) {
		...
		c := atipicialtest.CompileSource(t, acc.ScriptHash(), strings.NewReader(src), &compiler.Options{
...
func TestBlockchain_MemPoolRemoval(t *testing.T) {
	...
	for i := range addedTxes {
		addedTxes[i] = e.PrepareInvocation(t, []byte{byte(opcode.PUSH1)}, []atipicialtest.Signer{acc}, 100)
		require.NoError(t, bc.PoolTx(addedTxes[i]))
	}
	for i := range notAddedTxes {
		notAddedTxes[i] = e.PrepareInvocation(t, []byte{byte(opcode.PUSH1)}, []atipicialtest.Signer{acc}, 100)
...
func TestBlockchain_GetTransaction(t *testing.T) {
	...
	tx1 := e.PrepareInvocation(t, []byte{byte(opcode.PUSH1)}, []atipicialtest.Signer{acc})
	e.AddNewBlock(t, tx1)

	tx2 := e.PrepareInvocation(t, []byte{byte(opcode.PUSH2)}, []atipicialtest.Signer{acc})
...
func TestBlockchain_Subscriptions(t *testing.T) {
	...
	txGood1 := e.PrepareInvocation(t, script.Bytes(), []atipicialtest.Signer{acc})
	...
	require.NoError(t, script.Err)
	txBad := e.PrepareInvocation(t, script.Bytes(), []atipicialtest.Signer{acc})

	script = io.NewBufBinWriter()
	emit.Bytes(script.BinWriter, []byte("yay! yay! yay!"))
	emit.Syscall(script.BinWriter, interopnames.SystemRuntimeNotify)
	require.NoError(t, script.Err)
	txGood2 := e.PrepareInvocation(t, script.Bytes(), []atipicialtest.Signer{acc})
...
func TestBlockchain_RemoveUntraceable(t *testing.T) {
	...
	atipicialCommitteeKey := []byte{0xfb, 0xff, 0xff, 0xff, 0x0e}
	check := func(t *testing.T, bc *core.Blockchain, tHash, bHash, sHash util.Uint256, errorExpected bool) {
		...
		if !sHash.Equals(util.Uint256{}) {
			...
			_, err = sm.GetState(sHash, atipicialCommitteeKey)
			if errorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
	...
	t.Run("P2PStateExchangeExtensions off", func(t *testing.T) {
		...
		tx1Hash := atipicialValidatorInvoker.Invoke(t, true, "transfer", acc.ScriptHash(), util.Uint160{1, 2, 3}, 1, nil)
	...
	t.Run("P2PStateExchangeExtensions on", func(t *testing.T) {
		...
		tx2Hash := atipicialValidatorInvoker.Invoke(t, true, "transfer", acc.ScriptHash(), util.Uint160{1, 2, 3}, 1, nil)
...
func TestBlockchain_InvalidNotification(t *testing.T) {
	...
	e.DeployContract(t, &atipicialtest.Contract{
		Hash:     cs.Hash,
		AEF:      &cs.AEF,
		Manifest: &cs.Manifest,
	}, nil)
...
func TestConfig_LoadDefaultConfigs(t *testing.T) {
	...
	testCases := []any{
		...
		netmode.TestNet,
		netmode.UnitTestNet,
		netmode.MainNetAtipicialFs,
		netmode.TestNetAtipicialFs,
		"privnet.docker.one",
		"privnet.docker.two",
...
func TestBlockchain_VerifyTx(t *testing.T) {
	...
	atipicialOwner := validator.ScriptHash()

	atipicialAmount := int64(1_000_000)
	gasAmount := int64(1_000_000_000)
	txs := make([]*transaction.Transaction, 0, 2*len(accs)+2)
	for _, a := range accs {
		txs = append(txs, atipicialValidatorsInvoker.PrepareInvoke(t, "transfer", atipicialOwner, a.Contract.ScriptHash(), atipicialAmount, nil))
		txs = append(txs, gasValidatorsInvoker.PrepareInvoke(t, "transfer", atipicialOwner, a.Contract.ScriptHash(), gasAmount, nil))
	}
	txs = append(txs, atipicialValidatorsInvoker.PrepareInvoke(t, "transfer", atipicialOwner, committee.ScriptHash(), atipicialAmount, nil))
	txs = append(txs, gasValidatorsInvoker.PrepareInvoke(t, "transfer", atipicialOwner, committee.ScriptHash(), gasAmount, nil))
	txs = append(txs, gasValidatorsInvoker.PrepareInvoke(t, "transfer", atipicialOwner, notaryHash, gasAmount, stackitem.Make([]stackitem.Item{stackitem.Make(validator.ScriptHash()), stackitem.Make(100500)}))) // make Notary deposit.
	e.AddNewBlock(t, txs...)
	for _, tx := range txs {
	...
	t.Run("Attribute", func(t *testing.T) {
		...
		t.Run("NotValidBefore", func(t *testing.T) {
			getNVBTx := func(e *atipicialtest.Executor, height uint32) *transaction.Transaction {
				tx := newTestTx(t, h, testScript)
			...
			t.Run("Disabled", func(t *testing.T) { // check that NVB attribute is not an extension anymore.
				...
				eBad := atipicialtest.NewExecutor(t, bcBad, validatorBad, committeeBad)
		...
		t.Run("Reserved", func(t *testing.T) {
			getReservedTx := func(e *atipicialtest.Executor, attrType transaction.AttrType) *transaction.Transaction {
				tx := newTestTx(t, h, testScript)
		...
		t.Run("Conflicts", func(t *testing.T) {
			getConflictsTx := func(e *atipicialtest.Executor, hashes ...util.Uint256) *transaction.Transaction {
				tx := newTestTx(t, h, testScript)
			...
			t.Run("enabled", func(t *testing.T) {
				...
				t.Run("attribute on-chain conflict", func(t *testing.T) {
					tx := atipicialValidatorsInvoker.Invoke(t, stackitem.NewBool(true), "transfer", atipicialOwner, atipicialOwner, 1, nil)
		...
		t.Run("NotaryAssisted", func(t *testing.T) {
			...
			getNotaryAssistedTx := func(e *atipicialtest.Executor, signaturesCount uint8, serviceFee int64) *transaction.Transaction {
				tx := newTestTx(t, h, testScript)
		...
		getPartiallyFilledTx := func(nvb uint32, validUntil uint32) *transaction.Transaction {
			...
			tx.NetworkFee = netFee + // multisig witness verification price
				...
				vm.PicoGasToDatoshiInt64(fee.Opcode(bc.GetBaseExecFee(), // Notary verification script
					...
					opcode.PUSH0, opcode.SYSCALL, opcode.RET)) + // Atipicial.Native.Call
...
func TestBlockchain_Bug1728(t *testing.T) {
	...
	c := atipicialtest.CompileSource(t, acc.ScriptHash(), strings.NewReader(src), &compiler.Options{Name: "TestContract"})
...
func TestBlockchain_ResetState(t *testing.T) {
	...
	staleID := basicchain.NFSOContractID // AEP11
	...
	atipicialH := e.NativeHash(t, nativenames.Atipicial)
	gasID := e.NativeID(t, nativenames.Gas)
	atipicialID := e.NativeID(t, nativenames.Atipicial)
	...
	acc0 := e.Validator.(atipicialtest.MultiSigner).Single(2) // priv0 index->order and order->index conversion
	priv0ScriptHash := acc0.ScriptHash()
	var (
		expectedAEP11t []*state.AEP11Transfer
		expectedAEP17t []*state.AEP17Transfer
	)
	require.NoError(t, bc.ForEachAEP11Transfer(priv0ScriptHash, resetBlockHeader.Timestamp, func(t *state.AEP11Transfer) (bool, error) {
		if t.Block <= resetBlockIndex {
			expectedAEP11t = append(expectedAEP11t, t)
		}
		return true, nil
	}))
	require.NoError(t, bc.ForEachAEP17Transfer(priv0ScriptHash, resetBlockHeader.Timestamp, func(t *state.AEP17Transfer) (bool, error) {
		if t.Block <= resetBlockIndex {
			expectedAEP17t = append(expectedAEP17t, t)
		}
		return true, nil
	}))
	...
	// Check AEP-compatible contracts.
	aep11 := bc.GetAEP11Contracts()
	require.Equal(t, 1, len(aep11)) // NNS
	require.Equal(t, nnsH, aep11[0])
	aep17 := bc.GetAEP17Contracts()
	require.Equal(t, 3, len(aep17)) // Atipicial, Gas, Rubles
	require.ElementsMatch(t, []util.Uint160{gasH, atipicialH, rublesH}, aep17)
	...
	// Check transfers.
	var (
		actualAEP11t []*state.AEP11Transfer
		actualAEP17t []*state.AEP17Transfer
	)
	require.NoError(t, bc.ForEachAEP11Transfer(priv0ScriptHash, e.TopBlock(t).Timestamp, func(t *state.AEP11Transfer) (bool, error) {
		actualAEP11t = append(actualAEP11t, t)
		return true, nil
	}))
	require.NoError(t, bc.ForEachAEP17Transfer(priv0ScriptHash, e.TopBlock(t).Timestamp, func(t *state.AEP17Transfer) (bool, error) {
		actualAEP17t = append(actualAEP17t, t)
		return true, nil
	}))
	assert.Equal(t, expectedAEP11t, actualAEP11t)
	assert.Equal(t, expectedAEP17t, actualAEP17t)
...
func TestBlockchain_GenesisTransactionExtension(t *testing.T) {
	...
	emit.Syscall(script.BinWriter, interopnames.SystemRuntimeCheckWitness)
	emit.AppCall(script.BinWriter, nativehashes.AtipicialCoin, "transfer", callflag.All, from, to, amount, nil)
	...
	actualAtipicial, lub := e.Chain.GetGoverningTokenBalance(to)
	require.Equal(t, int64(amount), actualAtipicial.Int64())
...
// sure that AtipicialGo node is not affected by https://github.com/atipicial-project/atipicial/issues/3300 and does not need
// the https://github.com/atipicial-project/atipicial/pull/3301.
