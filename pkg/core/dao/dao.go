...
// GetTokenTransferInfo retrieves AEP-17 transfer info from the cache.
...
// PutTokenTransferInfo saves AEP-17 transfer info in the cache.
...
// -- end AEP-17 transfer info.

// -- start transfer log.

func (dao *Simple) getTokenTransferLogKey(acc util.Uint160, newestTimestamp uint64, index uint32, isAEP11 bool) []byte {
	key := dao.getKeyBuf(1 + util.Uint160Size + 8 + 4)
	if isAEP11 {
		key[0] = byte(storage.STAEP11Transfers)
	} else {
		key[0] = byte(storage.STAEP17Transfers)
	}
	copy(key[1:], acc.BytesBE())
...
// SeekAEP17TransferLog executes f for each AEP-17 transfer in log starting from
...
func (dao *Simple) SeekAEP17TransferLog(acc util.Uint160, newestTimestamp uint64, f func(*state.AEP17Transfer) (bool, error)) error {
...
// SeekAEP11TransferLog executes f for each AEP-11 transfer in log starting from
// the transfer with the newest timestamp up to the oldest transfer. It continues
// iteration until false is returned from f. The last non-nil error is returned.
func (dao *Simple) SeekAEP11TransferLog(acc util.Uint160, newestTimestamp uint64, f func(*state.AEP11Transfer) (bool, error)) error {
...
func (dao *Simple) GetTokenTransferLog(acc util.Uint160, newestTimestamp uint64, index uint32, isAEP11 bool) (*state.TokenTransferLog, error) {
	key := dao.getTokenTransferLogKey(acc, newestTimestamp, index, isAEP11)
...
func (dao *Simple) PutTokenTransferLog(acc util.Uint160, start uint64, index uint32, isAEP11 bool, lg *state.TokenTransferLog) {
	key := dao.getTokenTransferLogKey(acc, start, index, isAEP11)
