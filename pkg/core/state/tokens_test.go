...
func TestTokenTransferLog_Append17(t *testing.T) {
	...
	i := len(expected) - 1
	cont, err := lg.ForEachAEP17(func(tr *AEP17Transfer) (bool, error) {
...
func TestTokenTransferLog_Append(t *testing.T) {
	...
	i := len(expected) - 1
	cont, err := lg.ForEachAEP11(func(tr *AEP11Transfer) (bool, error) {
		require.Equal(t, expected[i], tr)
		i--
		return true, nil
	})
...
func BenchmarkTokenTransferLog_Append(b *testing.B) {
	r := rand.New(rand.NewPCG(42, 100500))
	ts := make([]*AEP17Transfer, TokenTransferBatchSize)
	for i := range ts {
...
func TestAEP17Transfer_DecodeBinary(t *testing.T) {
	expected := &AEP17Transfer{
		Asset:        123,
	...
	testserdes.EncodeDecodeBinary(t, expected, new(AEP17Transfer))
}

func TestAEP11Transfer_DecodeBinary(t *testing.T) {
	expected := &AEP11Transfer{
		AEP17Transfer: AEP17Transfer{
			Asset:        123,
			Counterparty: util.Uint160{5, 6, 7},
			Amount:       big.NewInt(42),
			Block:        12345,
	...
	testserdes.EncodeDecodeBinary(t, expected, new(AEP11Transfer))
}

func random17Transfer(r *rand.Rand) *AEP17Transfer {
	return &AEP17Transfer{
...
func random11Transfer(r *rand.Rand) *AEP11Transfer {
	return &AEP11Transfer{
		AEP17Transfer: AEP17Transfer{
			Amount:       big.NewInt(int64(r.Uint64())),
			Block:        r.Uint32(),
			Asset:        int32(random.Int(10, 10000000)),
