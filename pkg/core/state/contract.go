...
// Contract holds information about a smart contract in the Atipicial blockchain.
...
func (c *Contract) ToStackItem() (stackitem.Item, error) {
	// Do not skip the AEF size check, it won't affect native Management related
	// states as the same checked is performed during contract deploy/update.
	rawAef, err := c.AEF.Bytes()
	if err != nil {
		return nil, err
	}
...
func (c *Contract) FromStackItem(item stackitem.Item) error {
	...
	if err != nil {
		return fmt.Errorf("invalid AEF: %w", err)
	}
	c.AEF, err = aef.FileFromBytes(bytes)
	if err != nil {
		return fmt.Errorf("failed to decode AEF: %w", err)
	}
...
func (c *Contract) ToSCParameter() (smartcontract.Parameter, error) {
	...
	rawAef, err := c.AEF.Bytes()
	if err != nil {
		return smartcontract.Parameter{}, fmt.Errorf("field AEF: %w", err)
	}
	...
	prm, err = smartcontract.NewParameterFromValue(rawAef)
	if err != nil {
		return smartcontract.Parameter{}, fmt.Errorf("field AEF: %w", err)
	}
	prms = append(prms, prm)
