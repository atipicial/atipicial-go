...
func TestStateSyncModule_Init(t *testing.T) {
	...
	e := atipicialtest.NewExecutor(t, bcSpout, validators, committee)
	...
	boltCfgStorage := func(c *config.Blockchain) {
		...
		c.KeepOnlyLatestState = true
		c.RemoveUntraceableBlocks = true
		c.AtipicialFsStateSyncExtensions = true
		c.AtipicialFsStateFetcher.Enabled = true
		c.AtipicialFsBlockFetcher.Enabled = true
	}
	...
	t.Run("error: bolt chain is too low to start state sync process", func(t *testing.T) {
		...
		eBolt := atipicialtest.NewExecutor(t, bcBolt, validatorsBolt, committeeBolt)
		eBolt.AddNewBlock(t)
...
func TestStateSyncModule_SetOnStageChanged(t *testing.T) {
	...
	e := atipicialtest.NewExecutor(t, bcSpout, vals, comm)
