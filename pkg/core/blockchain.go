...
import (
	...
	json "github.com/github.com/atipicial/go-ordered-json"
	...
	"github.com/atipicial/atipicial-go/pkg/core/interop/contract"
	...
	"github.com/atipicial/atipicial-go/pkg/core/stateroot"
	"github.com/atipicial/atipicial-go/pkg/core/statesync"
...
const (
	...
	headersReset
	// transfersReset denotes AEP transfers were successfully updated (applicable to state reset only).
	transfersReset
...
func NewBlockchain(s storage.Store, cfg config.Blockchain, log *zap.Logger, newNatives ...func(cfg config.ProtocolConfiguration) []interop.Contract) (*Blockchain, error) {
	...
	if cfg.P2PStateExchangeExtensions && cfg.AtipicialFsStateSyncExtensions {
		return nil, errors.New("P2PStateExchangeExtensions and AtipicialFsStateSyncExtensions cannot be enabled simultaatipicialusly")
	}
	if cfg.TrustedHeader.Index > 0 && !cfg.P2PStateExchangeExtensions && !cfg.AtipicialFsStateSyncExtensions {
		return nil, errors.New("TrustedHeader can not be used without P2PStateExchangeExtensions or AtipicialFsStateSyncExtensions")
	}
	if cfg.TrustedHeader.Index == 0 && (cfg.P2PStateExchangeExtensions || cfg.AtipicialFsStateSyncExtensions) {
	...
	if cfg.AtipicialFsStateSyncExtensions {
		if !cfg.AtipicialFsBlockFetcher.Enabled || !cfg.AtipicialFsStateFetcher.Enabled {
			return nil, errors.New("AtipicialFsStateSyncExtensions are enabled, but AtipicialFsBlockFetcher or AtipicialFsStateFetcher are off")
		}
		if !cfg.RemoveUntraceableBlocks {
			return nil, errors.New("AtipicialFsStateFetcher cannot be used on archival nodes; set RemoveUntraceableBlocks=true")
		}
		if cfg.StateSyncInterval <= 0 {
			cfg.StateSyncInterval = config.DefaultStateSyncInterval
			log.Info("StateSyncInterval is not set or wrong, using default value",
	...
	bc.atipicial = bc.contracts.ATC()
	if bc.atipicial == native.IATC(nil) {
		return nil, errors.New("native AtipicialCoin implementation is required")
	}
	if err := validateNative(bc.atipicial, nativeids.AtipicialCoin, nativenames.Atipicial, nativehashes.AtipicialCoin); err != nil {
		return nil, err
	}
	bc.gas = bc.contracts.GAS()
	if bc.gas == native.IGAS(nil) {
		return nil, errors.New("native AtipicialDollar implementation is required")
	}
	if err := validateNative(bc.gas, nativeids.AtipicialDollar, nativenames.Gas, nativehashes.AtipicialDollar); err != nil {
		return nil, err
	}
...
func (bc *Blockchain) SetOracle(mod native.OracleService) {
	...
	if mod != nil {
		...
		mod.UpdateNativeContract(orcMd.AEF.Script, orc.GetOracleResponseScript(),
...
func (bc *Blockchain) init() error {
	...
	if err != nil {
		...
		if bc.config.TrustedHeader.Index > 0 {
			...
			if (bc.config.P2PStateExchangeExtensions || bc.config.AtipicialFsStateSyncExtensions) &&
	...
	if err == nil {
		...
		if (!bc.config.P2PStateExchangeExtensions || !bc.config.RemoveUntraceableBlocks) && !bc.config.AtipicialFsStateSyncExtensions {
			return errors.New("state jump was not completed, P2PStateExchangeExtensions (with RemoveUntraceableBlocks) " +
				"or AtipicialFsStateSyncExtensions must be enabled. " +
	...
	// Check autogenerated native contracts' manifests and AEFs against the stored ones.
...
func (bc *Blockchain) jumpToStateInternal(p uint32, stage stateChangeStage) error {
	...
	case newStorageItemsAdded:
		...
		if int(p)-int(mtb) > 0 {
			...
			prefixes := []byte{byte(storage.STAEP11Transfers), byte(storage.STAEP17Transfers), byte(storage.STTokenTransferInfo)}
	...
	if bc.config.AtipicialFsStateSyncExtensions {
...
func (bc *Blockchain) resetStateInternal(height uint32, stage stateChangeStage) error {
	...
	case headersReset:
		// Reset MPT.
		bc.log.Debug("trying to reset state root information and AEP transfers")
		...
		bc.log.Info("state root information and AEP transfers are reset", zap.Duration("took", time.Since(p)))
		...
		persistCh <- func(persistedKeys int, err error) error {
			...
			bc.log.Debug("state root information and AEP transfers are persisted", zap.Duration("took", time.Since(persistStart)), zap.Int("keys", persistedKeys))
			return nil
		}
		p = time.Now()
		fallthrough
...
// resetTransfers is a helper function that strips the top newest AEP17 and AEP11 transfer logs
...
func (bc *Blockchain) resetTransfers(cache *dao.Simple, height uint32) error {
	...
	prefixes := []byte{byte(storage.STAEP11Transfers), byte(storage.STAEP17Transfers)}
	for i := range prefixes {
		var (
			acc             util.Uint160
		...
		cache.Store.Seek(storage.SeekRange{
			...
			for range v[0] { // From oldest to newest transfer of the batch.
				var t *state.AEP17Transfer
				if k[0] == byte(storage.STAEP11Transfers) {
					tr := new(state.AEP11Transfer)
					tr.DecodeBinary(r)
					t = &tr.AEP17Transfer
				} else {
					t = new(state.AEP17Transfer)
					t.DecodeBinary(r)
				}
				if r.Err != nil {
				...
				appendTokenTransferInfo(trInfo, t.Asset, t.Block, t.Timestamp, k[0] == byte(storage.STAEP11Transfers), newBatchSize >= state.TokenTransferBatchSize)
			}
...
func appendTokenTransferInfo(transferData *state.TokenTransferInfo,
	token int32, bIndex uint32, bTimestamp uint64, isAEP11 bool, lastTransferInBatch bool) {
	var (
		newBatch      *bool
		nextBatch     *uint32
	...
	if !isAEP11 {
		newBatch = &transferData.NewAEP17Batch
		nextBatch = &transferData.NextAEP17Batch
		currTimestamp = &transferData.NextAEP17NewestTimestamp
	} else {
		newBatch = &transferData.NewAEP11Batch
		nextBatch = &transferData.NextAEP11Batch
		currTimestamp = &transferData.NextAEP11NewestTimestamp
	}
...
func (bc *Blockchain) storeBlock(block *block.Block, txpool *mempool.Pool) error {
	...
	go func() {
		...
		for acc, trData := range transCache {
			...
			if !trData.Info.NewAEP11Batch {
				kvcache.PutTokenTransferLog(acc, trData.Info.NextAEP11NewestTimestamp, trData.Info.NextAEP11Batch, true, &trData.Log11)
			}
			if !trData.Info.NewAEP17Batch {
				kvcache.PutTokenTransferLog(acc, trData.Info.NextAEP17NewestTimestamp, trData.Info.NextAEP17Batch, false, &trData.Log17)
			}
		}
		close(aerdone)
	}()
...
func (bc *Blockchain) updateExtensibleWhitelist(height uint32) error {
	...
	newList := []util.Uint160{bc.atipicial.GetCommitteeAddress(bc.dao)}
	nextVals := bc.atipicial.GetNextBlockValidatorsInternal(bc.dao)
...
func (bc *Blockchain) processTokenTransfer(cache *dao.Simple, transCache map[util.Uint160]transferData,
	...
	var aep17xfer *state.AEP17Transfer
	var isAEP11 = (tokenID != nil)
	if !isAEP11 {
		aep17xfer = &state.AEP17Transfer{
			Asset:        id,
			Amount:       amount,
			Block:        b.Index,
			Counterparty: to,
		...
		aep11xfer := &state.AEP11Transfer{
			AEP17Transfer: state.AEP17Transfer{
				Asset:        id,
				Amount:       amount,
				Block:        b.Index,
				Counterparty: to,
		...
		transfer = aep11xfer
		aep17xfer = &aep11xfer.AEP17Transfer
	}
	if !from.Equals(util.Uint160{}) {
		_ = aep17xfer.Amount.Neg(aep17xfer.Amount)
		err := appendTokenTransfer(cache, transCache, from, transfer, id, b.Index, b.Timestamp, isAEP11)
		_ = aep17xfer.Amount.Neg(aep17xfer.Amount)
		if err != nil {
			return
		}
	...
	if !to.Equals(util.Uint160{}) {
		aep17xfer.Counterparty = from
		_ = appendTokenTransfer(cache, transCache, to, transfer, id, b.Index, b.Timestamp, isAEP11) // Nothing useful we can do.
	}
}
...
func appendTokenTransfer(cache *dao.Simple, transCache map[util.Uint160]transferData, addr util.Uint160, transfer io.Serializable,
	token int32, bIndex uint32, bTimestamp uint64, isAEP11 bool) error {
	...
	if !ok {
		...
		if !balances.NewAEP11Batch {
			trLog, err := cache.GetTokenTransferLog(addr, balances.NextAEP11NewestTimestamp, balances.NextAEP11Batch, true)
			if err != nil {
				return err
			}
			transferData.Log11 = *trLog
		}
		if !balances.NewAEP17Batch {
			trLog, err := cache.GetTokenTransferLog(addr, balances.NextAEP17NewestTimestamp, balances.NextAEP17Batch, false)
			if err != nil {
				return err
			}
	...
	if !isAEP11 {
		...
		nextBatch = transferData.Info.NextAEP17Batch
		currTimestamp = transferData.Info.NextAEP17NewestTimestamp
	} else {
		log = &transferData.Log11
		nextBatch = transferData.Info.NextAEP11Batch
		currTimestamp = transferData.Info.NextAEP11NewestTimestamp
	}
	...
	if newBatch {
		cache.PutTokenTransferLog(addr, currTimestamp, nextBatch, isAEP11, log)
		// Put makes a copy of it anyway.
		log.Reset()
	}
	appendTokenTransferInfo(&transferData.Info, token, bIndex, bTimestamp, isAEP11, newBatch)
	transCache[addr] = transferData
	return nil
}

// ForEachAEP17Transfer executes f for each AEP-17 transfer in log starting from
...
func (bc *Blockchain) ForEachAEP17Transfer(acc util.Uint160, newestTimestamp uint64, f func(*state.AEP17Transfer) (bool, error)) error {
	return bc.dao.SeekAEP17TransferLog(acc, newestTimestamp, f)
}

// ForEachAEP11Transfer executes f for each AEP-11 transfer in log starting from
// the transfer with the newest timestamp up to the oldest transfer. It continues
// iteration until false is returned from f. The last non-nil error is returned.
func (bc *Blockchain) ForEachAEP11Transfer(acc util.Uint160, newestTimestamp uint64, f func(*state.AEP11Transfer) (bool, error)) error {
	return bc.dao.SeekAEP11TransferLog(acc, newestTimestamp, f)
}

// GetAEP17Contracts returns the list of deployed AEP-17 contracts.
func (bc *Blockchain) GetAEP17Contracts() []util.Uint160 {
	return bc.management.GetAEP17Contracts(bc.dao)
}

// GetAEP11Contracts returns the list of deployed AEP-11 contracts.
func (bc *Blockchain) GetAEP11Contracts() []util.Uint160 {
	return bc.management.GetAEP11Contracts(bc.dao)
}
...
func (bc *Blockchain) GetTokenLastUpdated(acc util.Uint160) (map[int32]uint32, error) {
	...
	if bc.config.P2PStateExchangeExtensions && bc.config.RemoveUntraceableBlocks {
		if _, ok := info.LastUpdated[bc.atipicial.Metadata().ID]; !ok {
			nBalance, lub := bc.atipicial.BalanceOf(bc.dao, acc)
			if nBalance.Sign() != 0 {
				info.LastUpdated[bc.atipicial.Metadata().ID] = lub
			}
		}
	}
	stateSyncPoint, err := bc.dao.GetStateSyncPoint()
...
// amount of ATC between specified blocks.
func (bc *Blockchain) CalculateClaimable(acc util.Uint160, endHeight uint32) (*big.Int, error) {
	...
	return bc.atipicial.CalculateBonus(ic, acc, endHeight)
}
...
func (bc *Blockchain) ApplyPolicyToTxSet(txes []*transaction.Transaction) []*transaction.Transaction {
	...
	if oldVC == nil || oldVC != curVC {
		...
		verification, _ := smartcontract.CreateDefaultMultiSigRedeemScript(bc.atipicial.GetNextBlockValidatorsInternal(bc.dao))
...
func (bc *Blockchain) verifyTxAttributes(d *dao.Simple, tx *transaction.Transaction, isPartialTx bool) error {
	...
	for i := range tx.Attributes {
		...
		case transaction.HighPriority:
			h := bc.atipicial.GetCommitteeAddress(d)
			if !tx.HasSigner(h) {
...
func (bc *Blockchain) GetCommittee() (keys.PublicKeys, error) {
	pubs := bc.atipicial.GetCommitteeMembers(bc.dao)
...
// recalculated from the latest relevant information about ATC votes; in this case
...
func (bc *Blockchain) ComputeNextBlockValidators() []*keys.PublicKey {
	return bc.atipicial.ComputeNextBlockValidators(bc.dao)
}
...
func (bc *Blockchain) InitVerificationContext(ic *interop.Context, hash util.Uint160, witness *transaction.Witness) error {
	if len(witness.VerificationScript) != 0 {
		...
		ic.VM.LoadAEFMethod(&cs.AEF, &cs.Manifest, util.Uint160{}, hash, callflag.ReadOnly,
...
// verification.
// Golang implementation of VerifyWitnesses method in C# (https://github.com/atipicial-project/atipicial/blob/master/atipicial/SmartContract/Helper.cs#L87).
