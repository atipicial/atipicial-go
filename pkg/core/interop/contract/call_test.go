...
func TestSystemContractCall_Permissions(t *testing.T) {
	check := func(t *testing.T, cfg func(*config.Blockchain), shouldUpdateFail bool) {
		...
		ctrA := atipicialtest.CompileSource(t, acc.ScriptHash(), strings.NewReader(srcA), &compiler.Options{
			NoEventsCheck:      true,
		...
		func Update(aef []byte, manifest []byte) int {
			management.Update(aef, manifest)
			res := contract.Call(interop.Hash160{` + hashAStr.String() + `}, "retOne", contract.All).(int)
			return res
		}`
		ctrB := atipicialtest.CompileSource(t, acc.ScriptHash(), strings.NewReader(srcB), &compiler.Options{
			Name:               "contractB",
			NoEventsCheck:      true,
			NoPermissionsCheck: true,
		...
		ctrBUpdated := atipicialtest.CompileSource(t, acc.ScriptHash(), strings.NewReader(srcB), &compiler.Options{
			Name:               "contractB",
			NoEventsCheck:      true,
			NoPermissionsCheck: true,
			Permissions: []manifest.Permission{
...
func TestSnapshotIsolation_Exceptions(t *testing.T) {
	...
	srcA := `package contractA
		...
		func DoAndPanic(key, value []byte, nNtf int) int { // avoid https://github.com/atipicial/atipicial-go/issues/2509
...
// This test is written to avoid https://github.com/atipicial-project/atipicial/issues/2746.
...
// This test is written to check https://github.com/atipicial/atipicial-go/issues/2509
// and https://github.com/atipicial-project/atipicial/pull/2745#discussion_r879167180.
func TestRET_after_FINALLY_PanicInsideVoidMethod(t *testing.T) {
	...
	srcB := `package contractB
		...
		func Catch() {
			defer func() {
				if r := recover(); r != nil {
					// Call method with return value to check https://github.com/atipicial-project/atipicial/pull/2745#discussion_r879167180.
...
// This test is written to check https://github.com/atipicial-project/atipicial/pull/2745#discussion_r879125733.
...
// This test is created to check https://github.com/atipicial-project/atipicial/pull/2755#discussion_r880087983.
