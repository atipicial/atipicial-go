...
func TestCreateStandardAccount(t *testing.T) {
	...
	t.Run("Good", func(t *testing.T) {
		...
		tx := e.PrepareInvocation(t, script, []atipicialtest.Signer{e.Validator}, bc.BlockHeight()+1)
		e.AddNewBlock(t, tx)
...
func TestCreateMultisigAccount(t *testing.T) {
	...
	t.Run("Good", func(t *testing.T) {
		...
		txH := e.InvokeScript(t, script, []atipicialtest.Signer{acc})
		e.CheckHalt(t, txH)
	...
	t.Run("InvalidKey", func(t *testing.T) {
		...
		e.InvokeScriptCheckFAULT(t, script, []atipicialtest.Signer{acc}, "invalid prefix 1")
	})
	t.Run("Invalid m", func(t *testing.T) {
		pk, err := keys.NewPrivateKey()
		require.NoError(t, err)
		script := createScript(t, []any{pk.PublicKey().Bytes()}, 2)
		e.InvokeScriptCheckFAULT(t, script, []atipicialtest.Signer{acc}, "length of the signatures (2) is higher then the number of public keys")
	})
	t.Run("m overflows int32", func(t *testing.T) {
		...
		emit.Syscall(w.BinWriter, interopnames.SystemContractCreateMultisigAccount)
		require.NoError(t, w.Err)
		e.InvokeScriptCheckFAULT(t, w.Bytes(), []atipicialtest.Signer{acc}, "m must be positive and fit int32")
	})
}
...
func TestCreateAccount_HFAspidochelone(t *testing.T) {
	...
	createAccTx := func(t *testing.T, script []byte) *transaction.Transaction {
		tx := e.PrepareInvocation(t, script, []atipicialtest.Signer{e.Committee}, bc.BlockHeight()+1)
		return tx
	}
