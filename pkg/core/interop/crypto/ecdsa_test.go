...
func initCheckMultisigVMNoArgs(container *transaction.Transaction) *vm.VM {
	...
	binary.LittleEndian.PutUint32(buf[1:], atipicialCryptoCheckMultisigID)
...
func initCHECKMULTISIGVM(t *testing.T, n int, ik, is []int) *vm.VM {
	tx := transaction.New([]byte("ATC - An Open Network For Smart Economy"), 10)
...
func testCurveCHECKMULTISIGBad(t *testing.T) {
	...
	msg := []byte("ATC - An Open Network For Smart Economy")
