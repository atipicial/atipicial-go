...
func (c *ContractMD) buildHFSpecificMD(hf config.Hardfork) {
	...
	// AEF is now stored in the contract state and affects state dump.
	// Therefore, values are taken from C# node.
	nf := aef.File{
		Header: aef.Header{
			Magic:    aef.Magic,
			Compiler: "atipicial-core-v3.0",
		},
		Tokens: []aef.MethodToken{}, // avoid `nil` result during JSON marshalling,
		Script: w.Bytes(),
	}
