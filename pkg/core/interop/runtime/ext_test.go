...
func getDeployedInternal(t *testing.T) (*atipicialtest.Executor, atipicialtest.Signer, *core.Blockchain, *state.Contract) {
	...
	require.NoError(t, err)
	rawAef, err := cs.AEF.Bytes()
	require.NoError(t, err)
	tx := managementInvoker.PrepareInvoke(t, "deploy", rawAef, rawManifest)
	e.AddNewBlock(t, tx)
		...
		t.Run("check scope", func(t *testing.T) {
			...
			t.Run("CustomGroups", func(t *testing.T) {
				...
				t.Run("positive", func(t *testing.T) {
					...
					ne, err := aef.NewFile(contractScript)
					require.NoError(t, err)
...
func TestLoadScript(t *testing.T) {
	...
	t.Run("no ret val", func(t *testing.T) {
		...
		e.InvokeScriptCheckHALT(t, script, []atipicialtest.Signer{acc}, stackitem.Null{})
	})
	...
	t.Run("bad script", func(t *testing.T) {
		...
		e.InvokeScriptCheckFAULT(t, script, []atipicialtest.Signer{acc}, "invalid script")
	})
	t.Run("ret val, no params", func(t *testing.T) {
		script := wrapDynamicScript(t, []byte{byte(opcode.PUSH1)}, callflag.All)
		e.InvokeScriptCheckHALT(t, script, []atipicialtest.Signer{acc}, stackitem.Make(1))
	})
	t.Run("ret val with params", func(t *testing.T) {
		script := wrapDynamicScript(t, []byte{byte(opcode.MUL)}, callflag.All, 2, 2)
		e.InvokeScriptCheckHALT(t, script, []atipicialtest.Signer{acc}, stackitem.Make(4))
	})
	t.Run("two return values", func(t *testing.T) {
		...
		e.InvokeScriptCheckFAULT(t, script, []atipicialtest.Signer{acc}, "multiple return values in a cross-contract call")
	})
	t.Run("invalid flags", func(t *testing.T) {
		script := wrapDynamicScript(t, []byte{byte(opcode.MUL)}, callflag.CallFlag(0xff), 2, 2)
		e.InvokeScriptCheckFAULT(t, script, []atipicialtest.Signer{acc}, "call flags out of range")
	})
	t.Run("abort", func(t *testing.T) {
		script := wrapDynamicScript(t, []byte{byte(opcode.ABORT)}, callflag.All)
		e.InvokeScriptCheckFAULT(t, script, []atipicialtest.Signer{acc}, "ABORT")
	})
	t.Run("internal call", func(t *testing.T) {
		...
		e.InvokeScriptCheckHALT(t, script, []atipicialtest.Signer{acc}, stackitem.Make(8))
	})
	t.Run("forbidden internal call", func(t *testing.T) {
		script, err := smartcontract.CreateCallScript(e.NativeHash(t, nativenames.Atipicial), "decimals")
		require.NoError(t, err)
		script = wrapDynamicScript(t, script, callflag.ReadStates)
		e.InvokeScriptCheckFAULT(t, script, []atipicialtest.Signer{acc}, "missing call flags")
	})
	t.Run("internal state-changing call", func(t *testing.T) {
		script, err := smartcontract.CreateCallScript(e.NativeHash(t, nativenames.Atipicial), "transfer", acc.ScriptHash(), acc.ScriptHash(), 1, nil)
		require.NoError(t, err)
	...
	t.Run("exception", func(t *testing.T) {
		...
		e.InvokeScriptCheckHALT(t, script, []atipicialtest.Signer{acc}, stackitem.Make("exception"))
	})
}
...
func TestGetAddressVersion(t *testing.T) {
	...
	require.NoError(t, w.Err)
	e.InvokeScriptCheckHALT(t, w.Bytes(), []atipicialtest.Signer{acc}, stackitem.NewBigInteger(big.NewInt(int64(address.ATIPICIALPrefix))))
}
...
func TestGetNetwork(t *testing.T) {
	...
	require.NoError(t, w.Err)
	e.InvokeScriptCheckHALT(t, w.Bytes(), []atipicialtest.Signer{acc}, stackitem.NewBigInteger(big.NewInt(int64(bc.GetConfig().Magic))))
}
...
func TestGetRandom_DifferentTransactions(t *testing.T) {
	...
	tx1 := e.PrepareInvocation(t, script, []atipicialtest.Signer{e.Validator}, bc.BlockHeight()+1)
	tx2 := e.PrepareInvocation(t, script, []atipicialtest.Signer{e.Validator}, bc.BlockHeight()+1)
...
// https://github.com/atipicial-project/atipicial/blob/master/tests/atipicial.UnitTests/SmartContract/UT_ApplicationEngine.Runtime.cs
...
func TestNotify(t *testing.T) {
	...
	newIC := func(name string, args any) *interop.Context {
		...
		require.NoError(t, err)
		ic.VM.LoadAEFMethod(&cs.AEF, &cs.Manifest, caller, cs.Hash, callflag.NoneFlag, true, 0, -1, nil, nil, false)
...
func TestSystemRuntimeNotify_HFBasilisk(t *testing.T) {
	...
	ne, err := aef.NewFile(script.Bytes())
	require.NoError(t, err)
