...
func Platform(ic *interop.Context) error {
	ic.VM.Estack().PushItem(stackitem.NewByteArray([]byte("ATC")))
	return nil
}
...
// in atipicial-go the only meaningful thing to do here is to log.
...
// BurnGas burns GAS to beaefit Atipicial ecosystem.
