...
func NewNotary() *Notary {
	n := &Notary{ContractMD: *interop.NewContractMD(nativenames.Notary, nativeids.Notary, func(m *manifest.Manifest, hf config.Hardfork) {
		m.SupportedStandards = []string{manifest.AEP27StandardName}
		if hf.Cmp(config.HFFaun) >= 0 {
			m.SupportedStandards = append(m.SupportedStandards, manifest.AEP30StandardName)
		}
	})}
...
func (n *Notary) OnPersist(ic *interop.Context) error {
	...
	for _, notary := range notaries {
		n.GAS.MintDeferrable(ic, notary.GetScriptHash(), singleReward, false, func() {}) // no onAEP17Payment call => no source of asynchronous execution.
	}
	return nil
}
