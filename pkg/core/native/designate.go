...
func (s *Designate) InitializeCache(_ interop.IsHardforkEnabled, blockHeight uint32, d *dao.Simple) error {
	cache := &DesignationCache{}
	roles := []noderoles.Role{noderoles.Oracle, noderoles.AtipicialFsAlphabet, noderoles.StateValidator, noderoles.P2PNotary}
	for _, r := range roles {
...
func (s *Designate) notifyServicesInternal(dao *dao.Simple, force bool) {
	...
	s.notifyRoleChanged(&cache.atipicialfsAlphabet, noderoles.AtipicialFsAlphabet)
...
func (s *Designate) updateCachedRoleData(cache *DesignationCache, d *dao.Simple, r noderoles.Role) error {
	...
	case noderoles.StateValidator:
		v = &cache.stateVals
	case noderoles.AtipicialFsAlphabet:
		v = &cache.atipicialfsAlphabet
	case noderoles.P2PNotary:
		v = &cache.notaries
	}
...
func getCachedRoleData(cache *DesignationCache, r noderoles.Role) *roleData {
	...
	case noderoles.AtipicialFsAlphabet:
		return &cache.atipicialfsAlphabet
	case noderoles.P2PNotary:
...
func (s *Designate) DesignateAsRole(ic *interop.Context, r noderoles.Role, pubs keys.PublicKeys) error {
	...
	if ic.Trigger != trigger.OnPersist {
		h := s.ATC.GetCommitteeAddress(ic.DAO)
