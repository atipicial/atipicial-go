...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/util/bitfield"
...
type ManagementCache struct {
	...
	// aep11 is a map of AEP11-compliant contracts which is updated with every PostPersist.
	aep11 map[util.Uint160]struct{}
	// aep17 is a map of AEP-17-compliant contracts which is updated with every PostPersist.
	aep17 map[util.Uint160]struct{}
}
...
func (c *ManagementCache) Copy() dao.NativeContractCache {
	cp := &ManagementCache{
		...
		aep11:     maps.Clone(c.aep11),
		aep17:     maps.Clone(c.aep17),
	}
	return cp
}
...
func NewManagement() *Management {
	...
	desc = NewDescriptor("deploy", smartcontract.ArrayType,
		manifest.NewParameter("aefFile", smartcontract.ByteArrayType),
...
// getAefAndManifestFromItems converts input arguments into AEF and manifest
// adding an appropriate deployment GAS price and sanitizing inputs.
func (m *Management) getAefAndManifestFromItems(ic *interop.Context, args []stackitem.Item, isDeploy bool) (*aef.File, *manifest.Manifest, error) {
	aefBytes, err := getLimitedSlice(args[0], math.MaxInt32) // Upper limits are checked during AEF deserialization.
	if err != nil {
		return nil, nil, fmt.Errorf("invalid AEF file: %w", err)
	}
	...
	gas := ic.BaseStorageFee() * int64(len(aefBytes)+len(manifestBytes))
	if isDeploy {
	...
	var resAef *aef.File
	if aefBytes != nil {
		nf, err := aef.FileFromBytes(aefBytes)
		if err != nil {
			return nil, nil, fmt.Errorf("invalid AEF file: %w", err)
		}
		resAef = &nf
...
func (m *Management) deployWithDataDeferrable(ic *interop.Context, args []stackitem.Item, popArgsPushRes func(res stackitem.Item)) {
	aeff, manif, err := m.getAefAndManifestFromItems(ic, args, true)
	if err != nil {
		panic(err)
	}
	if aeff == nil {
		panic(errors.New("no valid AEF provided"))
	}
	if manif == nil {
	...
	newcontract, err := m.Deploy(ic, ic.Tx.Sender(), aeff, manif)
	if err != nil {
		panic(err)
	}
...
func markUpdated(d *dao.Simple, managementID int32, hash util.Uint160, cs *state.Contract) {
	...
	delete(cache.aep11, hash)
	delete(cache.aep17, hash)
	if cs == nil {
...
func (m *Management) Deploy(ic *interop.Context, sender util.Uint160, aeff *aef.File, manif *manifest.Manifest) (*state.Contract, error) {
	h := state.CreateContractHash(sender, aeff.Checksum, manif.Name)
	...
	err = checkScriptAndMethods(ic, aeff.Script, manif.ABI.Methods)
	if err != nil {
		return nil, err
	}
...
func (m *Management) updateWithDataDeferrable(ic *interop.Context, args []stackitem.Item, popArgsPushRes func(res stackitem.Item)) {
	aeff, manif, err := m.getAefAndManifestFromItems(ic, args, false)
	if err != nil {
		panic(err)
	}
	if aeff == nil && manif == nil {
		panic(errors.New("both AEF and manifest are nil"))
	}
	contract, err := m.Update(ic, ic.VM.GetCallingScriptHash(), aeff, manif)
	if err != nil {
		panic(err)
	}
...
// It doesn't run _deploy method and doesn't emit notification.
func (m *Management) Update(ic *interop.Context, hash util.Uint160, aeff *aef.File, manif *manifest.Manifest) (*state.Contract, error) {
	...
	// if AEF was provided, update the contract script
	if aeff != nil {
		contract.AEF = *aeff
	}
	...
	err = checkScriptAndMethods(ic, contract.AEF.Script, contract.Manifest.ABI.Methods)
	if err != nil {
		return nil, err
	}
...
// updateContractCache saves the contract in the common and AEP-related caches. It's
...
func updateContractCache(cache *ManagementCache, cs *state.Contract) {
	...
	if cs.Manifest.IsStandardSupported(manifest.AEP11StandardName) {
		cache.aep11[cs.Hash] = struct{}{}
	}
	if cs.Manifest.IsStandardSupported(manifest.AEP17StandardName) {
		cache.aep17[cs.Hash] = struct{}{}
	}
}
...
func (m *Management) InitializeCache(_ interop.IsHardforkEnabled, blockHeight uint32, d *dao.Simple) error {
	cache := &ManagementCache{
		...
		aep11:     make(map[util.Uint160]struct{}),
		aep17:     make(map[util.Uint160]struct{}),
	}
...
// GetAEP11Contracts returns hashes of all deployed contracts that support AEP-11 standard. The list
// is updated every PostPersist, so until PostPersist is called, the result for the previous block
// is returned.
func (m *Management) GetAEP11Contracts(d *dao.Simple) []util.Uint160 {
	cache := d.GetROCache(m.ID).(*ManagementCache)
	result := make([]util.Uint160, 0, len(cache.aep11))
	for h := range cache.aep11 {
		result = append(result, h)
	}
	return result
}

// GetAEP17Contracts returns hashes of all deployed contracts that support AEP-17 standard. The list
// is updated every PostPersist, so until PostPersist is called, the result for the previous block
// is returned.
func (m *Management) GetAEP17Contracts(d *dao.Simple) []util.Uint160 {
	cache := d.GetROCache(m.ID).(*ManagementCache)
	result := make([]util.Uint160, 0, len(cache.aep17))
	for h := range cache.aep17 {
		result = append(result, h)
	}
	return result
}
