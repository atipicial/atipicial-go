...
// aep17TokenNative represents a AEP-17 token contract.
type aep17TokenNative struct {
	interop.ContractMD
...
type IGASMinter interface {
	...
	// optionally calls onAEP17Payment method. It always either runs or schedules
...
func (c *aep17TokenNative) Metadata() *interop.ContractMD {
	return &c.ContractMD
}

func newAEP17Native(name string, id int32, onManifestConstruction func(m *manifest.Manifest, hf config.Hardfork)) *aep17TokenNative {
	n := &aep17TokenNative{ContractMD: *interop.NewContractMD(name, id, func(m *manifest.Manifest, hf config.Hardfork) {
		m.SupportedStandards = []string{manifest.AEP17StandardName}
...
func (c *aep17TokenNative) Initialize(_ *interop.Context) error {
	return nil
}

func (c *aep17TokenNative) Symbol(_ *interop.Context, _ []stackitem.Item) stackitem.Item {
	return stackitem.NewByteArray([]byte(c.symbol))
}

func (c *aep17TokenNative) Decimals(_ *interop.Context, _ []stackitem.Item) stackitem.Item {
	return stackitem.NewBigInteger(big.NewInt(c.decimals))
}

func (c *aep17TokenNative) TotalSupply(ic *interop.Context, _ []stackitem.Item) stackitem.Item {
	_, supply := c.getTotalSupply(ic.DAO)
	return stackitem.NewBigInteger(supply)
}

func (c *aep17TokenNative) getTotalSupply(d *dao.Simple) (state.StorageItem, *big.Int) {
...
func (c *aep17TokenNative) saveTotalSupply(d *dao.Simple, si state.StorageItem, supply *big.Int) {
	d.PutBigInt(c.ID, totalSupplyKey, supply)
}

func (c *aep17TokenNative) transferDeferrable(ic *interop.Context, args []stackitem.Item, popArgsPushRes func(res stackitem.Item)) {
...
// postTransfer emits Transfer notification, calls onAEP17Payment (if so) and schedules
...
func (c *aep17TokenNative) postTransfer(ic *interop.Context, from, to *util.Uint160, amount *big.Int,
	...
	continuation := func() {
		...
		// Execute a part of Mint, enqueue context with onAEP17Payment and return.
	...
	err = contract.CallFromNative(ic, c.Hash, cs, manifest.MethodOnAEP17Payment, args, false, func(v *vm.VM) {
		// onAEP17Payment returns void => no need to pop the result from the parent's stack.
		continuation()
	})
	if err != nil {
		panic(err)
...
func (c *aep17TokenNative) emitTransfer(ic *interop.Context, from, to *util.Uint160, amount *big.Int) error {
...
func (c *aep17TokenNative) updateAccBalance(ic *interop.Context, acc util.Uint160, amount *big.Int, requiredBalance *big.Int) (*gasDistribution, error) {
...
func (c *aep17TokenNative) balanceOf(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	h := toUint160(args[0])
	return stackitem.NewBigInteger(c.balanceOfInternal(ic.DAO, h))
}

func (c *aep17TokenNative) balanceOfInternal(d *dao.Simple, h util.Uint160) *big.Int {
...
func (c *aep17TokenNative) MintDeferrable(ic *interop.Context, h util.Uint160, amount *big.Int, callOnPayment bool, continuation func()) {
	if amount.Sign() == 0 {
		continuation()
		return
	}
...
func (c *aep17TokenNative) Burn(ic *interop.Context, h util.Uint160, amount *big.Int) {
	if amount.Sign() == 0 {
		return
	}
	amount.Neg(amount)
	...
	amount.Neg(amount)
	c.postTransfer(ic, &h, nil, amount, stackitem.Null{}, false, func(res stackitem.Item) {}, dist) // no onAEP17Payment call => no source of asynchronous execution.
}

func (c *aep17TokenNative) addTokens(ic *interop.Context, h util.Uint160, amount *big.Int) *gasDistribution {
	if amount.Sign() == 0 {
		return nil
	}
