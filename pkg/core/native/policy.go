...
// (the only case is native cache initialization pipeline, where native Atipicial cache
...
func (p *Policy) BlockAccountInternalDeferrable(ic *interop.Context, hash util.Uint160, handleRes func(res bool)) {
	...
	if ic.IsHardforkEnabled(config.HFFaun) {
		var continuationScheduled, _ = p.ATC.RevokeVotesDeferrable(ic, hash, continuation)
...
func (p *Policy) recoverFundDeferrable(ic *interop.Context, args []stackitem.Item, popArgsPushRes func(res stackitem.Item)) {
	...
	if !p.ATC.CheckAlmostFullCommittee(ic) {
		panic("invalid committee signature")
	}
	...
	if !cs.Manifest.IsStandardSupported("AEP-17") {
		panic(fmt.Errorf("contract %s does not support AEP-17", token.StringLE()))
	}
