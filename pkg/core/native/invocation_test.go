...
func TestNativeContract_InvokeInternal(t *testing.T) {
	...
	t.Run("fail, bad current script hash", func(t *testing.T) {
		...
		v.LoadScriptWithHash(clState.AEF.Script, fakeH, callflag.All)
		input := []byte{1, 2, 3, 4}
	...
	/*
		t.Run("fail, bad NativeUpdateHistory height", func(t *testing.T) {
			...
			require.NoError(t, err)
			v := ic.SpawnVM()
			v.LoadScriptWithHash(clState.AEF.Script, clState.Hash, callflag.All) // hash is not affected by native update history
			input := []byte{1, 2, 3, 4}
			v.Estack().PushVal(input)
			v.Context().Jump(md.Offset)
	...
	*/
	...
	t.Run("fail, bad call flag", func(t *testing.T) {
		...
		require.NoError(t, err)
		v := ic.SpawnVM()
		v.LoadScriptWithHash(manState.AEF.Script, manState.Hash, callflag.States|callflag.AllowNotify)
		input := []byte{1, 2, 3, 4}
		v.Estack().PushVal(input)
		v.Estack().PushVal(input)
	...
	t.Run("good, pre-aspidochelone deploy", func(t *testing.T) {
		...
		err = v.Run()
		require.Error(t, err)
		require.True(t, strings.Contains(err.Error(), "invalid AEF file"))
	})
	...
	t.Run("success", func(t *testing.T) {
		...
		require.NoError(t, err)
		v := ic.SpawnVM()
		v.LoadScriptWithHash(clState.AEF.Script, clState.Hash, callflag.All)
		input := []byte{1, 2, 3, 4}
		v.Estack().PushVal(input)
		v.Context().Jump(md.Offset)
...
func TestNativeContract_InvokeOtherContract(t *testing.T) {
	...
	cs.Hash = state.CreateContractHash(validator.ScriptHash(), cs.AEF.Checksum, cs.Manifest.Name) // set proper hash
	...
	require.NoError(t, err)
	managementInvoker.Invoke(t, si, "deploy", aefB, manifB)

	t.Run("non-native, no return", func(t *testing.T) {
		// `onAEP17Payment` will be invoked on test contract from GAS contract.
