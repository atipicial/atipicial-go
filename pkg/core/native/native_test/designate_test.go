...
func newDesignateClient(t *testing.T) *atipicialtest.ContractInvoker {
...
func TestDesignate_DesignateAsRole(t *testing.T) {
	...
	t.Run("atipicialfs", func(t *testing.T) {
		priv, err := keys.NewPrivateKey()
		require.NoError(t, err)
		pubs = keys.PublicKeys{priv.PublicKey()}
		setNodesByRole(t, designateInvoker, true, noderoles.AtipicialFsAlphabet, pubs, nil)
		checkNodeRoles(t, designateInvoker, true, noderoles.AtipicialFsAlphabet, e.Chain.BlockHeight()+1, pubs)
	})
