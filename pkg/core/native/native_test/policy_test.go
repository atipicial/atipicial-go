...
func newPolicyClient(t *testing.T) *atipicialtest.ContractInvoker {
...
func TestPolicy_BlockedAccounts(t *testing.T) {
	...
	t.Run("block-unblock contract", func(t *testing.T) {
		committeeInvoker.InvokeFail(t, "cannot block native contract", "blockAccount", c.NativeHash(t, nativenames.Atipicial))

		helper := atipicialtest.CompileFile(t, c.CommitteeHash, "./helpers/policyhelper", "./helpers/policyhelper/policyhelper.yml")
	...
	t.Run("block voter", func(t *testing.T) {
		...
		pub := candidate.(atipicialtest.SingleSigner).Account().PublicKey()

		// Transfer some ATC to the account.
		tx := e.NewTx(t, []atipicialtest.Signer{e.Validator}, e.NativeHash(t, nativenames.Atipicial), "transfer", e.Validator.ScriptHash(), acc.ScriptHash(), 1_000, nil)
		e.AddNewBlock(t, tx)
		e.CheckHalt(t, tx.Hash())

		// Register a candidate and vote.
		g := c.NewInvoker(nativehashes.AtipicialDollar, acc, candidate)
		n := c.NewInvoker(nativehashes.AtipicialCoin, acc)
		nCommittee := c.NewInvoker(nativehashes.AtipicialCoin, e.Committee)
		g.Invoke(t, true, "transfer", acc.ScriptHash(), nativehashes.AtipicialCoin, 1000*native.GASFactor, pub.Bytes())
...
func TestPolicy_ExecPicoFeeFactor_InteropAPI(t *testing.T) {
	...
	src := `package execfeefactor
		import "github.com/atipicial/atipicial-go/pkg/interop/native/policy"
		func GetFactor() int {
...
func TestPolicy_RecoverFunds(t *testing.T) {
	...
	committeeInvoker.InvokeFail(t, "method not found: recoverFund/2", "recoverFund", unlucky.ScriptHash(), nativehashes.AtipicialDollar)
	...
	// Invalid signer
	randomInvoker.InvokeFail(t, "invalid committee signature", "recoverFund", unlucky.ScriptHash(), nativehashes.AtipicialDollar)

	// Non-blocked account
	committeeInvoker.InvokeFail(t, "account is not blocked", "recoverFund", util.Uint160{1, 2, 3}, nativehashes.AtipicialDollar)

	// Locked recovery.
	committeeInvoker.InvokeFail(t, "funds recovery request must be signed at least 1 year ago; remaining time is", "recoverFund", unlucky.ScriptHash(), nativehashes.AtipicialDollar)
	...
	tx := c.PrepareInvoke(t, "recoverFund", unlucky.ScriptHash(), nativehashes.AtipicialDollar)
	...
	// Double-recovery.
	c.Invoke(t, stackitem.Make(false), "recoverFund", unlucky.ScriptHash(), nativehashes.AtipicialDollar)
...
func TestPolicy_RecoverFundsInteropAPI(t *testing.T) {
	...
	ctrInvoker.Invoke(t, stackitem.NewBool(true), "recoverFunds", unlucky.ScriptHash(), nativehashes.AtipicialDollar)
}
