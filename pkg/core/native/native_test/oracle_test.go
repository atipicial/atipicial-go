...
func newOracleClient(t *testing.T) *atipicialtest.ContractInvoker {
...
func putOracleRequest(t *testing.T, oracleInvoker *atipicialtest.ContractInvoker,
...
func TestOracle_Request(t *testing.T) {
	...
	oracleNodeKey := oracleNode.(atipicialtest.SingleSigner).Account().PublicKey()
	designationCommitteeInvoker.Invoke(t, stackitem.Null{}, "designateAsRole", int(noderoles.Oracle), []any{oracleNodeKey.Bytes()})
	err = oracleNode.(atipicialtest.SingleSigner).Account().ConvertMultisig(1, []*keys.PublicKey{oracleNodeKey})
	require.NoError(t, err)
	oracleNodeMulti := atipicialtest.NewMultiSigner(oracleNode.(atipicialtest.SingleSigner).Account())
