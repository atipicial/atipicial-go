...
func TestCryptoLib_KoblitzVerificationScript(t *testing.T) {
	check := func(
		...
		// Firstly, we need to build the  user's account address based on the user's public key.
		...
		atipicialtest.AddNetworkFee(t, e.Chain, tx)
		e.AddSystemFee(tx, -1)
...
func buildKoblitzVerificationScript(t *testing.T, pub *keys.PublicKey) []byte {
	...
	// ATIPICIAL-GO-VM > loadbase64 ABhQDCECoIi/qx5LS+3n1GJFcoYbQByyDDsU6QaHvYhiJypOYWZBxfug4AMAAAAAAQAAAJ4UjUEtUQgwEM6LFMAfDA92ZXJpZnlXaXRoRUNEc2EMFBv1dasRiWiEE2EKNaEohs3gtmxyQWJ9W1I=
	// READY: loaded 110 instructions
	// ATIPICIAL-GO-VM 0 > ops
...
func buildKoblitzVerificationScriptSimpleSingleHash(t *testing.T, pub *keys.PublicKey) []byte {
	...
	// ATIPICIAL-GO-VM > loadbase64 ABZQDCEDY9ekgSWnbN6m4JjJ8SjoKSDtQo5ftMrx1/gcFsrQwgVBxfug4EEtUQgwEM6LFMAfDA92ZXJpZnlXaXRoRUNEc2EMFBv1dasRiWiEE2EKNaEohs3gtmxyQWJ9W1I=
...
func buildKoblitzVerificationScriptSimpleSingleHashStaticMagic(t *testing.T, pub *keys.PublicKey) []byte {
	...
	// ATIPICIAL-GO-VM > loadbase64 ABZQDCECluEwgK3pKiq3IjOMKiSe6Ng6FPZJxoMhZkFl8GvREL0AKkEtUQgwEM6LFMAfDA92ZXJpZnlXaXRoRUNEc2EMFBv1dasRiWiEE2EKNaEohs3gtmxyQWJ9W1I=
...
func buildKoblitzVerificationScriptSimple(t *testing.T, pub *keys.PublicKey) []byte {
	...
	// ATIPICIAL-GO-VM 0 > loadbase64 ABZQDCEDp38Tevu0to16RQqloo/jNfgExYmoCElLS2JuuYcH831Bxfug4EEtUQgwEM6LEcAfDAZzaGEyNTYMFBv1dasRiWiEE2EKNaEohs3gtmxyQWJ9W1IUwB8MD3ZlcmlmeVdpdGhFQ0RzYQwUG/V1qxGJaIQTYQo1oSiGzeC2bHJBYn1bUg==
...
func buildKoblitzVerificationScriptCompat(t *testing.T, pub *keys.PublicKey) []byte {
	...
	// ATIPICIAL-GO-VM 0 > loadbase64 ABZQDCECYn75w2MePMuPvExbbEnjjM7eWnmvseGwcI+7lYp4AtdBxfug4EoB/wCRUBipSgH/AJFQGKlKAf8AkVAYqQH/AJEUiEoTE1LQShITUtBKERNS0EoQE1LQQS1RCDAQzosRwB8MBnNoYTI1NgwUG/V1qxGJaIQTYQo1oSiGzeC2bHJBYn1bUhTAHwwPdmVyaWZ5V2l0aEVDRHNhDBQb9XWrEYlohBNhCjWhKIbN4LZsckFifVtS
...
func buildKoblitzInvocationScript(t *testing.T, signatures [][]byte) []byte {
	...
	// ATIPICIAL-GO-VM > loadbase64 DEBMGKU/MdSizlzaVNDUUbd1zMZQJ43eTaZ4vBCpmkJ/wVh1TYrAWEbFyHhkqq+aYxPCUS43NKJdJTXavcjB8sTP
	...
	// ATIPICIAL-GO-VM > loadbase64 
...
// constructMessageCompat constructs message for signing that does not follow  rules,
...
// constructMessageCompat constructs message for signing following the  rules:
//
//	sha256([4-bytes-network-magic-LE, txHash-bytes-BE])
...
func TestCryptoLib_KoblitzMultisigVerificationScript(t *testing.T) {
	check := func(
		...
		// Firstly, we need to build the  multisig account address based on the users' public keys.
...
func buildKoblitzMultisigVerificationScript(t *testing.T, m int, pubs keys.PublicKeys) []byte {
	...
	// In fact, the following algorithm is implemented via AtipicialVM instructions:
	//
