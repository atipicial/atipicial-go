...
func newNotaryClient(t *testing.T) *atipicialtest.ContractInvoker {
...
func TestNotary_Pipeline(t *testing.T) {
	...
	// `onPayment`: bad token
	atipicialCommitteeInvoker.InvokeFail(t, "only GAS can be accepted for deposit", "transfer", multisigHash, notaryHash, int64(1), &notary.OnAEP17PaymentData{Till: uint32(depositLock)})

	// `onPayment`: insufficient first deposit
	gasCommitteeInvoker.InvokeFail(t, "first deposit can not be less than", "transfer", multisigHash, notaryHash, int64(2*feePerKey-1), &notary.OnAEP17PaymentData{Till: uint32(depositLock)})
	...
	gasCommitteeInvoker.InvokeFail(t, "`till` shouldn't be less than the chain's height", "transfer", multisigHash, notaryHash, 2*feePerKey, &notary.OnAEP17PaymentData{})

	// `onPayment`: good
	gasCommitteeInvoker.Invoke(t, true, "transfer", multisigHash, notaryHash, 2*feePerKey, &notary.OnAEP17PaymentData{Till: uint32(depositLock)})
	...
	gasCommitteeInvoker.Invoke(t, true, "transfer", multisigHash, notaryHash, feePerKey, &notary.OnAEP17PaymentData{Account: &multisigHash, Till: uint32(depositLock + 1)})
	...
	gasCommitteeInvoker.InvokeFail(t, "`till` shouldn't be less than the previous value", "transfer", multisigHash, notaryHash, int64(0), &notary.OnAEP17PaymentData{Account: &multisigHash, Till: uint32(depositLock)})
	checkBalanceOf(t, notaryHash, 3*feePerKey)
	notaryCommitteeInvoker.Invoke(t, depositLock+1, "expirationOf", multisigHash)

	// `onPayment`: empty payment, should fail because `till` less than the chain height
	gasCommitteeInvoker.InvokeFail(t, "`till` shouldn't be less than the chain's height", "transfer", multisigHash, notaryHash, int64(0), &notary.OnAEP17PaymentData{Account: &multisigHash, Till: uint32(1)})
	checkBalanceOf(t, notaryHash, 3*feePerKey)
	notaryCommitteeInvoker.Invoke(t, depositLock+1, "expirationOf", multisigHash)

	// `onPayment`: empty payment, should successfully update `till`
	gasCommitteeInvoker.Invoke(t, true, "transfer", multisigHash, notaryHash, int64(0), &notary.OnAEP17PaymentData{Account: &multisigHash, Till: uint32(depositLock + 2)})
	checkBalanceOf(t, notaryHash, 3*feePerKey)
	...
	gasCommitteeInvoker.Invoke(t, true, "transfer", multisigHash, notaryHash, 2*feePerKey, &notary.OnAEP17PaymentData{Account: &accHash, Till: uint32(math.MaxUint32 - 1)})
	checkBalanceOf(t, notaryHash, 2*feePerKey)
	...
	gasCommitteeInvoker.Invoke(t, true, "transfer", multisigHash, notaryHash, feePerKey, &notary.OnAEP17PaymentData{Account: &accHash, Till: uint32(math.MaxUint32 - 1)})
	checkBalanceOf(t, notaryHash, 3*feePerKey)
...
func TestNotary_MaliciousWithdrawal(t *testing.T) {
	...
	count := 3
	for range count {
		gasCommitteeInvoker.Invoke(t, true, "transfer", multisigHash, notaryHash, 2*feePerKey, &notary.OnAEP17PaymentData{Account: new(random.Uint160()), Till: e.Chain.BlockHeight() + 2})
	}
	...
	src := `package foo
		import (
			"github.com/atipicial/atipicial-go/pkg/interop/native/notary"
	...
	ctr := atipicialtest.CompileSource(t, e.CommitteeHash, strings.NewReader(src), &compiler.Options{Name: "Helper", Permissions: []manifest.Permission{{
		Methods: manifest.WildStrings{},
	}}})
	e.DeployContract(t, ctr, nil)
	gasCommitteeInvoker.Invoke(t, true, "transfer", multisigHash, notaryHash, 2*feePerKey, &notary.OnAEP17PaymentData{Account: &ctr.Hash, Till: e.Chain.BlockHeight() + 2})
	...
	maliciousInvoker := e.NewInvoker(notaryHash, notaryCommitteeInvoker.Signers[0], atipicialtest.NewContractSigner(ctr.Hash, func(tx *transaction.Transaction) []any {
		return nil
	}))
...
func TestNotary_NotaryNodesReward(t *testing.T) {
	checkReward := func(nKeys int, nNotaryNodes int, spendFullDeposit bool) {
		...
		gasCommitteeInvoker.Invoke(t, true, "transfer", multisigHash, notaryHash, depositAmount, &notary.OnAEP17PaymentData{Account: &multisigHash, Till: e.Chain.BlockHeight() + 2})
