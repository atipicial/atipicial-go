...
func newLedgerClient(t *testing.T) *atipicialtest.ContractInvoker {
...
func TestLedger_GetTransactionHeight(t *testing.T) {
	...
	hash := e.InvokeScript(t, []byte{byte(opcode.RET)}, []atipicialtest.Signer{c.Committee})
...
func TestLedger_GetTransactionState(t *testing.T) {
	...
	t.Run("good: FAULT", func(t *testing.T) {
		faultedH := e.InvokeScript(t, []byte{byte(opcode.ABORT)}, []atipicialtest.Signer{c.Committee})
...
func TestLedger_GetTransactionSignersInteropAPI(t *testing.T) {
	...
	atipicialtest.AddNetworkFee(t, e.Chain, tx, c.Committee)
	e.AddSystemFee(tx, -1)
	...
	ctr := atipicialtest.CompileSource(t, c.Committee.ScriptHash(), strings.NewReader(src), &compiler.Options{
