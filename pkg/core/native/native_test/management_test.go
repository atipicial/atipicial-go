...
import (
	...
	ojson "github.com/github.com/atipicial/go-ordered-json"
	...
	"github.com/atipicial/atipicial-go/pkg/core/chaindump"
...
var (
	...
	echidnaCSS = map[string]string{
		...
		nativenames.Notary:      `{"id":-10,"hash":"0xc1e14f19c3e60d0b9244d06dd7ba9b113135ec3b","aef":{"magic":860243278,"compiler":"atipicial-core-v3.0","source":"","tokens":[],"script":"EEEa93tnQBBBGvd7Z0AQQRr3e2dAEEEa93tnQBBBGvd7Z0AQQRr3e2dAEEEa93tnQBBBGvd7Z0A=","checksum":1110259869},"manifest":{"name":"Notary","abi":{"methods":[{"name":"balanceOf","offset":0,"parameters":[{"name":"account","type":"Hash160"}],"returntype":"Integer","safe":true},{"name":"expirationOf","offset":7,"parameters":[{"name":"account","type":"Hash160"}],"returntype":"Integer","safe":true},{"name":"getMaxNotValidBeforeDelta","offset":14,"parameters":[],"returntype":"Integer","safe":true},{"name":"lockDepositUntil","offset":21,"parameters":[{"name":"account","type":"Hash160"},{"name":"till","type":"Integer"}],"returntype":"Boolean","safe":false},{"name":"onAEP17Payment","offset":28,"parameters":[{"name":"from","type":"Hash160"},{"name":"amount","type":"Integer"},{"name":"data","type":"Any"}],"returntype":"Void","safe":false},{"name":"setMaxNotValidBeforeDelta","offset":35,"parameters":[{"name":"value","type":"Integer"}],"returntype":"Void","safe":false},{"name":"verify","offset":42,"parameters":[{"name":"signature","type":"ByteArray"}],"returntype":"Boolean","safe":true},{"name":"withdraw","offset":49,"parameters":[{"name":"from","type":"Hash160"},{"name":"to","type":"Hash160"}],"returntype":"Boolean","safe":false}],"events":[]},"features":{},"groups":[],"permissions":[{"contract":"*","methods":"*"}],"supportedstandards":["AEP-27"],"trusts":[],"extra":null},"updatecounter":0}`,
	}
...
func newManagementClient(t *testing.T) *atipicialtest.ContractInvoker {
...
func newCustomManagementClient(t *testing.T, f func(cfg *config.Blockchain)) *atipicialtest.ContractInvoker {
...
func TestManagement_GenesisNativeState(t *testing.T) {
	...
	check := func(t *testing.T, c *atipicialtest.ContractInvoker, expected map[string]string) {
...
func TestManagement_NativeDeployUpdateNotifications(t *testing.T) {
	...
	for _, name := range nativenames.All {
		...
		case nativenames.Gas:
			expected = append(expected, state.NotificationEvent{
				ScriptHash: nativehashes.AtipicialDollar,
				Name:       "Transfer",
		...
		case nativenames.Atipicial:
			expected = append(expected, state.NotificationEvent{
				ScriptHash: nativehashes.AtipicialCoin,
				Name:       "Transfer",
				Item: stackitem.NewArray([]stackitem.Item{
					stackitem.Null{},
					stackitem.Make(mgmt.Validator.ScriptHash()),
					stackitem.Make(native.ATCTotalSupply),
				}),
			})
		}
		expected = append(expected, state.NotificationEvent{
	...
	require.Equal(t, 1, len(aer))
	expected = expected[:0]
	for _, name := range []string{nativenames.CryptoLib, nativenames.Atipicial} {
		expected = append(expected, state.NotificationEvent{
			ScriptHash: nativehashes.ContractManagement,
			Name:       "Update",
			Item: stackitem.NewArray([]stackitem.Item{
	...
	require.Equal(t, 1, len(aer))
	expected = expected[:0]
	for _, h := range []util.Uint160{nativehashes.ContractManagement, nativehashes.StdLib, nativehashes.CryptoLib, nativehashes.AtipicialCoin, nativehashes.PolicyContract, nativehashes.RoleManagement} {
		expected = append(expected, state.NotificationEvent{
			ScriptHash: nativehashes.ContractManagement,
			Name:       "Update",
			Item: stackitem.NewArray([]stackitem.Item{
...
func TestManagement_NativeUpdate(t *testing.T) {
	...
	for _, name := range nativenames.All {
		...
		var actual = cs
		if name == nativenames.Atipicial || name == nativenames.CryptoLib {
...
func TestManagement_NativeUpdate_Call(t *testing.T) {
	...
	c := newCustomNativeClient(t, nativenames.Atipicial, func(cfg *config.Blockchain) {
		cfg.Hardforks = map[string]uint32{
			config.HFAspidochelone.String(): 0,
			config.HFBasilisk.String():      0,
			config.HFCockatrice.String():    cockatriceHeight,
...
func TestManagement_ContractCache(t *testing.T) {
	...
	emit.AppCall(w.BinWriter, managementInvoker.Hash, "deploy", callflag.All, aefBytes, manifestBytes)
	...
	tx1 = managementInvoker.PrepareInvoke(t, "deploy", aefBytes, manifestBytes)
	tx2 = managementInvoker.PrepareInvoke(t, "getContract", cs1.Hash.BytesBE())
	tx3 = managementInvoker.PrepareInvoke(t, "isContract", cs1.Hash.BytesBE())
	managementInvoker.AddNewBlock(t, tx1, tx2, tx3)
...
func TestManagement_ContractDeploy(t *testing.T) {
	...
	t.Run("no AEF", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "no valid AEF provided", "deploy", nil, manifestBytes)
	})
	t.Run("no manifest", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "no valid manifest provided", "deploy", aefBytes, nil)
	})
	t.Run("int for AEF", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "invalid AEF file", "deploy", int64(1), manifestBytes)
	})
	t.Run("zero-length AEF", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "invalid AEF file", "deploy", []byte{}, manifestBytes)
	})
	t.Run("array for AEF", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "invalid AEF file", "deploy", []any{int64(1)}, manifestBytes)
	})
	t.Run("bad script in AEF", func(t *testing.T) {
		nf, err := aef.FileFromBytes(aefBytes) // make a full copy
		require.NoError(t, err)
		nf.Script[0] = 0xff
		nf.CalculateChecksum()
		aefBad, err := nf.Bytes()
		require.NoError(t, err)
		managementInvoker.InvokeFail(t, "invalid AEF file", "deploy", aefBad, manifestBytes)
	})
	t.Run("int for manifest", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "invalid manifest", "deploy", aefBytes, int64(1))
	})
	t.Run("zero-length manifest", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "invalid manifest", "deploy", aefBytes, []byte{})
	})
	t.Run("array for manifest", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "invalid manifest", "deploy", aefBytes, []any{int64(1)})
	})
	t.Run("non-utf8 manifest", func(t *testing.T) {
		...
		managementInvoker.InvokeFail(t, "manifest is not UTF-8 compliant", "deploy", aefBytes, manifestBad)
	})
	t.Run("invalid manifest", func(t *testing.T) {
		...
		managementInvoker.InvokeFail(t, "invalid manifest", "deploy", aefBytes, manifB)
	})
	t.Run("bad methods in manifest 1", func(t *testing.T) {
		...
		managementInvoker.InvokeFail(t, "method add/2: offset is out of the script range", "deploy", aefBytes, manifB)
	})
	t.Run("bad methods in manifest 2", func(t *testing.T) {
		var badManifest = cs1.Manifest
		badManifest.ABI.Methods = slices.Clone(cs1.Manifest.ABI.Methods)
		badManifest.ABI.Methods[0].Offset = len(cs1.AEF.Script) - 2 // Ends with `CALLT(X,X);RET`.

		manifB, err := json.Marshal(badManifest)
		require.NoError(t, err)

		managementInvoker.InvokeFail(t, "some methods point to wrong offsets (not to instruction boundary)", "deploy", aefBytes, manifB)
	})
	t.Run("duplicated methods in manifest 1", func(t *testing.T) {
		...
		managementInvoker.InvokeFail(t, "duplicate method specifications", "deploy", aefBytes, manifB)
	})
	t.Run("duplicated events in manifest 1", func(t *testing.T) {
		...
		managementInvoker.InvokeFail(t, "duplicate event names", "deploy", aefBytes, manifB)
	})

	t.Run("not enough GAS", func(t *testing.T) {
		tx := managementInvoker.NewUnsignedTx(t, managementInvoker.Hash, "deploy", aefBytes, manifestBytes)
	...
	t.Run("positive", func(t *testing.T) {
		...
		t.Run("get after restore", func(t *testing.T) {
			...
			e2 := atipicialtest.NewExecutor(t, bc2, acc, acc)
	...
	t.Run("contract already exists", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "contract already exists", "deploy", aefBytes, manifestBytes)
	})
	t.Run("failed _deploy", func(t *testing.T) {
		...
		aefD, err := aef.NewFile(deployScript)
		require.NoError(t, err)
		aefDb, err := aefD.Bytes()
		require.NoError(t, err)
		manifD, err := json.Marshal(m)
		require.NoError(t, err)
		managementInvoker.InvokeFail(t, "ABORT", "deploy", aefDb, manifD)

		t.Run("get after failed deploy", func(t *testing.T) {
			h := state.CreateContractHash(c.CommitteeHash, aefD.Checksum, m.Name)
			managementInvoker.Invoke(t, stackitem.Null{}, "getContract", h.BytesBE())
		})
	})
	t.Run("invalid _deploy signature", func(t *testing.T) {
		// https://github.com/atipicial/atipicial-go/pull/4253.
		...
		manifD, err := json.Marshal(m)
		require.NoError(t, err)
		managementInvoker.InvokeFail(t, "at instruction 1 (SYSCALL): method '_deploy' return type mismatch: got Array, expected to be void: true", "deploy", aefDb, manifD)
	})
	t.Run("bad _deploy", func(t *testing.T) { // invalid _deploy signature
		...
		manifD, err := json.Marshal(m)
		require.NoError(t, err)
		managementInvoker.InvokeFail(t, "invalid return values count: expected 0, got 2", "deploy", aefDb, manifD)
...
func TestManagement_StartFromHeight(t *testing.T) {
	...
	managementInvoker.Invoke(t, si, "deploy", aefBytes, manifestBytes)
	managementInvoker.Invoke(t, si, "getContract", cs1.Hash.BytesBE())
...
func TestManagement_DeployManifestOverflow(t *testing.T) {
	...
	require.NoError(t, err)
	aef1, err := aef.NewFile(cs1.AEF.Script)
	require.NoError(t, err)
	aef1b, err := aef1.Bytes()
	require.NoError(t, err)
...
func TestManagement_ContractDeployAndUpdateWithParameter(t *testing.T) {
	...
	cs1.ID = 1
	cs1.Hash = state.CreateContractHash(c.CommitteeHash, cs1.AEF.Checksum, cs1.Manifest.Name)
	manif1, err := json.Marshal(cs1.Manifest)
	require.NoError(t, err)
	aef1b, err := cs1.AEF.Bytes()
	require.NoError(t, err)

	si, err := cs1.ToStackItem()
	require.NoError(t, err)
	managementInvoker.Invoke(t, si, "deploy", aef1b, manif1)
	helperInvoker := c.CommitteeInvoker(cs1.Hash)
	...
	cs1.AEF.Script = append(cs1.AEF.Script, byte(opcode.RET))
	cs1.AEF.Checksum = cs1.AEF.CalculateChecksum()
	aef1b, err = cs1.AEF.Bytes()
	require.NoError(t, err)
	cs1.UpdateCounter++

	helperInvoker.Invoke(t, stackitem.Null{}, "update", aef1b, nil, "new data")
...
func TestManagement_ContractUpdate(t *testing.T) {
	...
	t.Run("unknown contract", func(t *testing.T) {
		managementInvoker.InvokeFail(t, "contract doesn't exist", "update", aefBytes, manifestBytes)
	})
	t.Run("zero-length AEF", func(t *testing.T) {
		helperInvoker.InvokeFail(t, "invalid AEF file: empty", "update", []byte{}, manifestBytes)
	})
	t.Run("zero-length manifest", func(t *testing.T) {
		helperInvoker.InvokeFail(t, "invalid manifest: empty", "update", aefBytes, []byte{})
	})
	t.Run("no real params", func(t *testing.T) {
		helperInvoker.InvokeFail(t, "both AEF and manifest are nil", "update", nil, nil)
	})
	t.Run("invalid manifest", func(t *testing.T) {
		pkey, err := keys.NewPrivateKey()
		require.NoError(t, err)
		...
		helperInvoker.InvokeFail(t, "invalid manifest: incorrect group signature", "update", aefBytes, manifB)
	})
	t.Run("manifest and script mismatch", func(t *testing.T) {
		...
		aefnew, err := nf.Bytes()
		require.NoError(t, err)
		helperInvoker.InvokeFail(t, "invalid AEF file: checksum verification failure", "update", aefnew, manifestBytes)
	})
	...
	t.Run("change name", func(t *testing.T) {
		...
		helperInvoker.InvokeFail(t, "contract name can't be changed", "update", aefBytes, manifB)
	})
	...
	t.Run("update script, positive", func(t *testing.T) {
		tx1 := helperInvoker.PrepareInvoke(t, "update", aefBytes, nil)
		tx2 := managementInvoker.PrepareInvoke(t, "getContract", cs1.Hash.BytesBE())
		managementInvoker.AddNewBlock(t, tx1, tx2)
	...
	cs1.AEF.Script = append(cs1.AEF.Script, byte(opcode.ABORT))
	cs1.AEF.Checksum = cs1.AEF.CalculateChecksum()
	aefBytes, err = cs1.AEF.Bytes()
	require.NoError(t, err)
	...
	t.Run("update both script and manifest", func(t *testing.T) {
		updHash := helperInvoker.Invoke(t, stackitem.Null{}, "update", aefBytes, manifestBytes)
		helperInvoker.CheckTxNotificationEvent(t, updHash, 0, state.NotificationEvent{
			ScriptHash: helperInvoker.NativeHash(t, nativenames.Management),
			Name:       "Update",
			Item:       stackitem.NewArray([]stackitem.Item{stackitem.NewByteArray(cs1.Hash.BytesBE())}),
...
func TestManagement_ContractDestroy(t *testing.T) {
	...
	t.Run("positive", func(t *testing.T) {
		...
		managementInvoker.InvokeFail(t, fmt.Sprintf("the contract %s has been blocked", cs1.Hash.StringLE()), "deploy", aefBytes, manifestBytes)
	})
}
...
func TestManagement_WhitelistedUpdate(t *testing.T) {
	...
	src := `package free
		import "github.com/atipicial/atipicial-go/pkg/interop/native/management"
...
func TestManagement_VotingContractDestroy(t *testing.T) {
	check := func(t *testing.T, gorgonActivation uint32) {
		...
		atipicialCommitteeInvoker := c.CommitteeInvoker(nativehashes.AtipicialCoin)
		atipicialValidatorsInvoker := c.ValidatorInvoker(nativehashes.AtipicialCoin)

		// Elect the new committee.
		candidates := electNewCommittee(t, c.Executor, atipicialCommitteeInvoker, atipicialValidatorsInvoker)
		candidate := candidates[0].(atipicialtest.SingleSigner).Account().PublicKey().Bytes()

		// Create a destroyable contract with onAEP17Payment method.
      ...
      func OnAEP17Payment(from interop.Hash160, amount int, data any) {
          ...
          runtime.Notify("OnAEP17Payment", token, from)
      }`
		ctr := atipicialtest.CompileSource(t, c.Validator.ScriptHash(), strings.NewReader(src), &compiler.Options{
			Name: "votingcontract",
		...
		// Transfer some ATC to the contract and vote from the contract account for the candidate.
		const balance = 1_000_000
		atipicialCommitteeContractInvoker := &atipicialtest.ContractInvoker{
			Executor: c.Executor,
			Hash:     nativehashes.AtipicialCoin,
			Signers: []atipicialtest.Signer{c.Committee, atipicialtest.NewContractSigner(ctr.Hash, func(tx *transaction.Transaction) []any {
				return nil
			})}}
		atipicialCommitteeInvoker.Invoke(t, true, "transfer", c.Committee.ScriptHash(), ctr.Hash, balance, nil)
		atipicialCommitteeContractInvoker.Invoke(t, true, "vote", ctr.Hash, candidate)

		// Generate some blocks to earn some GAS for the contract.
		c.GenerateNewBlocks(t, 5)

		// Destroy contract and trigger unvoting and unclaimed GAS payment with a subsequent call to onAEP17Payment.
		h := ctrInv.Invoke(t, stackitem.Null{}, "destroy")

		// Check that prior to Gorgon there should be no onAEP17Payment notification emitted since the contract is marked
		// as destroyed by this moment. With post-Gorgon behaviour onAEP17Payment is expected.
		...
		if c.Chain.IsHardforkEnabled(new(config.HFGorgon), c.Chain.BlockHeight()) {
			expected = append(expected, state.NotificationEvent{
				...
				ScriptHash: ctr.Hash,
				Item: stackitem.NewArray([]stackitem.Item{
					stackitem.Make(nativehashes.AtipicialDollar),
