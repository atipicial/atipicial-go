...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/core/interop/runtime"
	istorage "github.com/atipicial/atipicial-go/pkg/core/interop/storage"
...
// ATC represents ATC native contract.
type ATC struct {
	aep17TokenNative
	gas    IGAS
	Policy IPolicy
...
const (
	// ATCTotalSupply is the total amount of ATC in the system.
	ATCTotalSupply = 100000000
	...
	prefixCandidate = 33
	// prefixVotersCount is a prefix for storing total amount of ATC of voters.
	prefixVotersCount = 1
	...
	// atipicialHolderRewardRatio is a percent of generated GAS that is distributed to ATC holders.
...
var (
	_ interop.Contract        = (*ATC)(nil)
	_ dao.NativeContractCache = (*AtipicialCache)(nil)
)

// Copy implements NativeContractCache interface.
func (c *AtipicialCache) Copy() dao.NativeContractCache {
	cp := &AtipicialCache{}
	copyAtipicialCache(c, cp)
	return cp
}

func copyAtipicialCache(src, dst *AtipicialCache) {
...
// NewATC returns AtipicialCoin native contract.
func NewATC(cfg config.ProtocolConfiguration, gas IGAS) *ATC {
	n := &ATC{gas: gas}
	defer n.BuildHFSpecificMD(n.ActiveIn())

	aep17 := newAEP17Native(nativenames.Atipicial, nativeids.AtipicialCoin, func(m *manifest.Manifest, hf config.Hardfork) {
		if hf.Cmp(config.HFEchidna) >= 0 {
			m.SupportedStandards = append(m.SupportedStandards, manifest.AEP27StandardName)
		}
	})
	aep17.symbol = "ATC"
	aep17.decimals = 0
	aep17.factor = 1
	aep17.incBalance = n.increaseBalance
	aep17.balFromBytes = n.balanceFromBytes
	aep17.gasMinter = gas

	n.aep17TokenNative = *aep17

	err := n.initConfigCache(cfg)
	if err != nil {
		panic(fmt.Errorf("failed to initialize ATC config cache: %w", err))
	}
	...
	md = NewMethodAndPrice(n.onAEP17Payment, 1<<15, callflag.States|callflag.AllowNotify, config.HFEchidna)
	n.AddMethod(md, desc)
...
// Initialize initializes a ATC contract.
func (n *ATC) Initialize(ic *interop.Context, hf *config.Hardfork, newMD *interop.HFSpecificContractMD) error {
	if hf != n.ActiveIn() {
		return nil
	}

	if err := n.aep17TokenNative.Initialize(ic); err != nil {
		return err
	}
	...
	n.MintDeferrable(ic, h, big.NewInt(ATCTotalSupply), false, func() {}) // no onAEP17Payment call => no source of asynchronous execution.
...
// InitializeCache initializes all ATC cache with the proper values from the storage.
...
// interface.
func (n *ATC) InitializeCache(_ interop.IsHardforkEnabled, blockHeight uint32, d *dao.Simple) error {
	cache := &AtipicialCache{
		gasPerVoteCache: make(map[string]big.Int),
		votesChanged:    true,
	}
...
func (n *ATC) ActiveIn() *config.Hardfork {
	return nil
}

func (n *ATC) initConfigCache(cfg config.ProtocolConfiguration) error {
	var err error
...
func (n *ATC) updateCache(cache *AtipicialCache, cvs keysWithVotes, blockHeight uint32) error {
	cache.committee = cvs
...
// of the latest epoch.
func (n *ATC) updateCachedNewEpochValues(d *dao.Simple, cache *AtipicialCache, blockHeight uint32, numOfCNs int) error {
...
func (n *ATC) OnPersist(ic *interop.Context) error {
	if n.cfg.ShouldUpdateCommitteeAt(ic.Block.Index) {
		cache := ic.DAO.GetRWCache(n.ID).(*AtipicialCache)
...
func (n *ATC) PostPersist(ic *interop.Context) error {
	gas := n.GetGASPerBlock(ic.DAO, ic.BlockHeight()+1)
	cache := ic.DAO.GetROCache(n.ID).(*AtipicialCache)
	...
	n.gas.MintDeferrable(ic, pubs[index].GetScriptHash(), committeeReward.Div(committeeReward, big100), false, func() {}) // no onAEP17Payment call => no source of asynchronous execution.
...
func (n *ATC) getLatestGASPerVote(d *dao.Simple, key []byte) big.Int {
	var g big.Int
	cache := d.GetROCache(n.ID).(*AtipicialCache)
...
func (n *ATC) increaseBalance(ic *interop.Context, h util.Uint160, si *state.StorageItem, amount *big.Int, checkBal *big.Int) (*gasDistribution, error) {
	var dist *gasDistribution
	acc, err := state.ATCBalanceFromBytes(*si)
	if err != nil {
		return nil, err
	}
...
func (n *ATC) balanceFromBytes(si *state.StorageItem) (*big.Int, error) {
	acc, err := state.ATCBalanceFromBytes(*si)
	if err != nil {
		return nil, err
	}
...
func (n *ATC) distributeGas(ic *interop.Context, acc *state.ATCBalance) (*big.Int, error) {
...
func (n *ATC) unclaimedGas(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	u := toUint160(args[0])
...
func (n *ATC) getGASPerBlock(ic *interop.Context, _ []stackitem.Item) stackitem.Item {
	gas := n.GetGASPerBlock(ic.DAO, ic.BlockHeight()+1)
	return stackitem.NewBigInteger(gas)
}

func (n *ATC) getSortedGASRecordFromDAO(d *dao.Simple) gasRecord {
...
func (n *ATC) GetGASPerBlock(d *dao.Simple, index uint32) *big.Int {
	cache := d.GetROCache(n.ID).(*AtipicialCache)
	...
	panic("ATC cache not initialized")
}

// GetCommitteeAddress returns address of the committee.
func (n *ATC) GetCommitteeAddress(d *dao.Simple) util.Uint160 {
	cache := d.GetROCache(n.ID).(*AtipicialCache)
	return cache.committeeHash
}

func (n *ATC) CheckCommittee(ic *interop.Context) bool {
...
func (n *ATC) CheckAlmostFullCommittee(ic *interop.Context) bool {
...
func (n *ATC) setGASPerBlock(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	gas := toBigInt(args[0])
...
func (n *ATC) SetGASPerBlock(ic *interop.Context, index uint32, gas *big.Int) error {
...
func (n *ATC) getRegisterPrice(ic *interop.Context, _ []stackitem.Item) stackitem.Item {
	return stackitem.NewBigInteger(big.NewInt(n.getRegisterPriceInternal(ic.DAO)))
}

func (n *ATC) getRegisterPriceInternal(d *dao.Simple) int64 {
	cache := d.GetROCache(n.ID).(*AtipicialCache)
	return cache.registerPrice
}

func (n *ATC) setRegisterPrice(ic *interop.Context, args []stackitem.Item) stackitem.Item {
...
func (n *ATC) dropCandidateIfZero(d *dao.Simple, cache *AtipicialCache, pub *keys.PublicKey, c *candidate) bool {
...
// CalculateBonus calculates amount of gas generated for holding value ATC from start to end block
// and having voted for active committee member.
func (n *ATC) CalculateBonus(ic *interop.Context, acc util.Uint160, end uint32) (*big.Int, error) {
	key := makeAccountKey(acc)
	si := ic.DAO.GetStorageItem(n.ID, key)
	if si == nil {
		return big.NewInt(0), nil
	}
	st, err := state.ATCBalanceFromBytes(si)
	if err != nil {
		return nil, err
	}
...
func (n *ATC) calculateBonus(d *dao.Simple, acc *state.ATCBalance, end uint32) (*big.Int, error) {
	r, err := n.CalculateATCHolderReward(d, &acc.Balance, acc.BalanceHeight, end)
...
// CalculateATCHolderReward return GAS reward for holding `value` of ATC from start to end block.
func (n *ATC) CalculateATCHolderReward(d *dao.Simple, value *big.Int, start, end uint32) (*big.Int, error) {
	...
	res.Mul(res, tmp.SetInt64(atipicialHolderRewardRatio))
	res.Div(res, tmp.SetInt64(100*ATCTotalSupply))
	return res, nil
}

func (n *ATC) registerCandidate(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	pub := toPublicKey(args[0])
	if !ic.IsHardforkEnabled(config.HFEchidna) { // doesn't affect  states but affects execution result, so need to be kept, ref. #4262.
...
func (n *ATC) checkRegisterCandidate(ic *interop.Context, pub *keys.PublicKey) error {
	ok, err := runtime.CheckKeyedWitness(ic, pub)
	if err != nil {
		panic(err)
	} else if !ok {
...
func (n *ATC) onAEP17Payment(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	var (
...
func (n *ATC) RegisterCandidateInternal(ic *interop.Context, pub *keys.PublicKey) error {
	var emitEvent = true
...
func (n *ATC) unregisterCandidate(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	pub := toPublicKey(args[0])
	ok, err := runtime.CheckKeyedWitness(ic, pub)
	if err != nil {
		panic(err)
...
// called outside of VM since it panics on critical errors.
func (n *ATC) UnregisterCandidateInternal(ic *interop.Context, pub *keys.PublicKey) error {
	var err error
...
func (n *ATC) voteDeferrable(ic *interop.Context, args []stackitem.Item, popArgsPushRes func(res stackitem.Item)) {
...
// RevokeVotesDeferrable implements IATC interface. It revokes votes of account h and
// doesn't check the h's witness.
func (n *ATC) RevokeVotesDeferrable(ic *interop.Context, h util.Uint160, continuation func()) (bool, error) {
...
// and an error.
func (n *ATC) voteInternalUncheckedDeferrable(ic *interop.Context, h util.Uint160, pub *keys.PublicKey, continuation func()) (bool, error) {
...
func (n *ATC) ModifyAccountVotes(acc *state.ATCBalance, d *dao.Simple, value *big.Int, isNewVote bool) error {
	cache := d.GetRWCache(n.ID).(*AtipicialCache)
	cache.votesChanged = true
	if acc.VoteTo != nil {
...
func (n *ATC) getCandidates(d *dao.Simple, sortByKey bool, maxNum int) ([]keyWithVotes, error) {
...
// and votes.
func (n *ATC) GetCandidates(d *dao.Simple) ([]state.Validator, error) {
...
func (n *ATC) getCandidatesCall(ic *interop.Context, _ []stackitem.Item) stackitem.Item {
...
func (n *ATC) getCommitteeAddress(ic *interop.Context, _ []stackitem.Item) stackitem.Item {
	return stackitem.NewByteArray(n.GetCommitteeAddress(ic.DAO).BytesBE())
}

func (n *ATC) getAllCandidatesCall(ic *interop.Context, _ []stackitem.Item) stackitem.Item {
...
func (n *ATC) getCandidateVoteCall(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	pub := toPublicKey(args[0])
	key := makeValidatorKey(pub)
	return stackitem.NewBigInteger(n.getCandidateVoteFromStorage(ic.DAO, key))
}

func (n *ATC) getCandidateVoteFromStorage(dao *dao.Simple, validatorKey []byte) *big.Int {
...
func (n *ATC) getAccountState(ic *interop.Context, args []stackitem.Item) stackitem.Item {
...
func (n *ATC) ComputeNextBlockValidators(d *dao.Simple) keys.PublicKeys {
...
func (n *ATC) getCommittee(ic *interop.Context, _ []stackitem.Item) stackitem.Item {
	pubs := n.GetCommitteeMembers(ic.DAO)
	slices.SortFunc(pubs, (*keys.PublicKey).Cmp)
	return pubsToArray(pubs)
}

func (n *ATC) modifyVoterTurnout(d *dao.Simple, amount *big.Int) error {
...
func (n *ATC) GetCommitteeMembers(d *dao.Simple) keys.PublicKeys {
	cache := d.GetROCache(n.ID).(*AtipicialCache)
...
func (n *ATC) computeCommitteeMembers(blockHeight uint32, d *dao.Simple) (keys.PublicKeys, keysWithVotes, error) {
	key := []byte{prefixVotersCount}
	si := d.GetStorageItem(n.ID, key)
	if si == nil {
...
func (n *ATC) getNextBlockValidators(ic *interop.Context, _ []stackitem.Item) stackitem.Item {
	result := n.GetNextBlockValidatorsInternal(ic.DAO)
	return pubsToArray(result)
}

// GetNextBlockValidatorsInternal returns next block validators.
func (n *ATC) GetNextBlockValidatorsInternal(d *dao.Simple) keys.PublicKeys {
	cache := d.GetROCache(n.ID).(*AtipicialCache)
	return cache.nextValidators.Copy()
}

// BalanceOf returns native ATC token balance for the acc.
func (n *ATC) BalanceOf(d *dao.Simple, acc util.Uint160) (*big.Int, uint32) {
	key := makeAccountKey(acc)
	si := d.GetStorageItem(n.ID, key)
	if si == nil {
	...
	st, err := state.ATCBalanceFromBytes(si)
	if err != nil {
		panic(fmt.Errorf("failed to decode ATC balance state: %w", err))
	}
...
func (n *ATC) putGASRecord(dao *dao.Simple, index uint32, value *big.Int) {
	key := make([]byte, 5)
