...
// ATC Executable Format 3 (AEF3)
// Standard: https://github.com/atipicial-project/proposals/pull/121/files
// Implementation: https://github.com/atipicial-project/atipicial/blob/v3.0.0-preview2/src/atipicial/SmartContract/AefFile.cs#L8
// +------------+-----------+------------------------------------------------------------+
...
// File represents a compiled contract file structure according to the AEF3 standard.
type File struct {
	Header
...
// NewFile returns a new AEF3 file with the script specified.
func NewFile(script []byte) (*File, error) {
	file := &File{
		Header: Header{
			Magic:    Magic,
			Compiler: "atipicial-go-" + config.Version,
		},
		Tokens: []MethodToken{},
		Script: script,
	}
...
// CalculateChecksum doesn't perform the resulting serialized AEF size check, and return
// the checksum irrespectively to the size limit constraint. It's a caller's duty to check
// the resulting AEF size.
...
// Bytes returns a byte array with a serialized AEF File. It performs the
// resulting AEF file size check and returns an error if serialized slice length
// exceeds [stackitem.MaxSize].
func (n File) Bytes() ([]byte, error) {
	return n.bytes(true)
}

// BytesLong returns a byte array with a serialized AEF File. It performs no
// resulting slice check.
func (n File) BytesLong() ([]byte, error) {
	return n.bytes(false)
}

// bytes returns the serialized AEF File representation and performs the resulting
...
func (n File) bytes(checkSize bool) ([]byte, error) {
	...
	if checkSize && len(res) > stackitem.MaxSize {
		return nil, fmt.Errorf("serialized AEF size exceeds VM stackitem limits: %d bytes is allowed at max, got %d", stackitem.MaxSize, len(res))
	}
	return res, nil
}

// FileFromBytes returns a AEF File deserialized from the given bytes.
func FileFromBytes(source []byte) (File, error) {
	result := File{}
	if len(source) > stackitem.MaxSize {
		return result, fmt.Errorf("invalid AEF file size: expected %d at max, got %d", stackitem.MaxSize, len(source))
	}
