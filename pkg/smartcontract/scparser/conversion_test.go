...
func TestGetAppCallNoArgsFromContext(t *testing.T) {
	...
	t.Run("unknown SYSCALL", func(t *testing.T) {
		...
		emit.String(w.BinWriter, m)
		emit.Bytes(w.BinWriter, h.BytesBE())
		emit.Syscall(w.BinWriter, "Atipicial.Ledger.Unknown")
...
func TestParseAEP17Transfer(t *testing.T) {
	check := func(t *testing.T,
		...
		build(b, nativehashes.AtipicialDollar, "transfer", from, to, amount, data)
		script, err := b.Script()
		require.NoError(t, err)

		t.Run("good", func(t *testing.T) {
			actualH, actualFrom, actualTo, actualAmount, actualData, err := ParseAEP17Transfer(script)
			require.NoError(t, err)
			require.Equal(t, nativehashes.AtipicialDollar, actualH)
		...
		t.Run("additional data after transfer", func(t *testing.T) {
			_, _, _, _, _, err := ParseAEP17Transfer(append(script, byte(opcode.NOP))) // even NOP is prohibited, we need to be strict.
			require.ErrorContains(t, err, "extra data after script end")
		})
		t.Run("invalid appcall", func(t *testing.T) {
			b.Reset()
			build(b, nativehashes.AtipicialDollar, "transferUnknown", from, to, amount, data)
			script, err = b.Script()
			require.NoError(t, err)
			_, _, _, _, _, err := ParseAEP17Transfer(script)
...
func TestGetAEP17TransferFromContext(t *testing.T) {
	check := func(t *testing.T, errText string, method string, args ...any) {
		b := smartcontract.NewBuilder()
		b.InvokeMethod(nativehashes.AtipicialDollar, method, args...)
		script, err := b.Script()
		require.NoError(t, err)
		_, _, _, _, _, err = GetAEP17TransferFromContext(NewContext(script, 0))
		require.ErrorContains(t, err, errText)
	}
	t.Run("invalid method", func(t *testing.T) {
...
func TestParseAEP11Transfer(t *testing.T) {
	check := func(t *testing.T,
		build func(*smartcontract.Builder, util.Uint160, string, ...any)) {
		b := smartcontract.NewBuilder()
		to := util.Uint160{4, 5, 6}
		id := []byte{1, 2, 3}
		data := []any{int64(5), true}
		build(b, nativehashes.AtipicialDollar, "transfer", to, id, data)
		script, err := b.Script()
		require.NoError(t, err)

		t.Run("good", func(t *testing.T) {
			actualH, actualTo, actualID, actualData, err := ParseAEP11Transfer(script)
			require.NoError(t, err)
			require.Equal(t, nativehashes.AtipicialDollar, actualH)
			require.Equal(t, to, actualTo)
		...
		t.Run("additional data after transfer", func(t *testing.T) {
			_, _, _, _, err := ParseAEP11Transfer(append(script, byte(opcode.NOP))) // even NOP is prohibited, we need to be strict.
			require.ErrorContains(t, err, "extra data after script end")
		})
		t.Run("invalid appcall", func(t *testing.T) {
			b.Reset()
			build(b, nativehashes.AtipicialDollar, "transferUnknown", to, id, data)
			script, err = b.Script()
			require.NoError(t, err)
			_, _, _, _, err := ParseAEP11Transfer(script)
			require.ErrorContains(t, err, "expected 'transfer' call, got 'transferUnknown'")
		})
	}
	t.Run("ASSERT", func(t *testing.T) {
...
func TestGetAEP11TransferFromContext(t *testing.T) {
	check := func(t *testing.T, errText string, method string, args ...any) {
		b := smartcontract.NewBuilder()
		b.InvokeMethod(nativehashes.AtipicialDollar, method, args...)
		script, err := b.Script()
		require.NoError(t, err)
		_, _, _, _, err = GetAEP11TransferFromContext(NewContext(script, 0))
		require.ErrorContains(t, err, errText)
	}
	t.Run("invalid method", func(t *testing.T) {
...
func TestParseAEP11TransferD(t *testing.T) {
	check := func(t *testing.T,
		build func(*smartcontract.Builder, util.Uint160, string, ...any)) {
		b := smartcontract.NewBuilder()
		from := util.Uint160{1, 2, 3}
		...
		id := []byte{1, 2, 3}
		data := []any{int64(5), true}
		build(b, nativehashes.AtipicialDollar, "transfer", from, to, amount, id, data)
		script, err := b.Script()
		require.NoError(t, err)

		t.Run("good", func(t *testing.T) {
			actualH, actualFrom, actualTo, actualAmount, actualID, actualData, err := ParseAEP11TransferD(script)
			require.NoError(t, err)
			require.Equal(t, nativehashes.AtipicialDollar, actualH)
			require.Equal(t, from, actualFrom)
			require.Equal(t, to, actualTo)
		...
		t.Run("additional data after transfer", func(t *testing.T) {
			_, _, _, _, _, _, err := ParseAEP11TransferD(append(script, byte(opcode.NOP))) // even NOP is prohibited, we need to be strict.
			require.ErrorContains(t, err, "extra data after script end")
		})
		t.Run("invalid appcall", func(t *testing.T) {
			b.Reset()
			build(b, nativehashes.AtipicialDollar, "transferUnknown", from, to, amount, id, data)
			script, err = b.Script()
			require.NoError(t, err)
			_, _, _, _, _, _, err := ParseAEP11TransferD(script)
			require.ErrorContains(t, err, "expected 'transfer' call, got 'transferUnknown'")
		})
	}
	t.Run("ASSERT", func(t *testing.T) {
...
func TestGetAEP11TransferDFromContext(t *testing.T) {
	check := func(t *testing.T, errText string, method string, args ...any) {
		b := smartcontract.NewBuilder()
		b.InvokeMethod(nativehashes.AtipicialDollar, method, args...)
		script, err := b.Script()
		require.NoError(t, err)
		_, _, _, _, _, _, err = GetAEP11TransferDFromContext(NewContext(script, 0))
		require.ErrorContains(t, err, errText)
	}
	t.Run("invalid method", func(t *testing.T) {
