...
{{end}}`
	...
	bindingDefinition = `// Code generated by atipicial-go contract generate-rpcwrapper --manifest <file.json> --out <file.go> [--hash <hash>] [--config <config>]; DO NOT EDIT.
...
{{if or .IsAep11D .IsAep11ND}}	aep11.Invoker
{{else -}}
{{ if .IsAep17}}	aep17.Invoker
...
type ContractReader struct {
	{{if .IsAep11D}}aep11.DivisibleReader
	{{end -}}
	{{if .IsAep11ND}}aep11.NonDivisibleReader
	{{end -}}
	{{if .IsAep17}}aep17.TokenReader
	{{end -}}
	{{if .IsAep24}}aep24.RoyaltyReader
	{{end -}}
	invoker Invoker
	hash util.Uint160
}
...
type Contract struct {
	...
	{{end -}}
	{{if .IsAep11D}}aep11.DivisibleWriter
	{{end -}}
	{{if .IsAep11ND}}aep11.BaseWriter
	{{end -}}
	{{if .IsAep17}}aep17.TokenWriter
	{{end -}}
	{{- range $index, $name := .ContractWriterStandards -}}
...
func NewReader(invoker Invoker{{- if not (len .Hash) -}}, hash util.Uint160{{- end -}}) *ContractReader {
	...
	return &ContractReader{
		{{- if .IsAep11D}}*aep11.NewDivisibleReader(invoker, hash), {{end}}
		{{- if .IsAep11ND}}*aep11.NewNonDivisibleReader(invoker, hash), {{end}}
		{{- if .IsAep17}}*aep17.NewReader(invoker, hash), {{end -}}
		{{- if .IsAep24}}*aep24.NewRoyaltyReader(invoker, hash), {{end -}}
		invoker, hash}
}
{{end -}}
{{- if .HasWriter}}
...
func New(actor Actor{{- if not (len .Hash) -}}, hash util.Uint160{{- end -}}) *Contract {
	...
	var hash = Hash
	{{end -}}
	{{if .IsAep11D}}var aep11dt = aep11.NewDivisible(actor, hash)
	{{end -}}
	{{if .IsAep11ND}}var aep11ndt = aep11.NewNonDivisible(actor, hash)
	{{end -}}
	{{if .IsAep17}}var aep17t = aep17.New(actor, hash)
	{{end -}}
	{{if .IsAep24}}var aep24t = aep24.NewRoyaltyReader(actor, hash)
	{{end -}}
	return &Contract{
		{{- if .HasReader}}ContractReader{
		{{- if .IsAep11D}}aep11dt.DivisibleReader, {{end -}}
		{{- if .IsAep11ND}}aep11ndt.NonDivisibleReader, {{end -}}
		{{- if .IsAep17}}aep17t.TokenReader, {{end -}}
		{{- if .IsAep24}}*aep24t, {{end -}}
		actor, hash}, {{end -}}
		{{- if .IsAep11D}}aep11dt.DivisibleWriter, {{end -}}
		{{- if .IsAep11ND}}aep11ndt.BaseWriter, {{end -}}
		{{- if .IsAep17}}aep17t.TokenWriter, {{end -}}
		{{- range $index, $name := .ContractWriterStandards -}}
...
type (
	ContractTmpl struct {
		...
		//
		//  1. Has `aepXX.Contract` structure declared in the package.
		//  2. Has `aepXX.NewContract(actor, hash) *Contract` constructor (which implies the presence of non-safe methods).
...
func Generate(cfg binding.Config) error {
	...
	var (
		...
		ctr                       ContractTmpl
		isAep22, isAep30, isAep31 bool
	)

	// Strip standard methods from AEP-XX packages.
	for _, std := range cfg.Manifest.SupportedStandards {
		switch std {
		case manifest.AEP11StandardName:
			imports["github.com/atipicial/atipicial-go/pkg/rpcclient/aep11"] = struct{}{}
			if standard.ComplyABI(cfg.Manifest, standard.Aep11Divisible) == nil {
				ctr.IsAep11D = true
			} else if standard.ComplyABI(cfg.Manifest, standard.Aep11NonDivisible) == nil {
				ctr.IsAep11ND = true
			}
		case manifest.AEP17StandardName:
			if standard.ComplyABI(cfg.Manifest, standard.Aep17) == nil {
				imports["github.com/atipicial/atipicial-go/pkg/rpcclient/aep17"] = struct{}{}
				ctr.IsAep17 = true
			}
		case manifest.AEP22StandardName:
			if standard.ComplyABI(cfg.Manifest, standard.Aep22) == nil {
				imports["github.com/atipicial/atipicial-go/pkg/rpcclient/aep22"] = struct{}{}
				ctr.ContractWriterStandards = append(ctr.ContractWriterStandards, std)
				isAep22 = true
			}
		case manifest.AEP24StandardName:
			if standard.ComplyABI(cfg.Manifest, standard.Aep24) == nil {
				imports["github.com/atipicial/atipicial-go/pkg/rpcclient/aep24"] = struct{}{}
				ctr.IsAep24 = true
			}
		case manifest.AEP24Payable:
			if standard.ComplyABI(cfg.Manifest, standard.Aep24Payable) == nil {
				ctr.IsAep24Payable = true
			}
		case manifest.AEP30StandardName:
			if standard.ComplyABI(cfg.Manifest, standard.Aep30) == nil {
				isAep30 = true
			}
		case manifest.AEP31StandardName:
			if standard.ComplyABI(cfg.Manifest, standard.Aep31) == nil {
				imports["github.com/atipicial/atipicial-go/pkg/rpcclient/aep31"] = struct{}{}
				ctr.ContractWriterStandards = append(ctr.ContractWriterStandards, std)
				isAep31 = true
			}
		}
	...
	if ctr.IsAep11D {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep11Divisible)
	}
	if ctr.IsAep11ND {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep11NonDivisible)
	}
	if ctr.IsAep11D || ctr.IsAep11ND {
		mfst.ABI.Events = dropStdEvents(mfst.ABI.Events, standard.Aep11Base)
	}
	if ctr.IsAep17 {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep17)
		mfst.ABI.Events = dropStdEvents(mfst.ABI.Events, standard.Aep17)
	}
	if isAep22 {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep22)
	}
	if ctr.IsAep24 {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep24)
		cfg = dropAep24Types(cfg)
	}
	if ctr.IsAep24Payable {
		mfst.ABI.Events = dropStdEvents(mfst.ABI.Events, standard.Aep24Payable)
	}
	if isAep30 {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep30)
	}
	if isAep31 {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep31)
	}

	// OnAepXXPayment handlers normally can't be called directly.
	if standard.ComplyABI(cfg.Manifest, standard.Aep26) == nil {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep26)
	}
	if standard.ComplyABI(cfg.Manifest, standard.Aep27) == nil {
		mfst.ABI.Methods = dropStdMethods(mfst.ABI.Methods, standard.Aep27)
	}
...
// dropAep24Types removes NamedTypes of AEP-24 from the config if they are used only from the methods of the standard.
func dropAep24Types(cfg binding.Config) binding.Config {
	var targetTypeName string
	// Find structure returned by standard.MethodRoyaltyInfo method
	// and remove it from binding.Config.NamedTypes as it will be imported from aep24 package.
...
var nativeTypes = map[string]nativeType{
	"management.Contract":         {"github.com/atipicial/atipicial-go/pkg/core/state", "state.Contract"},
	...
	"ledger.Block":                {"github.com/atipicial/atipicial-go/pkg/rpcclient/ledger", "ledger.Block"},
	"ledger.Transaction":          {"github.com/atipicial/atipicial-go/pkg/rpcclient/ledger", "ledger.Transaction"},
	"ledger.TransactionSigner":    {"github.com/atipicial/atipicial-go/pkg/core/transaction", "transaction.Signer"},
	"ledger.WitnessRule":          {"", "transaction.WitnessRule"},
	"ledger.WitnessCondition":     {"", "transaction.WitnessCondition"},
	"atipicial.AccountState":            {"github.com/atipicial/atipicial-go/pkg/core/state", "state.ATCBalance"},
	"policy.WhitelistFeeContract": {"github.com/atipicial/atipicial-go/pkg/core/state", "state.WhitelistFeeContract"},
}
...
func extendedTypeToGo(et binding.ExtendedType, named map[string]binding.ExtendedType) (string, string) {
	...
	case smartcontract.Hash160Type:
		return "util.Uint160", "github.com/atipicial/atipicial-go/pkg/util"
	case smartcontract.Hash256Type:
		return "util.Uint256", "github.com/atipicial/atipicial-go/pkg/util"
	case smartcontract.PublicKeyType:
		return "*keys.PublicKey", "github.com/atipicial/atipicial-go/pkg/crypto/keys"
	...
	case smartcontract.ArrayType:
		if len(et.Name) > 0 {
			...
			return "*" + toTypeName(et.Name), "github.com/atipicial/atipicial-go/pkg/vm/stackitem"
		} else if et.Value != nil {
			if et.Value.Base == smartcontract.PublicKeyType { // Special array wrapper.
				return "keys.PublicKeys", "github.com/atipicial/atipicial-go/pkg/crypto/keys"
			}
	...
	case smartcontract.MapType:
		...
		return "map[" + kt + "]" + vt, "github.com/atipicial/atipicial-go/pkg/vm/stackitem"
...
func scTemplateToRPC(cfg binding.Config, ctr ContractTmpl, imports map[string]struct{}, scTypeConverter func(string, smartcontract.ParamType, *binding.Config) (string, string)) ContractTmpl {
	...
	for i := 0; i < len(ctr.Methods); i++ {
		...
		if abim.Safe {
			...
			if ctr.Methods[i].ReturnType == "bool" {
				imports["github.com/atipicial/atipicial-go/pkg/smartcontract"] = struct{}{}
			}
		}
	}
	...
	if len(ctr.CustomEvents) > 0 {
		imports["github.com/atipicial/atipicial-go/pkg/atipicialrpc/result"] = struct{}{}
		imports["github.com/atipicial/atipicial-go/pkg/vm/stackitem"] = struct{}{}
		imports["fmt"] = struct{}{}
		imports["errors"] = struct{}{}
	}
	...
	imports["github.com/atipicial/atipicial-go/pkg/util"] = struct{}{}
	if len(ctr.SafeMethods) > 0 {
		imports["github.com/atipicial/atipicial-go/pkg/rpcclient/unwrap"] = struct{}{}
		if !ctr.IsAep17 && !ctr.IsAep11D && !ctr.IsAep11ND && !ctr.IsAep24 {
			imports["github.com/atipicial/atipicial-go/pkg/atipicialrpc/result"] = struct{}{}
		}
	}
	if len(ctr.Methods) > 0 {
		imports["github.com/atipicial/atipicial-go/pkg/core/transaction"] = struct{}{}
	}
	if len(ctr.Methods) > 0 || ctr.IsAep17 || ctr.IsAep11D || ctr.IsAep11ND || len(ctr.ContractWriterStandards) > 0 {
		ctr.HasWriter = true
	}
	if len(ctr.SafeMethods) > 0 || ctr.IsAep17 || ctr.IsAep11D || ctr.IsAep11ND || ctr.IsAep24 {
		ctr.HasReader = true
	}
