/*
Package atipicialrpc contains a set of types used for JSON-RPC communication with Atipicial servers.
...
type (
	...
	// tailored for AtipicialGo RPC Client needs.
	Request struct {
		...
		// technically allows it to be an object, all Atipicial calls expect params
		// to be an array.
		Params []any `json:"params"`
		// ID is an identifier associated with this request. JSON-RPC itself allows
		// any strings to be used for it as well, but AtipicialGo RPC client uses numeric
		// identifiers.
		ID uint64 `json:"id"`
	}
