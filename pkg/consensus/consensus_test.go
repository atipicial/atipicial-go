...
func initServiceNextConsensus(t *testing.T, newAcc *wallet.Account, offset uint32) (*service, *wallet.Account) {
	...
	b.InvokeWithAssert(bc.GoverningTokenHash(), "transfer",
		acc.Contract.ScriptHash().BytesBE(), newPriv.GetScriptHash().BytesBE(), int64(native.ATCTotalSupply), nil)
...
func TestVerifyBlock(t *testing.T) {
	...
	t.Run("good empty", func(t *testing.T) {
		...
		require.True(t, srv.verifyBlock(&atipicialBlock{Block: *b}))
	})
	...
	t.Run("bad old", func(t *testing.T) {
		...
		require.False(t, srv.verifyBlock(&atipicialBlock{Block: *b}))
	})
...
var atipicialOwner = testchain.MultisigScriptHash()

func addSender(t *testing.T, txs ...*transaction.Transaction) {
	for _, tx := range txs {
		tx.Signers = []transaction.Signer{
			{
				Account: atipicialOwner,
			},
		}
	}
}
