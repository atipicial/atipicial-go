...
UNITWORKDIR = "/var/lib/atipicial-go"
...
ENV_IMAGE_TAG="env_atipicial_go_image"
...
IMAGE_REPO=atipicial/atipicial-go

DISABLE_ATCTEST_COVER=1
...
atipicial-go.service: atipicial-go.service.template
	@sed -r -e 's_BINDIR_$(BINDIR)_' -e 's_UNITWORKDIR_$(UNITWORKDIR)_' -e 's_SYSCONFIGDIR_$(SYSCONFIGDIR)_' $< >$@

install: build atipicial-go.service
	@echo "=> Installing systemd service"
	@mkdir -p $(DESTDIR)$(SYSCONFIGDIR)/atipicial-go \
		&& mkdir -p $(SYSTEMDUNIT_DIR) \
		&& cp ./atipicial-go.service $(SYSTEMDUNIT_DIR) \
		&& cp ./config/protocol.mainnet.yml $(DESTDIR)$(SYSCONFIGDIR)/atipicial-go \
		&& cp ./config/protocol.privnet.yml $(DESTDIR)$(SYSCONFIGDIR)/atipicial-go \
		&& cp ./config/protocol.testnet.yml $(DESTDIR)$(SYSCONFIGDIR)/atipicial-go \
		&& install -m 0755 -t $(BINDIR) $(BINARY_PATH) \

postinst: install
	@echo "=> Preparing directories and configs"
	@id atipicial-go || useradd -s /usr/sbin/nologin -d $(UNITWORKDIR) atipicial-go \
		&& mkdir -p $(UNITWORKDIR) \
		&& chown -R atipicial-go:atipicial-go $(UNITWORKDIR) $(BINDIR)/atipicial-go \
		&& systemctl enable atipicial-go.service
...
.golangci.yml:
	curl -L -o $@ https://github.com/github.com/atipicial/.github/raw/master/.golangci.yml
