...
func main() {
	...
	txMoveAtipicial, err := testchain.NewTransferFromOwner(bc, bc.GoverningTokenHash(), h, native.ATCTotalSupply, 0, 2)
	handleError("can't transfer ATC", err)
	...
	lastBlock = addBlock(bc, lastBlock, valScript, txMoveAtipicial, txMoveGas)
