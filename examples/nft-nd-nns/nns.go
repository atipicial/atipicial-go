/*
Package nns contains non-divisible non-fungible AEP-11-compatible token
implementation. This token is a compatible analogue of C# Atipicial Name Service
token and is aimed to serve as a domain name service for Atipicial smart-contracts,
thus it's AtipicialNameService. This token can be minted with new domain name
...
func Update(aef []byte, manifest []byte, data any) {
	checkCommittee()
	management.UpdateWithData(aef, []byte(manifest), data)
}
...
// Symbol returns AtipicialNameService symbol.
func Symbol() string {
	return "NNS"
}

// Decimals returns AtipicialNameService decimals.
func Decimals() int {
	return 0
}

// TotalSupply returns overall number of domains minted by the AtipicialNameService contract.
...
// postTransfer sends Transfer notification to the network and calls onAEP11Payment
// method.
func postTransfer(from, to interop.Hash160, tokenID []byte, data any) {
	runtime.Notify("Transfer", from, to, 1, tokenID)
	if management.GetContract(to) != nil {
		contract.Call(to, "onAEP11Payment", contract.All, from, 1, tokenID, data)
	}
}
...
func checkCommittee() {
	committee := atipicial.GetCommittee()
	if committee == nil {
