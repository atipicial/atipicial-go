...
// GetObject returns the AtipicialFs object stored in the contract storage.
func GetObject() []byte {
	return storage.Get(storage.GetReadOnlyContext(), objectKey).([]byte)
}

// SaveObject requests a AtipicialFs object specified by the container ID (cid) and
...
func SaveObject(cid, oid string) {
	...
	url := "atipicialfs:" + cid + "/" + oid
	oracle.Request(url, nil, "saveObjectCB", nil, 15*oracle.MinimumResponseGas)
}

// SaveObjectCB is called by the Oracle native contract when the AtipicialFs object
...
// DeleteObject removes the stored AtipicialFs object from the contract storage.
