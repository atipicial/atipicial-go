/*
Package nft contains divisible non-fungible AEP-11-compatible token
implementation. This token can be minted with GAS transfer to contract address,
it will retrieve AtipicialFs container ID and object ID from the transfer data and
produce NFT which represents AtipicialFs object.
*/
package nft
...
// object ID of the AtipicialFs object.
...
// postTransfer emits Transfer event and calls onAEP11Payment if needed.
func postTransfer(from interop.Hash160, to interop.Hash160, token []byte, amount int, data any) {
	...
	if management.GetContract(to) != nil {
		contract.Call(to, "onAEP11Payment", contract.All, from, amount, token, data)
	}
}
...
func Properties(id []byte) map[string]string {
	...
	result := map[string]string{
		"name":        "AtipicialFs Object " + std.Base64Encode(id), // Not a hex for contract simplicity.
...
// OnAEP17Payment mints tokens if at least 10 GAS is provided. You don't call
...
func Update(aef, manifest []byte, data any) {
	if !Verify() {
		panic("only owner can update")
	}
	management.UpdateWithData(aef, manifest, data)
}
