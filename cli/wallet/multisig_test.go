...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/smartcontract/context"
...
func TestSignMultisigTx(t *testing.T) {
	...
	addAccount := func(w string, wif string) {
		e.Run(t, "atipicial-go", "wallet", "init", "--wallet", w)
	...
	t.Run("bad cases", func(t *testing.T) {
		...
		e.Run(t, "atipicial-go", "wallet", "aep17", "transfer",
			...
			"--to", priv.Address(), "--token", "ATC", "--amount", "1",
			"--out", txPath)

		// missing wallet
		e.RunWithErrorCheck(t, `Required flag "in" not set`, "atipicial-go", "wallet", "sign")
	...
	t.Run("test invoke", func(t *testing.T) {
		t.Run("missing file", func(t *testing.T) {
			e.RunWithError(t, "atipicial-go", "util", "txdump")
			fmt.Println(e.Out.String())
		})

		t.Run("no invoke", func(t *testing.T) {
			e.Run(t, "atipicial-go", "util", "txdump", txPath)
		...
		e.Run(t, "atipicial-go", "util", "txdump",
			"--rpc-endpoint", "http://"+e.RPC.Addresses()[0],
			txPath)
		e.CheckTxTestInvokeOutput(t, 11)
