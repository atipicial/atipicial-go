...
import (
	...
	"github.com/atipicial/atipicial-go/cli/input"
...
func NewCommands() []*cli.Command {
	...
	return []*cli.Command{{
		Name:  "wallet",
		Usage: "Create, open and manage a Atipicial wallet",
		Subcommands: []*cli.Command{
			{
				Name:      "claim",
				Usage:     "Claim GAS",
				UsageText: "atipicial-go wallet claim -w wallet [--wallet-config path] [-g gas] [-e sysgas] -a address -r endpoint [-s timeout] [--out file] [--force] [--await]",
				Action:    claimGas,
				Flags:     claimFlags,
			},
			{
				...
				UsageText: "atipicial-go wallet init -w wallet [--wallet-config path] [-a]",
				Action:    createWallet,
				Flags: []cli.Flag{
					walletPathFlag,
					walletConfigFlag,
			...
			},
			{
				...
				UsageText: "atipicial-go wallet change-password -w wallet [-a address]",
			...
			{
				Name:      "convert",
				Usage:     "Convert addresses from existing Atipicial Legacy AEP6-wallet to Atipicial  format",
				UsageText: "atipicial-go wallet convert -w legacywallet [--wallet-config path] -o n3wallet",
			...
			},
			{
				...
				UsageText: "atipicial-go wallet create -w wallet [--wallet-config path]",
				Action:    addAccount,
				Flags: []cli.Flag{
					walletPathFlag,
					walletConfigFlag,
			...
			{
				Name:      "dump",
				Usage:     "Check and dump an existing Atipicial wallet",
				UsageText: "atipicial-go wallet dump -w wallet [--wallet-config path] [-d]",
...
`,
			...
			},
			{
				...
				UsageText: "atipicial-go wallet dump-keys -w wallet [--wallet-config path] [-a address]",
				Action:    dumpKeys,
				Flags: []cli.Flag{
					walletPathFlag,
					walletConfigFlag,
			...
			},
			{
				...
				UsageText: "export -w wallet [--wallet-config path] [--format aep2|wif|pem] [<address>]",
				Description: `Prints the key for the given account to the standard output. It uses AEP-2
   encrypted format by default (the way AEP-6 wallets store it). If --format
   option is given, one of 'aep2' (default), 'wif' (unencrypted WIF format) or
...
`,
				...
				Flags: []cli.Flag{
					...
					&cli.StringFlag{
						Name:    "format",
						Aliases: []string{"f"},
						Usage:   "Format of the output key: 'aep2' (default), 'wif' or 'pem'.",
						Value:   "aep2",
					},
				},
			},
...
`,
			...
			},
			{
				...
				UsageText: "atipicial-go wallet strip-keys -w wallet [--wallet-config path] [--force]",
...
`,
			...
			{
				Name:        "aep17",
				Usage:       "Work with AEP-17 contracts",
				Subcommands: newAEP17Commands(),
			},
			{
				Name:        "aep11",
				Usage:       "Work with AEP-11 contracts",
				Subcommands: newAEP11Commands(),
			},
			{
				Name:        "candidate",
...
func claimGas(ctx *cli.Context) error {
	return handleAtipicialAction(ctx, func(contract *atipicial.Contract, shash util.Uint160, _ *wallet.Account) (*transaction.Transaction, error) {
...
func exportKeys(ctx *cli.Context) error {
	...
	decrypt := format != exportAEP2
	if ctx.NArg() == 0 && decrypt {
		return cli.Exit(errors.New("address must be provided if '--decrypt' or '--format wif|pem' flags are used"), 1)
	} else if ctx.NArg() > 0 {
	...
	for _, wif := range wifs {
		if decrypt {
			...
			pk, err := keys.AEP2Decrypt(wif, *pass, wall.Scrypt)
			if err != nil {
				return cli.Exit(err, 1)
			}
			...
			case exportPEM:
				...
				if err != nil {
					return cli.Exit(fmt.Errorf("failed to marshal private key to DER form: %w", err), 1)
				}
				if err := pem.Encode(ctx.App.Writer, &pem.Block{Type: "PRIVATE KEY", Bytes: der}); err != nil {
					return cli.Exit(fmt.Errorf("failed to encode PEM: %w", err), 1)
				}
				continue
			default:
				return cli.Exit(fmt.Errorf("unknown private key export format: %d", format), 1)
			}
		}
...
func newAccountFromWIF(w io.Writer, wif string, scrypt keys.ScryptParams, label *string, pass *string) (*wallet.Account, error) {
	...
	// note: AEP2 strings always have length of 58 even though
	...
	if len(wif) == 58 {
		...
		if err != nil {
			...
			// see the https://github.com/atipicial/atipicial-go/issues/2883#issuecomment-1399923088.
			if pass == nil {
				return nil, err
			}
			phrase, err = input.ReadPassword(EnterPasswordPrompt)
