...
var (
	tokenFlag = &cli.StringFlag{
		Name:  "token",
		Usage: "Token to use (hash or name (for ATC/GAS or imported tokens))",
	}
...
func newAEP17Commands() []*cli.Command {
	...
	return []*cli.Command{
		{
			...
			Description: `Prints AEP-17 balances for address and tokens specified. One of wallet
   ...
   as they belong to a correct AEP-17 contract), while names or symbols (if
   they're not ATC or GAS names/symbols) are matched against the token data
...
`,
		...
		{
			Name:      "remove",
			Usage:     "Remove AEP-17 token from the wallet",
			UsageText: "remove -w wallet [--wallet-config path] --token <hash-or-name>",
			Action:    removeAEP17Token,
			Flags: []cli.Flag{
				walletPathFlag,
				walletConfigFlag,
				tokenFlag,
		...
		},
		{
			...
			Action:    transferAEP17,
			Flags:     transferFlags,
			Description: `Transfers specified AEP-17 token amount with optional 'data' parameter and cosigners
...
`,
		...
		{
			Name:  "multitransfer",
			Usage: "Transfer AEP-17 tokens to multiple recipients",
...
func getAEP17Balance(ctx *cli.Context) error {
	return getAEPBalance(ctx, manifest.AEP17StandardName, func(ctx *cli.Context, c *rpcclient.Client, addrHash util.Uint160, name string, token *wallet.Token, _ string) error {
		balances, err := c.GetAEP17Balances(addrHash)
		if err != nil {
			return err
		}
		...
		for i := range balances.Balances {
			curToken := tokenFromAEP17Balance(&balances.Balances[i])
		...
		if token != nil {
			...
			printAssetBalance(ctx, result.AEP17Balance{
				Asset:       token.Hash,
				Amount:      "0",
...
func getAEPBalance(ctx *cli.Context, standard string, accHandler func(*cli.Context, *rpcclient.Client, util.Uint160, string, *wallet.Token, string) error) error {
	...
	if name != "" {
		...
		if err != nil {
			...
			if standard == manifest.AEP17StandardName && (name == nativenames.Atipicial || name == "ATC") {
				h = atipicial.Hash
			} else if standard == manifest.AEP17StandardName && (name == nativenames.Gas || name == "GAS") {
				h = gas.Hash
			} else {
	...
	if standard == manifest.AEP11StandardName {
		if len(tokenID) > 0 {
...
func printAssetBalance(ctx *cli.Context, balance result.AEP17Balance) {
...
func tokenFromAEP17Balance(bal *result.AEP17Balance) *wallet.Token {
	return wallet.NewToken(bal.Asset, bal.Name, bal.Symbol, int64(bal.Decimals), manifest.AEP17StandardName)
}

func tokenFromAEP11Balance(bal *result.AEP11AssetBalance) *wallet.Token {
	return wallet.NewToken(bal.Asset, bal.Name, bal.Symbol, int64(bal.Decimals), manifest.AEP11StandardName)
}
...
func getMatchingTokenRPC(ctx *cli.Context, c *rpcclient.Client, addr util.Uint160, name string, standard string) (*wallet.Token, error) {
	...
	case manifest.AEP17StandardName:
		bs, err := c.GetAEP17Balances(addr)
		if err != nil {
			return nil, err
		}
		get := func(i int) *wallet.Token {
			return tokenFromAEP17Balance(&bs.Balances[i])
		}
		return getMatchingTokenAux(ctx, get, len(bs.Balances), name, standard)
	case manifest.AEP11StandardName:
		bs, err := c.GetAEP11Balances(addr)
		if err != nil {
			return nil, err
		}
		get := func(i int) *wallet.Token {
			return tokenFromAEP11Balance(&bs.Balances[i])
		}
		return getMatchingTokenAux(ctx, get, len(bs.Balances), name, standard)
	default:
...
func importAEP17Token(ctx *cli.Context) error {
	return importAEPToken(ctx, manifest.AEP17StandardName)
}

func importAEPToken(ctx *cli.Context, standard string) error {
	if err := cmdargs.EnsureNone(ctx); err != nil {
		return err
	}
...
func getTokenWithStandard(c *rpcclient.Client, hash util.Uint160, std string) (*wallet.Token, error) {
	token, err := aeptoken.Info(c, hash)
	if err != nil {
		return nil, err
	}
...
func printAEP17Info(ctx *cli.Context) error {
	return printAEPInfo(ctx, manifest.AEP17StandardName)
}

func printAEPInfo(ctx *cli.Context, standard string) error {
	if err := cmdargs.EnsureNone(ctx); err != nil {
		return err
	}
	wall, _, err := readWallet(ctx)
...
func removeAEP17Token(ctx *cli.Context) error {
	return removeAEPToken(ctx, manifest.AEP17StandardName)
}

func removeAEPToken(ctx *cli.Context, standard string) error {
	if err := cmdargs.EnsureNone(ctx); err != nil {
		return err
	}
	wall, _, err := openWallet(ctx, true)
...
func multiTransferAEP17(ctx *cli.Context) error {
	...
	for i := range cosignersSepPos {
		...
		token, ok := cache[ss[0]]
		if !ok {
			token, err = getMatchingToken(ctx, wall, ss[0], manifest.AEP17StandardName)
			if err != nil {
				token, err = getMatchingTokenRPC(ctx, c, from, ss[0], manifest.AEP17StandardName)
				if err != nil {
	...
	tx, err := makeMultiTransferAEP17(act, recipients)
	if err != nil {
...
func transferAEP17(ctx *cli.Context) error {
	return transferAEP(ctx, manifest.AEP17StandardName)
}

func transferAEP(ctx *cli.Context, standard string) error {
	...
	// It's OK for AEP-11 transfer to not have amount set.
	if err != nil && (standard == manifest.AEP17StandardName || amountArg != "") {
		return cli.Exit(fmt.Errorf("invalid amount: %w", err), 1)
	}
	switch standard {
	case manifest.AEP17StandardName:
		n17 := aep17.New(act, token.Hash)
	...
	case manifest.AEP11StandardName:
		...
		if amountArg == "" {
			n11 := aep11.NewNonDivisible(act, token.Hash)
			tx, err = n11.TransferUnsigned(to, tokenIDBytes, data)
		} else {
			n11 := aep11.NewDivisible(act, token.Hash)
...
func makeMultiTransferAEP17(act *actor.Actor, recipients []transferTarget) (*transaction.Transaction, error) {
