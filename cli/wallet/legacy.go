...
// newWalletV2FromFile reads a Atipicial Legacy wallet from the file.
...
func (a *accountV2) convert(pass string, scrypt keys.ScryptParams) (*wallet.Account, error) {
	address.Prefix = address.ATC2Prefix
	priv, err := keys.AEP2Decrypt(a.EncryptedWIF, pass, scrypt)
	if err != nil {
		return nil, err
	}

	address.Prefix = address.ATIPICIALPrefix
...
// parseMultisigContract accepts a multisig verification script from AtipicialLegacy
