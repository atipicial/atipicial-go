...
const (
	...
	ParamsParsingDoc = `   Arguments always do have regular Atipicial smart contract parameter types, either
    ...
    * 'int' values are decimal integers that can be successfully converted
      from the string.
    * 'hash160' values are Atipicial addresses and hex-encoded 20-bytes long (after
      decoding) strings.
   ...
   following logic:
    - anything that can be interpreted as a decimal integer gets
      an 'int' type
    - 'nil' string gets 'Any' AEP-14 parameter type and nil value which corresponds
      to Null stackitem
    - 'true' and 'false' strings get 'bool' type
    - valid Atipicial addresses and 20 bytes long hex-encoded strings get 'hash160'
      type
    ...
    * 'nil' is a parameter with Any AEP-14 type and nil value (corresponds to Null stackitem)
    ...
    * 'signer' is a signer's address (as Atipicial address or hex-encoded 160 bit (20 byte)
    ...
    * 'scope' is a comma-separated set of cosigner's scopes, which could be:
        ...
        - 'CalledByEntry' - means that this condition must hold: EntryScriptHash
                            ...
                            safe choice for native ATC/GAS.
   ...
   atipicial-go RPC server only. C# implementation does not support scopes capability.
