...
import (
	...
	"github.com/github.com/atipicial/atipicialfs-sdk-go/pool"
...
const (
	...
	// Atipicial  mainnet blocks accepting.
...
const (
	...
	// AtipicialFsRPCEndpointFlag is a long flag name for a AtipicialFs RPC endpoint.
	AtipicialFsRPCEndpointFlag = "fs-rpc-endpoint"
)
...
// AtipicialFsRPC is a set of flags used for AtipicialFs RPC connections (endpoint).
var AtipicialFsRPC = []cli.Flag{&cli.StringSliceFlag{
	Name:     AtipicialFsRPCEndpointFlag,
	Aliases:  []string{"fsr"},
	Usage:    "List of AtipicialFs storage node RPC addresses (comma-separated or multiple --fs-rpc-endpoint flags)",
	Required: true,
	Action: func(ctx *cli.Context, fsRpcEndpoints []string) error {
		if slices.Contains(fsRpcEndpoints, "") {
			return cli.Exit("AtipicialFs RPC endpoint cannot contain empty values", 1)
		}
		return nil
	},
}}
...
// GetAtipicialFsClientPool returns a AtipicialFs pool and a signer for the given Context.
func GetAtipicialFsClientPool(ctx *cli.Context, acc *wallet.Account) (user.Signer, *pool.Pool, error) {
	rpcAtipicialFs := ctx.StringSlice(AtipicialFsRPCEndpointFlag)
	signer := user.NewAutoIDSignerRFC6979(acc.PrivateKey().PrivateKey)

	params := pool.DefaultOptions()
	params.SetHealthcheckTimeout(atipicialfs.DefaultHealthcheckTimeout)
	params.SetNodeDialTimeout(atipicialfs.DefaultDialTimeout)
	params.SetNodeStreamTimeout(atipicialfs.DefaultStreamTimeout)
	p, err := pool.New(pool.NewFlatNodeParams(rpcAtipicialFs), signer, params)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to create AtipicialFs pool: %w", err)
	}
	if err = p.Dial(context.Background()); err != nil {
		return nil, nil, fmt.Errorf("failed to dial AtipicialFs pool: %w", err)
	}
	return signer, p, nil
}
