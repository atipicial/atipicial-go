...
> jump 12`,
	...
	{
		Name:      "loadaef",
		Usage:     "Load a AEF (possibly with a contract hash) into the VM optionally using provided scoped signers in the context",
		UsageText: `loadaef [--historic <height>] [--gas <int>] [--hash <hash-or-address>] <file> [<manifest>] [-- <signer-with-scope>, ...]`,
		Flags:     []cli.Flag{historicFlag, gasFlag, hashFlag},
		Description: `<file> parameter is mandatory, <manifest> parameter (if omitted) will
   be guessed from the <file> parameter by replacing '.aef' suffix with '.manifest.json'
   suffix.

` + cmdargs.SignersParsingDoc + `

Example:
> loadaef /path/to/script.aef /path/to/manifest.json`,
		Action: handleLoadAEF,
	},
	{
		Name:      "loadbase64",
...
func NewWithConfig(printLogotype bool, onExit func(int), c *readline.Config, cfg config.Config) (*CLI, error) {
	...
	// `filepath.Base(os.Args[0])` will be used which is `atipicial-go`.
	ctl.HelpName = ""
	ctl.UsageText = ""
	...
	ctl.Usage = "Official VM CLI for AtipicialGo"
...
func handleLoadAEF(c *cli.Context) error {
	args := c.Args().Slice()
	if len(args) < 1 {
		return fmt.Errorf("%w: <aef> is required", ErrMissingParameter)
	}
	aefFile := args[0]
	var (
		manifestFile       string
	...
	if len(manifestFile) == 0 {
		manifestFile = strings.TrimSuffix(aefFile, ".aef") + ".manifest.json"
	}
	b, err := os.ReadFile(aefFile)
	if err != nil {
		return err
	}
	aef, err := aef.FileFromBytes(b)
	if err != nil {
		return fmt.Errorf("failed to decode AEF file: %w", err)
	}
	...
	err = prepareVM(c, createFakeTransaction(aef.Script, signers))
	if err != nil {
		return err
	}
	h, err := getHashFlag(c)
...
func handleLoadDeployed(c *cli.Context) error {
	...
	err = prepareVM(c, createFakeTransaction(cs.AEF.Script, signers)) // prepare VM one more time for proper IC initialization.
	if err != nil {
		return err
	}
	...
	ic.VM.LoadScriptWithHash(cs.AEF.Script, cs.Hash, callflag.All)
...
func handleRun(c *cli.Context) error {
	...
	if len(args) != 0 {
		...
		if runCurrent {
			if cs == nil {
				return fmt.Errorf("manifest is not loaded; either use 'run' command to run loaded script from the start or use 'loadgo', 'loadaef' or 'loaddeployed' commands to provide manifest")
			}
			...
			// Clear context loaded by 'loadgo', 'loadaef' or 'loaddeployed' to properly handle LoadAEFMethod.
			...
			v.LoadAEFMethod(&cs.AEF, &cs.Manifest, util.Uint160{}, cs.Hash, callflag.All, hasRet, offset, initOff, nil, nil, false)
...
func changePrompt(app *cli.App) {
	...
	if v.Ready() && v.Context().NextIP() >= 0 && v.Context().NextIP() < v.Context().LenInstr() {
		l.SetPrompt(fmt.Sprintf("\033[32mATIPICIAL-GO-VM %d >\033[0m ", v.Context().NextIP()))
	} else {
		l.SetPrompt("\033[32mATIPICIAL-GO-VM >\033[0m ")
	}
}
