...
import (
	...
	"github.com/atipicial/atipicial-go/pkg/core/storage/dboper"
...
require (
	github.com/atipicial/atipicial-go/pkg/interop v0.0.0
)
replace github.com/atipicial/atipicial-go/pkg/interop => ` + filepath.Join(wd, "../../pkg/interop") + `
go 1.26`)
	require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "go.mod"), goMod, os.ModePerm))
	return filename
}

// prepareLoadaefSrc compiles provided SC source and prepares AEF and manifest for loading into VM
// via `loadaef` command. It returns the name of manifest and AEF files ready to be used in CLI
// commands.
func prepareLoadaefSrc(t *testing.T, tmpDir, src string) (string, string) {
	aefFile, di, err := compiler.CompileWithOptions("test.go", strings.NewReader(src), nil)
	require.NoError(t, err)
	filename := filepath.Join(tmpDir, "vmtestcontract.aef")
	rawAef, err := aefFile.Bytes()
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filename, rawAef, os.ModePerm))
...
`
	...
	t.Run("loadaef", func(t *testing.T) {
		tmpDir := t.TempDir()

		manifestFile, aefFile := prepareLoadaefSrc(t, tmpDir, src)
		filenameErr := filepath.Join(tmpDir, "vmtestcontract_err.aef")
		...
		e.runProg(t,
			"loadaef",
			"loadaef "+filenameErr+" "+manifestFile,
			"loadaef "+aefFile+" "+notExists,
			"loadaef "+aefFile+" "+aefFile,
			"loadaef "+aefFile+" "+manifestFile,
			"run main add 3 5",
			"loadaef "+aefFile,
			"run main add 3 5",
			"loadaef "+aefFile+" "+cmdargs.CosignersSeparator,
			"loadaef "+aefFile+" "+manifestFile+" "+cmdargs.CosignersSeparator,
			"loadaef "+aefFile+" "+manifestFile+" "+"not-a-separator",
			"loadaef "+aefFile+" "+cmdargs.CosignersSeparator+" "+util.Uint160{1, 2, 3}.StringLE(),
			"run main add 3 5",
			"loadaef "+aefFile+" "+manifestFile+" "+cmdargs.CosignersSeparator+" "+util.Uint160{1, 2, 3}.StringLE(),
			"run main add 3 5",
		)
...
func TestLoad_RunWithCALLT(t *testing.T) {
	...
	src := `package kek
		import "github.com/atipicial/atipicial-go/pkg/interop/native/std"
		func Main() int {
...
func TestLoadtx(t *testing.T) {
	...
	e.checkStack(t, 1)
	e.checkNextLine(t, "READY: loaded \\d+ instructions")
	e.checkError(t, fmt.Errorf("at instruction 3 (PACK): %w", vm.ErrGASLimitExceeded))
	e.checkNextLine(t, "READY: loaded \\d+ instructions")
	e.checkStack(t, 1)
	e.checkNextLine(t, "READY: loaded \\d+ instructions")
	e.checkStack(t, 1)
...
func TestLoaddeployed(t *testing.T) {
	...
	e.checkStack(t, []byte{2})
	e.checkNextLine(t, "READY: loaded \\d+ instructions")
	e.checkError(t, fmt.Errorf("at instruction 63 (SYSCALL): System.Storage.Get failed: %w", vm.ErrGASLimitExceeded))
	e.checkNextLine(t, "READY: loaded \\d+ instructions")
	e.checkStack(t, []byte{2})
	e.checkNextLine(t, "READY: loaded \\d+ instructions")
	e.checkStack(t, []byte{2})
