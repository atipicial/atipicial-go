		...
		Subcommands: []*cli.Command{
			{
				...
				UsageText: "atipicial-go query candidates -r endpoint [-s timeout]",
			...
			},
			{
				...
				UsageText: "atipicial-go query committee -r endpoint [-s timeout]",
			...
			},
			{
				...
				UsageText: "atipicial-go query height -r endpoint [-s timeout]",
				Action:    queryHeight,
				Flags:     options.RPC,
			},
			{
				...
				UsageText: "atipicial-go query notarypool -r endpoint [-s timeout] [-v]",
			...
			},
			{
				...
				UsageText: "atipicial-go query tx -r endpoint [-s timeout] [-v] <hash>",
				Action:    queryTx,
				Flags:     queryTxFlags,
			},
			{
				Name:      "voter",
				Usage:     "Print ATC holder account state",
				UsageText: "atipicial-go query voter -r endpoint [-s timeout] <address>",
				Action:    queryVoter,
				Flags:     options.RPC,
			},
		},
...
func queryVoter(ctx *cli.Context) error {
	...
	atipicialToken := atipicial.NewReader(invoker.New(c, nil))

	st, err := atipicialToken.GetAccountState(addr)
	if err != nil {
		return cli.Exit(err, 1)
	}
	if st == nil {
		st = new(state.ATCBalance)
	}
	dec, err := atipicialToken.Decimals()
	if err != nil {
