...
func TestQueryTx(t *testing.T) {
	...
	args := []string{"atipicial-go", "query", "tx", "--rpc-endpoint", "http://" + e.RPC.Addresses()[0]}
...
func TestQueryHeight(t *testing.T) {
	...
	args := []string{"atipicial-go", "query", "height", "--rpc-endpoint", "http://" + e.RPC.Addresses()[0]}
	e.Run(t, args...)
...
func TestQueryNotaryPool(t *testing.T) {
	...
	require.NoError(t, err)
	tx, err := ntr.MakeCall(nativehashes.AtipicialDollar, "transfer", acc.ScriptHash(), acc.ScriptHash(), big.NewInt(1), nil)
	mainH, _, vub, err := ntr.Notarize(tx, err)
	require.NoError(t, err)

	// Check the notary pool.
	args := []string{"atipicial-go", "query", "notarypool", "--rpc-endpoint", endpoint}
