...
func TestAEP11Import(t *testing.T) {
	e := testcli.NewExecutor(t, true)

	tmpDir := t.TempDir()
	walletPath := filepath.Join(tmpDir, "walletForImport.json")

	// deploy NFT AtipicialNameService contract
	nnsContractHash := deployNNSContract(t, e)
	// deploy NFT-D AtipicialFs Object contract
	...
	args := []string{
		"atipicial-go", "wallet", "aep11", "import",
	...
	// not a AEP-11 token
	e.RunWithError(t, append(args, "--token", atipicialContractHash.StringLE())...)
	...
	checkInfo := func(t *testing.T, h util.Uint160, name string, symbol string, decimals int) {
		...
		e.CheckNextLine(t, "^Standard:\\s*"+string(manifest.AEP11StandardName))
	}
	t.Run("Info", func(t *testing.T) {
		t.Run("excessive parameters", func(t *testing.T) {
			e.RunWithError(t, "atipicial-go", "wallet", "aep11", "info",
				"--wallet", walletPath, "--token", nnsContractHash.StringLE(), "qwerty")
		})
		t.Run("WithToken", func(t *testing.T) {
			e.Run(t, "atipicial-go", "wallet", "aep11", "info",
		...
		t.Run("NoToken", func(t *testing.T) {
			...
			checkInfo(t, nnsContractHash, "NameService", "NNS", 0)
			e.CheckNextLine(t, "")
			checkInfo(t, nfsContractHash, "AtipicialFs Object NFT", "NFSO", 2)
		})
	})

	t.Run("Remove", func(t *testing.T) {
		e.RunWithError(t, "atipicial-go", "wallet", "aep11", "remove",
			"--wallet", walletPath, "--token", nnsContractHash.StringLE(), "parameter")
		e.In.WriteString("y\r")
		e.Run(t, "atipicial-go", "wallet", "aep11", "remove",
			"--wallet", walletPath, "--token", nnsContractHash.StringLE())
		e.Run(t, "atipicial-go", "wallet", "aep11", "info",
			"--wallet", walletPath)
		checkInfo(t, nfsContractHash, "AtipicialFs Object NFT", "NFSO", 2)
...
func TestAEP11_ND_OwnerOf_BalanceOf_Transfer(t *testing.T) {
	e := testcli.NewExecutor(t, true)
	tmpDir := t.TempDir()
	...
	// check the balance
	cmdCheckBalance := []string{"atipicial-go", "wallet", "aep11", "balance",
		"--rpc-endpoint", "http://" + e.RPC.Addresses()[0],
		"--wallet", wall,
	...
	e.RunWithError(t, append(cmdCheckBalance, "--token", h.StringLE(), "atipicial-go")...)
	// balance check: by hash, ok
	e.Run(t, append(cmdCheckBalance, "--token", h.StringLE())...)
	checkBalanceResult(t, nftOwnerAddr, tokenID)

	// import token
	e.Run(t, "atipicial-go", "wallet", "aep11", "import",
		"--rpc-endpoint", "http://"+e.RPC.Addresses()[0],
		"--wallet", wall,
		"--token", h.StringLE())
	...
	e.Run(t, "atipicial-go", "wallet", "aep11", "balance",
		"--rpc-endpoint", "http://"+e.RPC.Addresses()[0],
		"--wallet", wall,
		"--token", h.StringLE())
	checkBalanceResult(t, nftOwnerAddr, tokenID)
	...
	cmdOwnerOf := []string{"atipicial-go", "wallet", "aep11", "ownerOf",
		"--rpc-endpoint", "http://" + e.RPC.Addresses()[0],
	}
	...
	cmdTokensOf := []string{"atipicial-go", "wallet", "aep11", "tokensOf",
		"--rpc-endpoint", "http://" + e.RPC.Addresses()[0],
	}
	...
	cmdProperties := []string{
		"atipicial-go", "wallet", "aep11", "properties",
		"--rpc-endpoint", "http://" + e.RPC.Addresses()[0],
	}
	...
	cmdTokens := []string{"atipicial-go", "wallet", "aep11", "tokens",
		"--rpc-endpoint", "http://" + e.RPC.Addresses()[0],
	}
	...
	cmdTransfer := []string{
		"atipicial-go", "wallet", "aep11", "transfer",
		"--rpc-endpoint", "http://" + e.RPC.Addresses()[0],
		"--wallet", wall,
	...
	// transfer: good, to AEP-11-Payable contract, with data
...
func TestAEP11_D_OwnerOf_BalanceOf_Transfer(t *testing.T) {
	e := testcli.NewExecutor(t, true)
	tmpDir := t.TempDir()
	...
	// deploy AtipicialFs Object contract
	...
	// check properties
	e.Run(t, "atipicial-go", "wallet", "aep11", "properties",
		"--rpc-endpoint", "http://"+e.RPC.Addresses()[0],
		"--token", h.StringLE(),
	...
	checkBalanceResult := func(t *testing.T, acc string, objs ...idAmount) {
		e.CheckNextLine(t, "^\\s*Account\\s+"+acc)
		e.CheckNextLine(t, "^\\s*NFSO:\\s+AtipicialFs Object NFT \\("+h.StringLE()+"\\)")
	...
	cmdOwnerOf := []string{"atipicial-go", "wallet", "aep11", "ownerOfD",
		"--rpc-endpoint", "http://" + e.RPC.Addresses()[0],
	}
	e.RunWithErrorCheck(t, `Required flag "token" not set`, cmdOwnerOf...)
	cmdOwnerOf = append(cmdOwnerOf, "--token", h.StringLE())
	...
	// transfer: good, 1/4 of the balance, to AEP-11-Payable contract, with data
	verifyH := deployVerifyContract(t, e)
	cmdTransfer = []string{
		"atipicial-go", "wallet", "aep11", "transfer",
		"--rpc-endpoint", "http://" + e.RPC.Addresses()[0],
