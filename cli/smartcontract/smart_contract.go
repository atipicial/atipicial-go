...
var (
	...
	inFlag = &cli.StringFlag{
		Name:    "in",
		Aliases: []string{"i"},
		Usage:   "Input file for the smart contract (*.aef)",
...
func NewCommands() []*cli.Command {
	testInvokeScriptFlags := []cli.Flag{
		&cli.StringFlag{
			...
			Aliases:  []string{"i"},
			Required: true,
			Usage:    "Input location of the .aef file that needs to be invoked",
			Action:   cmdargs.EnsureNotEmpty("in"),
		},
		options.Historic,
	}
	...
	updateFlags := append([]cli.Flag{
		...
		&cli.BoolFlag{
			Name:  "strict",
			Usage: "Fail immediately on AEP-22 nil-data call failure (skip 2-param fallback attempt)",
		},
	}, invokeFunctionFlags...)
	manifestAddGroupFlags := append([]cli.Flag{
		...
		&cli.StringFlag{
			...
			Aliases:  []string{"n"},
			Required: true,
			Usage:    "Path to the AEF file",
			Action:   cmdargs.EnsureNotEmpty("aef"),
		},
		&cli.StringFlag{
			Name:     "manifest",
			Aliases:  []string{"m"},
	...
	return []*cli.Command{{
		...
		Subcommands: []*cli.Command{
			{
				Name:      "compile",
				Usage:     "Compile a smart contract to a .aef file",
				UsageText: "atipicial-go contract compile -i path [-o aef] [-v] [-d] [-m manifest] [-c yaml] [--bindings file] [--no-standards] [--no-events] [--no-permissions] [--guess-eventtypes]",
				Description: `Compiles given smart contract to a .aef file and emits other associated
...
`,
			...
			{
				Name:      "deploy",
				Usage:     "Deploy a smart contract (.aef with description)",
				UsageText: "atipicial-go contract deploy -r endpoint -w wallet [-a address] [-g gas] [-e sysgas] --in contract.aef --manifest contract.manifest.json [--out file] [--force] [--await] [data]",
...
`,
			...
			},
			{
				...
				UsageText: "atipicial-go contract update -r endpoint -w wallet [-a address] [-g gas] [-e sysgas] --in contract.aef --manifest contract.manifest.json [--out file] [--force] [--await] [--strict] scripthash [data...] [--] [signers...]",
...
`,
			...
			},
			{
				...
				UsageText: "atipicial-go contract destroy -r endpoint -w wallet [-a address] [-g gas] [-e sysgas] [--out file] [--force] [--await] scripthash [--] [signers...]",
...
`,
			...
			generateRPCWrapperCmd,
			{
				...
				UsageText: "atipicial-go contract invokefunction -r endpoint -w wallet [-a address] [-g gas] [-e sysgas] [--out file] [--force] [--await] scripthash [method] [arguments...] [--] [signers...]",
				Description: `Executes given (as a script hash or case-insensitive full or shortened native
   contract name, e.g. AtipicialCoin, ATC, Atipicial, atipicial) deployed script with the given method,
...
`,
			...
			},
			{
				...
				UsageText: "atipicial-go contract testinvokefunction -r endpoint [--historic index/hash] scripthash [method] [arguments...] [--] [signers...]",
				Description: `Executes given (as a script hash or case-insensitive full or shortened native
   contract name, e.g. AtipicialCoin, ATC, Atipicial, atipicial) deployed script with the given method,
...
`,
			...
			},
			{
				...
				Usage:     "Invoke compiled AVM code in AEF format on the blockchain (test mode, not creating a transaction for it)",
				UsageText: "atipicial-go contract testinvokescript -r endpoint -i input.aef [--historic index/hash] [signers...]",
				Description: `Executes given compiled AVM instructions in AEF format with the given set of
...
`,
			...
			},
			{
				...
				UsageText: "atipicial-go contract init -n name [--skip-details]",
			...
			},
			{
				...
				UsageText: "atipicial-go contract inspect -i file [-c]",
				Action:    inspect,
				Flags: []cli.Flag{
					&cli.BoolFlag{
						Name:    "compile",
					...
					&cli.StringFlag{
						...
						Aliases:  []string{"i"},
						Required: true,
						Usage:    "Input file of the program (either .go or .aef)",
						Action:   cmdargs.EnsureNotEmpty("in"),
					},
				},
			},
			{
				...
				UsageText: "atipicial-go contract calc-hash -i aef -m manifest -s address",
				Action:    calcHash,
				Flags: []cli.Flag{
					&flags.AddressFlag{
						Name:     "sender",
				...
				Subcommands: []*cli.Command{
					{
						...
						UsageText: "atipicial-go contract manifest add-group -w wallet [--wallet-config path] -n aef -m manifest -a address -s address",
...
func initSmartContract(ctx *cli.Context) error {
	...
	if err := os.WriteFile(filepath.Join(basePath, "atipicial-go.yml"), b, 0644); err != nil {
		return cli.Exit(err, 1)
	}
...
require (
	github.com/atipicial/atipicial-go/pkg/interop ` + ver + `
)`)
...
func calcHash(ctx *cli.Context) error {
	...
	if err != nil {
		return cli.Exit(fmt.Errorf("can't unmarshal .aef file: %w", err), 1)
	}
	...
	fmt.Fprintln(ctx.App.Writer, "Contract hash:", state.CreateContractHash(sender.Uint160(), aefFile.Checksum, m.Name).StringLE())
	return nil
}
...
func testInvokeScript(ctx *cli.Context) error {
	...
	aefFile, err := aef.FileFromBytes(b)
	if err != nil {
		return cli.Exit(fmt.Errorf("failed to restore .aef file: %w", err), 1)
	}
	...
	resp, err := inv.Run(aefFile.Script)
	if err != nil {
		return cli.Exit(err, 1)
	}
...
func inspect(ctx *cli.Context) error {
	...
	if compile {
		...
		if err != nil {
			return cli.Exit(fmt.Errorf("failed to read .aef file: %w", err), 1)
		}
		aefFile, err := aef.FileFromBytes(f)
		if err != nil {
			return cli.Exit(fmt.Errorf("failed to restore .aef file: %w", err), 1)
...
func contractDeploy(ctx *cli.Context) error {
	aefFile, f, err := readAEFFile(ctx.String("in"))
	if err != nil {
		return cli.Exit(err, 1)
	}
	...
	hash := state.CreateContractHash(sender, aefFile.Checksum, m.Name)
...
func contractUpdate(ctx *cli.Context) error {
	...
	if ctx.String("in") != "" {
		_, aefBytes, err = readAEFFile(ctx.String("in"))
		if err != nil {
			return cli.Exit(fmt.Errorf("failed to read .aef file: %w", err), 1)
		}
		aefParam = aefBytes
	...
	if aefBytes == nil && manifestBytes == nil {
		return cli.Exit(fmt.Errorf("either manifest or .aef required"), 1)
	}

	params := []any{aefParam, manifestParam}
	...
	// First, try the AEP-22-style update (script, manifest, data)
	// (or pass nil as “data” for AEP-22 compatibility).
...
func nativeHash(name string) (util.Uint160, error) {
	...
	case strings.ToLower(nativenames.Atipicial), "atipicial":
		return nativehashes.AtipicialCoin, nil
	case strings.ToLower(nativenames.Gas), "gas":
		return nativehashes.AtipicialDollar, nil
