...
import (
	...
	vmcli "github.com/atipicial/atipicial-go/cli/vm"
	"github.com/atipicial/atipicial-go/pkg/services/helpers/atipicialfs"
	"github.com/atipicial/atipicial-go/pkg/vm"
	"github.com/urfave/cli/v2"
)

var atipicialFSFlags = append([]cli.Flag{
	&cli.StringFlag{
		Name:     "container",
		Aliases:  []string{"cid"},
		Usage:    "AtipicialFs container ID to upload objects to",
		Required: true,
		Action:   cmdargs.EnsureNotEmpty("container"),
	},
	&flags.AddressFlag{
		Name:  "address",
		Usage: "Address to use for signing the uploading and searching transactions in AtipicialFs",
	},
	&cli.UintFlag{
		Name:  "retries",
		Usage: "Maximum number of AtipicialFs node request retries",
		Value: atipicialfs.MaxRetries,
	...
	}}, options.AtipicialFsRPC...)

// NewCommands returns util commands for atipicial-go CLI.
func NewCommands() []*cli.Command {
	...
	uploadBinFlags := append([]cli.Flag{
		&cli.StringFlag{
			...
			Value:  atipicialfs.DefaultBlockAttribute,
		...
		&cli.UintFlag{
			...
			Value: atipicialfs.DefaultBatchSize,
		},
		&cli.UintFlag{
			Name:  "workers",
	...
	uploadBinFlags = append(uploadBinFlags, atipicialFSFlags...)

	uploadStateFlags := append([]cli.Flag{
		&cli.StringFlag{
			Name:   "state-attribute",
			Usage:  "Attribute key of the state object",
			Value:  atipicialfs.DefaultStateAttribute,
	...
	uploadStateFlags = append(uploadStateFlags, atipicialFSFlags...)
...
`,
				...
				{
					Name:      "upload-bin",
					Usage:     "Fetch blocks from RPC node and upload them to the AtipicialFs container",
					UsageText: "atipicial-go util upload-bin --fs-rpc-endpoint <address1>[,<address2>[...]] --container <cid> --block-attribute block --rpc-endpoint <node> [--timeout <time>] --wallet <wallet> [--wallet-config <config>] [--address <address>] [--workers <num>] [--batch-size <size>] [--retries <num>] [--debug]",
					Action:    uploadBin,
					Flags:     uploadBinFlags,
				},
				{
					Name:      "audit-bin",
					Usage:     "Audit AtipicialFs container for duplicate or missing block objects",
					UsageText: "atipicial-go util audit-bin -fs-rpc-endpoint <address1>[,<address2>[...]] --container <cid> [--block-attribute Block] [--wallet <wallet>] [--wallet-config <config>] [--address <address>] [--retries <num>] [--debug] [--dry-run]",
					Action:    auditBin,
					Flags:     auditBinFlags,
...
`,
				...
				{
					Name:      "upload-state",
					Usage:     "Start the node, traverse contract storage key-value pairs and upload them to the AtipicialFs container at every StateSyncInterval number of blocks",
					UsageText: "atipicial-go util upload-state --fs-rpc-endpoint <address1>[,<address2>[...]] --container <cid> --state-attribute state --wallet <wallet> [--wallet-config <config>] [--address <address>] [--searchers <num>] [--retries <num>] [--debug] [--config-path path] [-p/-m/-t] [--config-file file] [--force-timestamp-logs]",
					Action:    uploadState,
