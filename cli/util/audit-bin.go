...
func auditBinInt(ctx *cli.Context, tasks chan func() error, errs chan error) (func() error, error) {
	...
	if err != nil {
		if errors.Is(err, options.ErrNoWallet) {
			...
			if err != nil {
				return closer, cli.Exit(fmt.Errorf("no wallet provided and failed to create account for AtipicialFs interaction: %w", err), 1)
			}
		} else {
			return closer, cli.Exit(fmt.Errorf("failed to load account: %w", err), 1)
		}
	}
	signer, atipicialFSPool, err := options.GetAtipicialFsClientPool(ctx, acc)
	if err != nil {
		return closer, cli.Exit(err, 1)
	}
	closer = atipicialFSPool.Close

	var containerID cid.ID
	if err = containerID.DecodeString(cnrID); err != nil {
		return closer, cli.Exit(fmt.Errorf("failed to decode container ID: %w", err), 1)
	}
	if _, err = atipicialFSPool.ContainerGet(ctx.Context, containerID, client.PrmContainerGet{}); err != nil {
	...
	for {
		...
		err = retry(func() error {
			tPage, tCursor, tErr := atipicialFSPool.SearchObjects(ctx.Context, containerID, f, []string{blockAttr}, cursor, signer, client.SearchObjectsOptions{})
			if tErr != nil {
		...
		for _, itm := range page {
			...
			if !curOID.IsZero() && prevH == h {
				if dryRun {
					...
					tasks <- wrapDropBlock(ctx, atipicialFSPool, signer, containerID, itm.ID, retries, prevH, curOID, debug)
				}
				continue
			}
			...
			for ; curH < h; curH++ {
				if dryRun {
					...
					tasks <- wrapRestoreMissingBlock(ctx, rpc, atipicialFSPool, signer, containerID, blockAttr, retries, curH, debug)
				}
			}
			curOID = itm.ID
			prevH = curH
