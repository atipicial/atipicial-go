...
func uploadState(ctx *cli.Context) error {
	...
	signer, p, err := options.GetAtipicialFsClientPool(ctx, acc)
	if err != nil {
		return cli.Exit(err, 1)
	}
	defer p.Close()
	...
	results, errs := atipicialfs.ObjectSearch(ctx.Context, p, acc.PrivateKey(), containerID, filters, []string{attr})
	...
	for state := stateObjIndex + 1; state <= currentStateIndex; state++ {
		...
		var (
			...
			attrs            = []object.Attribute{
				...
				object.NewAttribute(atipicialfs.DefaultStateRootAttribute, stateRoot.Root.StringLE()),
		...
		if len(stateRoot.Witness) > 0 && bytes.Equal(stateRoot.Witness[0].VerificationScript, h.Script.VerificationScript) { // only useful for networks with state validators matching exactly consensus members.
			attrs = append(attrs, object.NewAttribute(atipicialfs.DefaultWitnessAttribute, base64.StdEncoding.EncodeToString(stateRoot.Witness[0].Bytes())))
		}
