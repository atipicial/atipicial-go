...
func TestUtilConvert(t *testing.T) {
	...
	t.Run("from uint160", func(t *testing.T) {
		e.Run(t, "atipicial-go", "util", "convert", util.Uint160{1, 2, 3}.StringLE())
	...
	t.Run("from base64", func(t *testing.T) {
		e.Run(t, "atipicial-go", "util", "convert", "AzAsKhDGXY3es9Sud1UZHlNuaIUeA8RP5L525bksW2X1")
	...
	t.Run("from hex", func(t *testing.T) {
		e.Run(t, "atipicial-go", "util", "convert", "03302c2a10c65d8ddeb3d4ae7755191e536e68851e03c44fe4be76e5b92c5b65f5")
...
func TestUtilOps(t *testing.T) {
	...
	e.Run(t, "atipicial-go", "util", "ops", base64Str) // base64
	check(t)

	e.Run(t, "atipicial-go", "util", "ops", hexStr) // base64 is checked firstly by default, but it's invalid script if decoded from base64
	e.CheckNextLine(t, "INDEX.*OPCODE.*PARAMETER")
	e.CheckNextLine(t, ".*ERROR: incorrect opcode")
	e.CheckEOF(t)

	e.Run(t, "atipicial-go", "util", "ops", "--hex", hexStr) // explicitly specify hex encoding
	check(t)

	e.RunWithError(t, "atipicial-go", "util", "ops", "%&~*") // unknown encoding

	tmp := filepath.Join(t.TempDir(), "script_base64.txt")
	require.NoError(t, os.WriteFile(tmp, []byte(base64Str), os.ModePerm))
	e.Run(t, "atipicial-go", "util", "ops", "--in", tmp) // base64 from file
	check(t)

	tmp = filepath.Join(t.TempDir(), "script_hex.txt")
	require.NoError(t, os.WriteFile(tmp, []byte(hexStr), os.ModePerm))
	e.Run(t, "atipicial-go", "util", "ops", "--hex", "--in", tmp) // hex from file
	check(t)
}
...
func TestUtilCancelTx(t *testing.T) {
	...
	args := []string{"atipicial-go", "util", "canceltx",
	...
	t.Run("invalid", func(t *testing.T) {
		...
		t.Run("not signed by main signer", func(t *testing.T) {
			e.In.WriteString("one\r")
			e.RunWithError(t, "atipicial-go", "util", "canceltx",
				"-r", "http://"+e.RPC.Addresses()[0],
				"--wallet", testcli.ValidatorWallet,
...
func TestUploadBin(t *testing.T) {
	e := testcli.NewExecutor(t, true)
	args := []string{
		"atipicial-go", "util", "upload-bin",
		"--cid", "test",
		"--wallet", "./not-exist.json",
		"--block-attribute", "block",
		"--fsr", "st1.local.fs.atipicial.org:8080",
	}
	e.In.WriteString("one\r")
	...
	e.In.WriteString("one\r")
	e.RunWithErrorCheckExit(t, "failed to dial AtipicialFs pool", append(args, "--cid", "9iVfUg8aDHKjPC4LhQXEkVUM4HDkR7UCXYLs8NQwYfSG", "--wallet", testcli.ValidatorWallet, "--rpc-endpoint", "http://"+e.RPC.Addresses()[0])...)
}
